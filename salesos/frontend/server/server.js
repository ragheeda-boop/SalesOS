...
app.get("/api/v1/dashboard", (req, res) => {
  res.json({
    ...
    intelligenceFeed: {
      data: {
        items: [
          { id: "i1", type: "signal", title: "إشارة توسع", severity: "high", timestamp: now },
        ],
      },
      status: "ready",
    },
    aiBrief: {
      data: { summary: "3 فرص استراتيجية جديدة", highlights: ["شراكة محتملة"], generatedAt: now },
      status: "ready",
    },
    marketPulse: {
      data: { topMovers: [{ name: "قطاع الطاقة", change: 2.5 }], trends: ["ارتفاع الطلب"] },
      status: "ready",
    },
    recentActivity: {
      data: {
        items: [
          { id: "a1", type: "meeting", title: "اجتماع", companyName: "أرامكو", timestamp: now },
        ],
        total: 1,
      },
      status: "ready",
...
app.get("/api/v1/companies/:id/intelligence", (req, res) => {
  res.json({
    ...
    dna: {
      ...
      technologyProfile: { erp: "sap" },
      financialHealth: { score: 82, revenue: 1200000000, growth: 12.5, trend: "up" },
      governmentExposure: { level: "high", contracts: 45 },
      expansionPotential: { score: 72, markets: ["UAE"] },
    ...
    relationships: {
      nodes: [{ id: "n1", type: "person", label: "د. أحمد", strength: 0.9 }],
      edges: [],
    },
    timeline: [
      {
    ...
    buyingJourney: {
      ...
      stageDescription: "تقييم الموردين",
    },
    goldenRecord: [
      {
        id: "gr1",
...
app.post("/api/v1/search", (req, res) => {
  const results = Array.from({ length: 5 }, (_, i) => ({
    id: `r_${i}`,
    entity_type: i === 0 ? "company" : "signal",
...
app.post("/api/v1/search/ai", (req, res) => {
  res.json({
    summary: `تحليل: ${req.body.text}`,
    recommendations: ["التواصل"],
    risks: [],
    sources: [],
    confidence: 0.85,
    tokens: 450,
