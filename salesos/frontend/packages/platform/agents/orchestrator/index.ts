...
import { evaluateDecision } from "../decisionHttp";
...
export async function executeTask(taskId: string): Promise<AgentResult> {
  ...
  try {
    ...
    // Decision Center HTTP — not the FE STUB package (PROD-W6-001)
    const decision = await evaluateDecision(decContext);
    ...
    for (const toolName of agent.tools) {
      ...
      try {
        ...
        if (
          ...
          "opportunityId" in (result as Record<string, unknown>)
        ) {
        ...
        if (
          result &&
          typeof result === "object" &&
          "taskId" in (result as Record<string, unknown>)
        ) {
