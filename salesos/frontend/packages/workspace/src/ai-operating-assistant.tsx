...
const QUICK_ACTIONS: QuickAction[] = [
  {
    ...
    query:
      "اعثر على أعلى الصفقات احتمالية في الرياض التي لديها إشارات توظيف في آخر 30 يوماً وأنشئ متابعة",
  },
  {
    id: "qa-revenue-risk",
...
export function AIOperatingAssistant({
  open,
  onClose,
  onExecute,
  className,
}: AIOperatingAssistantProps) {
        ...
        return new Promise<string>((resolve) => {
          ...
          setWorkflow((prev) => {
            ...
            return {
              ...
              steps: [
                ...prev.steps,
                { id: stepId, action, status: "executing" as WorkflowStep, timestamp: Date.now() },
              ],
            };
          });
          setTimeout(() => {
        ...
        setWorkflow((prev) => {
          ...
          return {
            ...
            steps: prev.steps.map((s) =>
              s.id === stepId
                ? { ...s, status: "completed" as WorkflowStep, result, timestamp: Date.now() }
                : s
            ),
          };
        });
          ...
          steps: [
            {
              ...
              action: "فهم الطلب",
              status: "completed",
              result: "تم فهم الطلب: " + q.slice(0, 50),
              timestamp: Date.now(),
            },
            {
              id: "s2",
              action: "البحث في البيانات",
              status: "completed",
              result: "تم العثور على 15 نتيجة ذات صلة",
              timestamp: Date.now(),
            },
            {
              id: "s3",
              action: "تحليل النتائج",
              status: "completed",
              result: "تم تحليل 15 نتيجة",
              timestamp: Date.now(),
            },
            {
              id: "s4",
              action: "توليد التوصيات",
              status: "completed",
              result: "تم توليد 3 توصيات",
              timestamp: Date.now(),
            },
            {
              id: "s5",
              action: "تنفيذ الإجراءات",
              status: "completed",
              result: "تم إنشاء تسلسل متابعة",
              timestamp: Date.now(),
            },
          ],
        };
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-[10px] text-gray-500">
                    {h.completedAt
                      ? `منذ ${Math.floor((Date.now() - h.completedAt) / 60000)} دقيقة`
                      : ""}
