...
function ErrorState({ message, onRetry }: { message: string; onRetry: () => void }) {
  return (
    ...
    >
      ...
      <p style={{ margin: '0.25rem 0 0', fontSize: '0.75rem', color: 'var(--danger-600, #b91c1c)' }}>{message}</p>
      <button
        onClick={onRetry}
        style={{
          marginTop: '0.75rem',
  ...
  const WidgetComponent = memo(function WidgetComponent() {
    ...
    useEffect(() => {
      if (status === 'error' && !telemetrySent.current) {
        ...
        toast({ variant: 'error', title: `فشل تحميل: ${config.metadata.title}`, description: error?.message })
