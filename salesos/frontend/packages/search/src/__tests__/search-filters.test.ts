...
describe("buildFilterQuery", () => {
  it("builds eq filter", () => {
    expect(buildFilterQuery([{ field: "industry", operator: "eq", value: "energy" }])).toBe(
      "industry:energy"
    );
  });

  it("builds neq filter", () => {
    expect(buildFilterQuery([{ field: "industry", operator: "neq", value: "energy" }])).toBe(
      "-industry:energy"
    );
  });

  it("builds gt/gte/lt/lte filters", () => {
    expect(buildFilterQuery([{ field: "revenue", operator: "gt", value: "1000000" }])).toBe(
      "revenue>1000000"
    );
    expect(buildFilterQuery([{ field: "revenue", operator: "gte", value: "1000000" }])).toBe(
      "revenue>=1000000"
    );
  });
  ...
  it("builds in filter", () => {
    expect(
      buildFilterQuery([{ field: "region", operator: "in", value: ["riyadh", "jeddah"] }])
    ).toBe("region:(riyadh,jeddah)");
  });
