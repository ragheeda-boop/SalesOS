...
export interface CompanyWidgetMap {
  ...
  relationshipGraph: {
    data: {
      nodes: CompanyIntelligenceDTO["relationships"]["nodes"];
      edges: CompanyIntelligenceDTO["relationships"]["edges"];
    } | null;
    status: WidgetStatus;
    lastUpdated: string | null;
    error: Error | null;
...
export function deriveCompanyIntelligenceWidgets(
  ...
  return {
    ...
    relationshipGraph: w(
      dto?.relationships
        ? { nodes: dto.relationships.nodes, edges: dto.relationships.edges }
        : null,
      isLoading,
      isError,
      lu,
