...
export const handlers = [
  ...
  http.get("/api/v1/dashboard", () => {
    return HttpResponse.json({
      ...
      missionCenter: {
        data: {
          mission: "زيادة الإيرادات بنسبة 15% هذا الربع",
          progress: 62,
          keyResults: [
            { label: "الصفقات المغلقة", current: 18, target: 25 },
            { label: "الاجتماعات", current: 42, target: 60 },
            { label: "الإشارات الجديدة", current: 85, target: 100 },
          ],
          updatedAt: now,
        },
        status: "ready",
          ...
          items: [
            {
              id: "i1",
              type: "signal",
              title: "إشارة توسع من أرامكو",
              summary: "خطة توسعية في الطاقة المتجددة",
              severity: "high",
              timestamp: now,
            },
            {
              id: "i2",
              type: "news",
              title: "STC تعلن عن شراكة",
              summary: "اتفاقية مع شركة تقنية",
              severity: "medium",
              timestamp: now,
            },
          ],
  ...
  http.get("/api/v1/companies/:id/intelligence", ({ params }) => {
    ...
    return HttpResponse.json({
      ...
      dna: {
        industry,
        businessModel: industry === "energy" ? "b2b" : "b2c",
      ...
      relationships: {
        nodes: [
          { id: "n1", type: "person", label: "د. أحمد", strength: 0.9 },
      ...
      timeline: [
        {
          id: "t1",
          type: "signal",
          summary: "إعلان توسع في المنطقة الشرقية",
          date: now,
          source: "News",
          aiHighlighted: true,
          confidence: 0.92,
      ...
      signals: [
        {
          id: "s1",
          type: "hiring",
          title: "توظيف 120 مهندس",
          description: "خطة توسعية",
          source: "LinkedIn",
          severity: "high",
          timestamp: now,
          aiConfidence: 0.92,
      ...
      documents: [
        {
          id: "d1",
          title: "عقد توريد 2026",
          type: "contract",
          date: now,
          aiSummary: "عقد توريد مواد بـ 2 مليون ريال",
          confidence: 0.92,
        },
      ],
      buyingJourney: {
        ...
        stageDescription: "العميل يقيم الموردين المحتملين",
      },
      goldenRecord: [
        {
          id: "gr1",
          entityName: company?.name_ar ?? "شركة",
          source: "CRM",
          confidence: 0.98,
          conflicts: [],
          freshness: now,
      ...
      firmographic: {
        nameAr: company?.name_ar ?? "شركة",
        nameEn: company?.name_en ?? "Company",
      ...
      facets: [
        {
          ...
          label: "القطاع",
          values: [
            { value: "energy", count: 15 },
            { value: "healthcare", count: 10 },
          ],
        },
      ],
      queryInterpretation: {
        original: body.text ?? "",
        interpreted: `نتائج عن"${body.text ?? ""}"`,
        entities: [],
        intent: "search",
        confidence: 0.9,
      },
  ...
  http.post("/api/v1/search/suggest", async ({ request }) => {
    ...
    return HttpResponse.json({
      suggestions: [
        {
          text: `${body.prefix ?? ""} في قطاع الطاقة`,
          type: "query",
          score: 0.95,
        },
        { text: `${body.prefix ?? ""} في الرياض`, type: "query", score: 0.8 },
      ],
    });
  }),
