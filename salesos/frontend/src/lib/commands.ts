...
export function registerBuiltinCommands(router: ReturnType<typeof useRouter>) {
  ...
  registerCommand({
    ...
    description:
      "GTM tip hub (ICP + market sizing + discovery + lookalikes + enrichment + verification + sequences)",
    category: "تنقل",
  ...
  registerCommand({
    id: "go.gtm.enrichment",
    label: "GTM · Enrichment Waterfall",
  ...
  registerCommand({
    id: "go.gtm.lookalikes",
    label: "GTM · Lookalike Accounts",
    description: "Won/lost lookalike ranking (tip STORY-11-04)",
    category: "تنقل",
    handler: () => router.push("/gtm/lookalikes"),
  });

  registerCommand({
    id: "go.gtm.sequences",
    label: "GTM · Email Sequences",
    description: "Email sequencing engine (tip STORY-11-09)",
    category: "تنقل",
    handler: () => router.push("/gtm/sequences"),
  });
