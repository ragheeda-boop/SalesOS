...
describe('searchCompanies — contract', () => {
  it('returns PaginatedResponse<Company>', async () => {
    const payload = {
      ...
      page_size: 10,
      items: [
        { id: 'c-1', name_ar: 'أرامكو', name_en: 'Aramco', cr_number: '1010000001', status: 'نشط', city: 'الظهران', region: null, phone: null, email: null, confidence_score: 0.9, created_at: '2026-01-01T00:00:00Z', updated_at: '2026-07-10T00:00:00Z' },
      ],
    }
    ...
    expect(mockAxios.get).toHaveBeenCalledWith('/api/v1/companies', expect.objectContaining({
      headers: { 'X-Tenant-Id': TENANT },
    }))
  })

  it('strips cursor from params for offset pagination', async () => {
    mockAxios.get.mockResolvedValueOnce(mockResponse({ total: 0, page: 1, page_size: 10, items: [] }))
...
describe('searchCompaniesCursor — contract', () => {
  it('returns CursorResponse<Company>', async () => {
    const payload = {
      data: [{ id: 'c-1', name_ar: 'أرامكو', name_en: 'Aramco', cr_number: '101', status: 'نشط', city: null, region: null, phone: null, email: null, confidence_score: null, created_at: '2026-01-01', updated_at: '2026-07-10' }],
    ...
    const { searchCompaniesCursor } = require('../api')
...
describe('getCompany — contract', () => {
  it('returns CompanyDetail with branches, licenses, contacts', async () => {
    const payload = {
      id: 'c-1', name_ar: 'أرامكو', name_en: 'Aramco', cr_number: '101', status: 'نشط',
      city: 'الظهران', region: null, phone: null, email: null, confidence_score: 0.9,
      created_at: '2026-01-01', updated_at: '2026-07-10',
      branches: [{ id: 'b-1', name: 'فرع الرياض', city: 'الرياض', region: null, phone: null }],
      licenses: [{ id: 'l-1', license_type: 'تجارية', license_number: 'L001', status: 'نشط', issue_date: null, expiry_date: null }],
      contacts: [{ id: 'ct-1', name: 'أحمد', email: null, phone: null, position: 'مدير', company_id: 'c-1' }],
    }
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { getCompany } = require('../api')
...
describe('createCompany — contract', () => {
  it('returns created Company', async () => {
    const payload = { id: 'c-new', name_ar: 'شركة جديدة', cr_number: '999', status: 'نشط', created_at: '2026-07-16', updated_at: '2026-07-16', name_en: null, city: null, region: null, phone: null, email: null, confidence_score: null }
    mockAxios.post.mockResolvedValueOnce(mockResponse(payload))

    const { createCompany } = require('../api')
    const result = await createCompany({ name_ar: 'شركة جديدة', cr_number: '999' }, TENANT)

    expect(result.id).toBe('c-new')
    expect(result.name_ar).toBe('شركة جديدة')
    expect(mockAxios.post).toHaveBeenCalledWith('/api/v1/companies', expect.anything(), expect.objectContaining({
      headers: { 'X-Tenant-Id': TENANT },
    }))
  })
})
...
describe('updateCompany — contract', () => {
  it('returns updated Company', async () => {
    const payload = { id: 'c-1', name_ar: 'أرامكو المحدّثة', cr_number: '101', status: 'نشط', created_at: '2026-01-01', updated_at: '2026-07-16', name_en: null, city: null, region: null, phone: null, email: null, confidence_score: null }
    mockAxios.patch.mockResolvedValueOnce(mockResponse(payload))

    const { updateCompany } = require('../api')
    const result = await updateCompany('c-1', { name_ar: 'أرامكو المحدّثة' }, TENANT)

    expect(result.name_ar).toBe('أرامكو المحدّثة')
    expect(mockAxios.patch).toHaveBeenCalledWith('/api/v1/companies/c-1', expect.anything(), expect.anything())
  })
})
...
describe('deleteCompany — contract', () => {
  it('sends DELETE and returns void', async () => {
    ...
    const { deleteCompany } = require('../api')
    await deleteCompany('c-1', TENANT)

    expect(mockAxios.delete).toHaveBeenCalledWith('/api/v1/companies/c-1', expect.objectContaining({
      headers: { 'X-Tenant-Id': TENANT },
    }))
  })
})
...
describe('searchContacts — contract', () => {
  it('returns PaginatedResponse<Contact>', async () => {
    const payload = {
      ...
      page_size: 10,
      items: [
        { id: 'ct-1', name: 'أحمد', name_ar: null, email: 'a@test.com', phone: null, mobile: null, position: 'مدير', position_ar: null, department: null, company_id: 'c-1', company_name: null, is_primary: false, source: null, confidence_score: null, tags: [], created_at: '2026-01-01', updated_at: '2026-07-10' },
      ],
    }
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { searchContacts } = require('../api')
...
describe('getContact — contract', () => {
  it('returns single Contact', async () => {
    const payload = { id: 'ct-1', name: 'أحمد', email: 'a@test.com', phone: null, position: 'مدير', company_id: 'c-1' }
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { getContact } = require('../api')
...
describe('createContact — contract', () => {
  it('returns created Contact', async () => {
    const payload = { id: 'ct-new', name: 'نورة', email: 'n@test.com', phone: null, position: 'مديرة', company_id: 'c-1' }
    mockAxios.post.mockResolvedValueOnce(mockResponse(payload))

    const { createContact } = require('../api')
    const result = await createContact({ name: 'نورة', email: 'n@test.com' }, TENANT)

    expect(result.id).toBe('ct-new')
    expect(mockAxios.post).toHaveBeenCalledWith('/api/v1/contacts', expect.anything(), expect.objectContaining({
      headers: { 'X-Tenant-Id': TENANT },
    }))
  })
})
...
describe('updateContact — contract', () => {
  it('returns updated Contact via PATCH', async () => {
    ...
    const { updateContact } = require('../api')
    const result = await updateContact('ct-1', { name: 'أحمد المحدّث' }, TENANT)

    expect(result.name).toBe('أحمد المحدّث')
    expect(mockAxios.patch).toHaveBeenCalledWith('/api/v1/contacts/ct-1', expect.anything(), expect.anything())
  })
})
...
describe('deleteContact — contract', () => {
  it('sends DELETE', async () => {
    ...
    const { deleteContact } = require('../api')
    await deleteContact('ct-1', TENANT)

    expect(mockAxios.delete).toHaveBeenCalledWith('/api/v1/contacts/ct-1', expect.objectContaining({
      headers: { 'X-Tenant-Id': TENANT },
    }))
  })
})
...
describe('getContactsByCompany — contract', () => {
  it('returns Contact[]', async () => {
    const payload = [{ id: 'ct-1', name: 'أحمد', email: null, phone: null, position: 'مدير', company_id: 'c-1' }]
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { getContactsByCompany } = require('../api')
...
describe('searchEmployees — contract', () => {
  it('returns CursorResponse<EmployeeListItem>', async () => {
    const payload = {
      data: [{ id: 'e-1', full_name: 'خالد', full_name_ar: null, email: 'k@test.com', role: 'admin', department: null, phone: null, avatar_url: null, is_active: true, signal_count: 5, score: 82, score_trend: 'up', confidence: 0.9, created_at: '2026-01-01' }],
      next_cursor: null,
      previous_cursor: null,
      has_next: false,
      has_previous: false,
    ...
    const { searchEmployees } = require('../api')
...
describe('getEmployee360 — contract', () => {
  it('returns Employee360Response with all sections', async () => {
    const payload = {
      profile: { id: 'e-1', full_name: 'خالد', full_name_ar: null, email: 'k@test.com', role: 'admin', phone: null, avatar_url: null, is_active: true, tenant_id: TENANT, created_at: '2026-01-01', team: [], manager: null },
    ...
    const { getEmployee360 } = require('../api')
...
describe('getEmployeeScore — contract', () => {
  it('returns EmployeeScoreResponse', async () => {
    const payload = { score: 82, trend: 'up', confidence: 0.9, factors: [{ name: 'engagement', contribution: 12, signal_type: 'activity', label: 'التفاعل' }] }
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { getEmployeeScore } = require('../api')
...
describe('getEmployeeTimeline — contract', () => {
  it('returns EmployeeTimelineResponse with cursor pagination', async () => {
    const payload = {
      events: [{ id: 'ev-1', action: 'created', title: 'تم الإنشاء', source: 'crm', source_label: 'CRM', timestamp: '2026-07-16T00:00:00Z', actor: 'خالد' }],
    ...
    const { getEmployeeTimeline } = require('../api')
...
describe('bulkEditEmployees — contract', () => {
  it('returns { updated: number }', async () => {
    ...
    const { bulkEditEmployees } = require('../api')
...
describe('bulkDeleteEmployees — contract', () => {
  it('returns { deleted: number }', async () => {
    ...
    const { bulkDeleteEmployees } = require('../api')
...
describe('exportEmployees — contract', () => {
  it('returns Blob', async () => {
    ...
    const { exportEmployees } = require('../api')
...
describe('unifiedSearch — contract', () => {
  it('returns SearchResponse with items, facets, suggestions', async () => {
    const payload = {
      ...
      total: 10,
      took_ms: 150,
      items: [{ id: 'r-1', type: 'company', score: 0.95, data: {}, matched_fields: ['name_ar'], explanation: null }],
      facets: { industry: { energy: 5 } },
      suggestions: ['أرامكو السعودية'],
    }
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { unifiedSearch } = require('../api')
  ...
  it('omits facets when include_facets is false', async () => {
    mockAxios.get.mockResolvedValueOnce(mockResponse({ query: 'test', strategy: 'fulltext', total: 0, took_ms: 5, items: [] }))
...
describe('getExecutiveDashboard — contract', () => {
  it('returns ExecutiveDashboardResponse', async () => {
    const payload = {
      revenue: { total_booked: 1000000, total_pipeline: 5000000, weighted_pipeline: 2500000, forecast: 3000000, growth_percent: 15 },
      team: { total_employees: 50, active_employees: 45, top_performers: [], avg_win_rate: 0.42 },
      risk: { expiring_contracts: 3, stalled_deals: 5, inactive_companies: 10, low_pipeline_employees: 2 },
      health: { overall_health: 'good', data_completeness: 0.85, sync_status: 'ok', last_activity: '2026-07-16' },
      pipeline: { total_deals: 30, total_value: 5000000, won_deals: 10, lost_deals: 5, win_rate: 0.33, avg_deal_size: 166666, by_stage: [] },
      renewals: { due_next_30_days: 2, due_next_90_days: 5, total_renewal_value: 200000, at_risk: [] },
      growth: { new_companies_30d: 8, new_contacts_30d: 25, new_opportunities_30d: 12, new_contracts_30d: 4 },
    }
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { getExecutiveDashboard } = require('../api')
...
describe('listOpportunities — contract', () => {
  it('returns OpportunityListResponse', async () => {
    const payload = {
      items: [{ id: 'opp-1', name: 'صفقة أرامكو', stage: 'proposal', value: 500000, company_id: 'c-1', probability: 0.6, expected_close_date: '2026-08-01' }],
      total: 1,
    }
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { listOpportunities } = require('../api')
...
describe('createOpportunity — contract', () => {
  it('returns created Opportunity', async () => {
    mockAxios.post.mockResolvedValueOnce(mockResponse({ id: 'opp-new', name: 'صفقة جديدة', stage: 'qualification', value: 100000, company_id: 'c-1' }))

    const { createOpportunity } = require('../api')
...
describe('listPipelines — contract', () => {
  it('returns PipelineListResponse', async () => {
    mockAxios.get.mockResolvedValueOnce(mockResponse({ items: [{ id: 'p-1', name: 'Pipeline رئيسي', stages: 5 }] }))

    const { listPipelines } = require('../api')
...
describe('getEntityActivities — contract', () => {
  it('returns EntityActivityResponse', async () => {
    const payload = {
      entity_type: 'company',
      entity_id: 'c-1',
      items: [{ id: 'a-1', tenant_id: TENANT, actor: 'user-1', action: 'created', entity_type: 'company', entity_id: 'c-1', timestamp: '2026-07-16T00:00:00Z' }],
      total: 1,
    }
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { getEntityActivities } = require('../api')
...
describe('login — contract', () => {
  it('returns tokens and stores them', async () => {
    mockAxios.post.mockResolvedValueOnce(mockResponse({ access_token: 'at_123', refresh_token: 'rt_123', token_type: 'bearer' }))
...
describe('register — contract', () => {
  it('returns tokens and stores them', async () => {
    mockAxios.post.mockResolvedValueOnce(mockResponse({ access_token: 'at_456', refresh_token: 'rt_456', token_type: 'bearer' }))
...
describe('getCurrentUser — contract', () => {
  it('returns UserProfile', async () => {
    const payload = { id: 'u-1', email: 'user@test.com', full_name: 'خالد', full_name_ar: null, role: 'admin', is_active: true, is_verified: true, tenant_id: TENANT, created_at: '2026-01-01', updated_at: '2026-07-16' }
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))
...
describe('getCompany360 — contract', () => {
  it('returns Company360Response with all sections', async () => {
    const payload = {
      company: { id: 'c-1', name_ar: 'أرامكو', name_en: null, cr_number: '101', status: 'نشط', city: null, region: null, phone: null, email: null, confidence_score: null, created_at: '2026-01-01', updated_at: '2026-07-16', branches: [], licenses: [], contacts: [] },
      overview: { total_contacts: 10, total_opportunities: 5, total_revenue: 1000000, active_contracts: 2, pending_tasks: 3, upcoming_meetings: 1, last_activity: null, signal_count: 8, contacts_page: 1, contacts_total: 10, opportunities_page: 1, opportunities_total: 5, timeline_page: 1, timeline_total: 20 },
      organization: { branches: [], departments: [], employees_count: 0, legal_form: null, incorporation_date: null },
      contacts: [],
      assigned_employees: [],
      opportunities: [],
      contracts: [],
    ...
    const { getCompany360 } = require('../api')
...
describe('listDlq — contract', () => {
  it('returns PaginatedResponse<DlqEntry>', async () => {
    const payload = {
      ...
      page: 1,
      page_size: 10,
      items: [{ id: 1, source_slug: 'scraper', cr_number: '101', stage: 'enrichment', error_message: 'timeout', error_type: 'TimeoutError', retry_count: 1, max_retries: 3, status: 'failed', created_at: '2026-07-16', last_retry_at: null }],
    }
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { listDlq } = require('../api')
...
describe('retryDlq — contract', () => {
  it('returns DlqRetryResponse', async () => {
    mockAxios.post.mockResolvedValueOnce(mockResponse({ processed: 10, retried: 5, resolved: 3, still_failed: 2 }))

    const { retryDlq } = require('../api')
...
describe('listGoldenRecords — contract', () => {
  it('returns PaginatedResponse<GoldenRecordAdmin>', async () => {
    const payload = {
      ...
      page: 1,
      page_size: 10,
      items: [{ id: 'gr-1', tenant_id: TENANT, cr_number: '101', company_name_ar: 'أرامكو', status: 'clean', confidence_score: 0.95, source_records: 3, created_at: '2026-07-16', updated_at: '2026-07-16' }],
    }
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { listGoldenRecords } = require('../api')
...
describe('listConflicts — contract', () => {
  it('returns PaginatedResponse<EntityResolutionConflict>', async () => {
    const payload = {
      ...
      page: 1,
      page_size: 10,
      items: [{ id: 'cf-1', tenant_id: TENANT, cr_number_a: '101', cr_number_b: '102', status: 'pending', reason: 'duplicate_cr', created_at: '2026-07-16' }],
    }
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { listConflicts } = require('../api')
...
describe('listTasks — contract', () => {
  it('returns TaskResponse[]', async () => {
    const payload = [{ id: 't-1', title: 'متابعة أرامكو', priority: 'high', source: 'copilot', company_id: 'c-1', completed: false, created_at: '2026-07-16' }]
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))
...
describe('completeTask — contract', () => {
  it('returns completed TaskResponse', async () => {
    mockAxios.put.mockResolvedValueOnce(mockResponse({ id: 't-1', title: 'متابعة', priority: 'high', source: 'copilot', completed: true }))

    const { completeTask } = require('../api')
...
describe('submitCopilotFeedback — contract', () => {
  it('returns CopilotFeedbackResponse', async () => {
    mockAxios.post.mockResolvedValueOnce(mockResponse({ success: true, helpful_rate: 0.85, total_ratings: 20 }))

    const { submitCopilotFeedback } = require('../api')
...
describe('getCopilotTelemetry — contract', () => {
  it('returns CopilotTelemetryData with all sections', async () => {
    const payload = {
      summary: { total_calls: 100, success_rate: 0.92, avg_latency_ms: 250, p95_latency_ms: 500 },
      tools: [],
    ...
    const { getCopilotTelemetry } = require('../api')
...
describe('listAdminTenants — contract', () => {
  it('returns AdminTenantListItem[]', async () => {
    const payload = [{ id: 't-1', name: 'SalesOS', slug: 'salesos', domain: null, plan: 'enterprise', is_active: true, user_count: 10, created_at: '2026-01-01', updated_at: '2026-07-16' }]
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { listAdminTenants } = require('../api')
...
describe('listAdminPlans — contract', () => {
  it('returns AdminPlan[]', async () => {
    const payload = [{ id: 'p-1', name: 'Enterprise', tier: 'enterprise', price_monthly: 999, price_yearly: 9990, max_users: 100, max_storage_mb: 10000, max_api_calls: 1000000, features: ['sso', 'api'], is_active: true, created_at: '2026-01-01', updated_at: '2026-07-16' }]
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { listAdminPlans } = require('../api')
...
describe('listAdminUsers — contract', () => {
  it('returns AdminUser[]', async () => {
    const payload = [{ id: 'u-1', email: 'admin@test.com', full_name: 'Admin', full_name_ar: null, role: 'admin', is_active: true, is_verified: true, tenant_id: TENANT, tenant_name: 'SalesOS', created_at: '2026-01-01', last_login_at: null }]
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { listAdminUsers } = require('../api')
...
describe('getAdminDetailedHealth — contract', () => {
  it('returns AdminDetailedHealth', async () => {
    const payload = {
      ...
      components: [{ component: 'database', status: 'healthy', latency_ms: 5, last_check: '2026-07-16T00:00:00Z', details: null }],
    }
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { getAdminDetailedHealth } = require('../api')
...
describe('getAdminAICostSummary — contract', () => {
  it('returns AdminAICostSummary', async () => {
    const payload = { total_cost: 150.5, total_tokens: 500000, by_model: [], by_tenant: [], by_operation: [] }
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { getAdminAICostSummary } = require('../api')
...
describe('listAdminAuditLogs — contract', () => {
  it('returns PaginatedResponse<AuditLogEntry>', async () => {
    const payload = {
      ...
      page: 1,
      page_size: 10,
      items: [{ id: 'al-1', action: 'login', action_type: 'auth', actor_id: 'u-1', actor_name: 'Admin', actor_email: 'admin@test.com', resource: 'session', resource_type: 'auth', resource_id: 's-1', tenant_id: TENANT, tenant_name: 'SalesOS', details: null, ip_address: '127.0.0.1', user_agent: null, created_at: '2026-07-16' }],
    }
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { listAdminAuditLogs } = require('../api')
...
describe('error handling — error propagation', () => {
  it('propagates axios errors when API call fails', async () => {
    ...
    ;(apiError as any).response = { status: 500, data: { detail: 'Internal Server Error' } }
  ...
  it('propagates 4xx errors with error data', async () => {
    ...
    ;(apiError as any).response = { status: 400, data: { detail: 'Bad request', errors: { field: 'name' } } }
    mockAxios.post.mockRejectedValueOnce(apiError)

    const { createCompany } = require('../api')
    await expect(createCompany({ name_ar: '', cr_number: '' }, TENANT)).rejects.toThrow('Request failed with status code 400')
  })

  it('propagates 422 validation errors', async () => {
    const apiError = new Error('Request failed with status code 422')
    ;(apiError as any).response = { status: 422, data: { detail: [{ loc: ['body', 'email'], msg: 'field required', type: 'value_error' }] } }
    mockAxios.post.mockRejectedValueOnce(apiError)

    const { createContact } = require('../api')
    await expect(createContact({ name: '' }, TENANT)).rejects.toThrow('Request failed with status code 422')
  })
})
...
describe('getMy360 — contract', () => {
  it('returns Employee360Response for current user', async () => {
    ...
    const { getMy360 } = require('../api')
    const result = await getMy360(TENANT)

    expect(result).toHaveProperty('profile')
    expect(result).toHaveProperty('kpis')
    expect(result).toHaveProperty('ai_coach')
    expect(mockAxios.get).toHaveBeenCalledWith('/api/v1/employees/me/360', expect.objectContaining({
      headers: { 'X-Tenant-Id': TENANT },
    }))
  })
})
...
describe('getEmployeeSignals — contract', () => {
  it('returns EmployeeSignalsResponse', async () => {
    ...
    const { getEmployeeSignals } = require('../api')
...
describe('getEmployeePerformance — contract', () => {
  it('returns EmployeePerformanceResponse', async () => {
    const payload = {
      score_trend: [{ date: '2026-07-10', score: 80 }, { date: '2026-07-16', score: 82 }],
      peer_comparison: [{ metric: 'pipeline', employee_value: 500000, department_avg: 350000, label: 'Pipeline' }],
      risk_flags: [{ type: 'stalled', label: 'صفقة متوقفة', severity: 'medium', description: 'No activity in 30 days' }],
      factors: [{ name: 'engagement', contribution: 12, signal_type: 'activity', label: 'التفاعل' }],
      current_score: 82,
    ...
    const { getEmployeePerformance } = require('../api')
...
describe('addCompanyContact — contract', () => {
  it('returns Contact via company endpoint', async () => {
    const payload = { id: 'ct-new', name: 'مريم', email: 'm@test.com', phone: null, position: 'محاسبة', company_id: 'c-1' }
    mockAxios.post.mockResolvedValueOnce(mockResponse(payload))

    const { addCompanyContact } = require('../api')
    const result = await addCompanyContact('c-1', { name: 'مريم', position: 'محاسبة' }, TENANT)

    expect(result).toHaveProperty('id')
    expect(result.name).toBe('مريم')
    expect(mockAxios.post).toHaveBeenCalledWith('/api/v1/companies/c-1/contacts', expect.anything(), expect.objectContaining({
      headers: { 'X-Tenant-Id': TENANT },
    }))
  })
})
...
describe('advanceOpportunity — contract', () => {
  it('posts to advance stage', async () => {
    mockAxios.post.mockResolvedValueOnce(mockResponse({ id: 'opp-1', stage: 'negotiation', name: 'صفقة', company_id: 'c-1' }))

    const { advanceOpportunity } = require('../api')
    const result = await advanceOpportunity('opp-1', 'negotiation')

    expect(result.stage).toBe('negotiation')
    expect(mockAxios.post).toHaveBeenCalledWith('/api/v1/opportunities/opp-1/advance', null, expect.objectContaining({
...
describe('closeWon — contract', () => {
  it('posts to won endpoint', async () => {
    mockAxios.post.mockResolvedValueOnce(mockResponse({ id: 'opp-1', stage: 'closed_won', won_amount: 500000 }))

    const { closeWon } = require('../api')
    const result = await closeWon('opp-1', 500000)

    expect(result.stage).toBe('closed_won')
    expect(mockAxios.post).toHaveBeenCalledWith('/api/v1/opportunities/opp-1/won', null, expect.objectContaining({
...
describe('closeLost — contract', () => {
  it('posts to lost endpoint', async () => {
    mockAxios.post.mockResolvedValueOnce(mockResponse({ id: 'opp-1', stage: 'closed_lost', loss_reason: 'budget' }))

    const { closeLost } = require('../api')
    const result = await closeLost('opp-1', 'budget')

    expect(result.stage).toBe('closed_lost')
    expect(mockAxios.post).toHaveBeenCalledWith('/api/v1/opportunities/opp-1/lost', null, expect.objectContaining({
...
describe('getGlobalActivities — contract', () => {
  it('returns ActivityQueryResponse', async () => {
    ...
    const { getGlobalActivities } = require('../api')
...
describe('queryActivities — contract', () => {
  it('returns ActivityQueryResponse with query params', async () => {
    ...
    const { queryActivities } = require('../api')
    const result = await queryActivities({ entity_type: 'company', limit: 10 }, TENANT)

    expect(result.total).toBe(0)
    expect(mockAxios.get).toHaveBeenCalledWith('/api/v1/activities', expect.objectContaining({
...
describe('createTask — contract', () => {
  it('returns created TaskResponse', async () => {
    const payload = { id: 't-new', title: 'مهمة جديدة', priority: 'high', source: 'manual', company_id: null, completed: false }
    mockAxios.post.mockResolvedValueOnce(mockResponse(payload))
    ...
    expect(mockAxios.post).toHaveBeenCalledWith('/api/v1/tasks', expect.objectContaining({ title: 'مهمة جديدة' }), expect.anything())
  })
})

describe('getDlqStats — contract', () => {
  it('returns failed_by_stage map', async () => {
    mockAxios.get.mockResolvedValueOnce(mockResponse({ failed_by_stage: { enrichment: 5, validation: 2 } }))

    const { getDlqStats } = require('../api')
...
describe('getAdminTenant — contract', () => {
  it('returns AdminTenantDetail', async () => {
    const payload = { id: 't-1', name: 'SalesOS', slug: 'salesos', domain: null, plan: 'enterprise', is_active: true, settings: {}, features: {}, user_count: 10, subscription_ends_at: null, created_at: '2026-01-01', updated_at: '2026-07-16' }
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { getAdminTenant } = require('../api')
...
describe('createAdminTenant — contract', () => {
  it('returns created AdminTenantDetail', async () => {
    const payload = { id: 't-new', name: 'NewCo', slug: 'newco', domain: null, plan: 'starter', is_active: true, settings: {}, features: {}, user_count: 0, subscription_ends_at: null, created_at: '2026-07-16', updated_at: '2026-07-16' }
    mockAxios.post.mockResolvedValueOnce(mockResponse(payload))

    const { createAdminTenant } = require('../api')
    const result = await createAdminTenant({ name: 'NewCo', slug: 'newco' })

    expect(result.name).toBe('NewCo')
    expect(mockAxios.post).toHaveBeenCalledWith('/api/v1/admin/tenants', { name: 'NewCo', slug: 'newco' })
  })
})

describe('updateAdminTenant — contract', () => {
  it('PUTs and returns updated detail', async () => {
    const payload = { id: 't-1', name: 'UpdatedCo', slug: 'salesos', domain: null, plan: 'enterprise', is_active: true, settings: {}, features: {}, user_count: 10, subscription_ends_at: null, created_at: '2026-01-01', updated_at: '2026-07-16' }
    mockAxios.put.mockResolvedValueOnce(mockResponse(payload))

    const { updateAdminTenant } = require('../api')
...
describe('deleteAdminTenant — contract', () => {
  it('sends DELETE and returns void', async () => {
    ...
    const { deleteAdminTenant } = require('../api')
...
describe('getAdminTenantUsage — contract', () => {
  it('returns AdminTenantUsage', async () => {
    const payload = { tenant_id: 't-1', tenant_name: 'SalesOS', api_calls: 5000, storage_mb: 250, active_users: 8, total_users: 10, period_start: '2026-07-01', period_end: '2026-07-31' }
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { getAdminTenantUsage } = require('../api')
...
describe('listAdminLicenses — contract', () => {
  it('returns AdminLicense[]', async () => {
    const payload = [{ id: 'l-1', tenant_id: 't-1', tenant_name: 'SalesOS', plan_id: 'p-1', plan_name: 'Enterprise', tier: 'enterprise', is_active: true, starts_at: null, ends_at: null, created_at: '2026-01-01', updated_at: '2026-07-16' }]
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { listAdminLicenses } = require('../api')
...
describe('createAdminLicense — contract', () => {
  it('returns created AdminLicense', async () => {
    const payload = { id: 'l-new', tenant_id: 't-1', tenant_name: 'NewCo', plan_id: 'p-1', plan_name: 'Starter', tier: 'starter', is_active: true, starts_at: '2026-07-16', ends_at: null, created_at: '2026-07-16', updated_at: '2026-07-16' }
    mockAxios.post.mockResolvedValueOnce(mockResponse(payload))

    const { createAdminLicense } = require('../api')
    const result = await createAdminLicense({ tenant_id: 't-1', plan_id: 'p-1' })

    expect(result).toHaveProperty('id')
    expect(mockAxios.post).toHaveBeenCalledWith('/api/v1/admin/licenses', { tenant_id: 't-1', plan_id: 'p-1' })
  })
})

describe('getAdminUser — contract', () => {
  it('returns AdminUserDetail', async () => {
    const payload = { id: 'u-1', email: 'admin@test.com', full_name: 'Admin', full_name_ar: null, role: 'admin', is_active: true, is_verified: true, tenant_id: TENANT, tenant_name: 'SalesOS', created_at: '2026-01-01', last_login_at: null, permissions: ['read', 'write'], updated_at: '2026-07-16' }
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { getAdminUser } = require('../api')
...
describe('listAdminInvoices — contract', () => {
  it('returns AdminInvoice[]', async () => {
    const payload = [{ id: 'inv-1', tenant_id: 't-1', tenant_name: 'SalesOS', amount: 999, currency: 'SAR', status: 'paid', description: 'Monthly subscription', due_date: null, paid_at: '2026-07-01', created_at: '2026-07-01' }]
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { listAdminInvoices } = require('../api')
...
describe('listAdminTransactions — contract', () => {
  it('returns AdminTransaction[]', async () => {
    const payload = [{ id: 'tr-1', tenant_id: 't-1', tenant_name: 'SalesOS', amount: 999, currency: 'SAR', status: 'completed', method: 'card', description: 'Payment', reference: null, created_at: '2026-07-01' }]
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { listAdminTransactions } = require('../api')
...
describe('createAdminFeatureFlag — contract', () => {
  it('returns created AdminFeatureFlag', async () => {
    const payload = { id: 'ff-1', key: 'new-dashboard', name: 'New Dashboard', description: null, enabled: false, is_global: false, created_at: '2026-07-16', updated_at: '2026-07-16' }
    mockAxios.post.mockResolvedValueOnce(mockResponse(payload))

    const { createAdminFeatureFlag } = require('../api')
    const result = await createAdminFeatureFlag({ key: 'new-dashboard', name: 'New Dashboard' })
...
describe('updateAdminFeatureFlag — contract', () => {
  it('returns updated AdminFeatureFlag', async () => {
    const payload = { id: 'ff-1', key: 'new-dashboard', name: 'New Dashboard', description: null, enabled: true, is_global: false, created_at: '2026-07-16', updated_at: '2026-07-16' }
    mockAxios.put.mockResolvedValueOnce(mockResponse(payload))

    const { updateAdminFeatureFlag } = require('../api')
...
describe('getAdminFlagTenants — contract', () => {
  it('returns AdminFlagTenant[]', async () => {
    const payload = [{ flag_id: 'ff-1', flag_key: 'new-dashboard', tenant_id: 't-1', tenant_name: 'SalesOS', enabled: true }]
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { getAdminFlagTenants } = require('../api')
...
describe('toggleAdminFlagForTenant — contract', () => {
  it('PUTs tenant flag toggle', async () => {
    ...
    const { toggleAdminFlagForTenant } = require('../api')
    await toggleAdminFlagForTenant('ff-1', 't-1', true)

    expect(mockAxios.put).toHaveBeenCalledWith('/api/v1/admin/feature-flags/ff-1/tenants/t-1', { enabled: true })
  })
})

describe('listAdminJobs — contract', () => {
  it('returns AdminJob[]', async () => {
    const payload = [{ id: 'job-1', type: 'enrichment', status: 'completed', progress: 100, tenant_id: null, created_by: null, payload: {}, result: {}, error_message: null, retry_count: 0, max_retries: 3, scheduled_at: null, started_at: null, completed_at: '2026-07-16', created_at: '2026-07-16', updated_at: '2026-07-16' }]
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { listAdminJobs } = require('../api')
...
describe('getAdminJob — contract', () => {
  it('returns AdminJobDetail with logs', async () => {
    const payload = { id: 'job-1', type: 'enrichment', status: 'completed', progress: 100, tenant_id: null, created_by: null, payload: {}, result: {}, error_message: null, retry_count: 0, max_retries: 3, scheduled_at: null, started_at: null, completed_at: '2026-07-16', created_at: '2026-07-16', updated_at: '2026-07-16', logs: [{ level: 'info', message: 'started', timestamp: '2026-07-16T00:00:00Z' }] }
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { getAdminJob } = require('../api')
...
describe('retryAdminJob — contract', () => {
  it('POSTs retry and returns void', async () => {
    ...
    const { retryAdminJob } = require('../api')
...
describe('listAdminAICosts — contract', () => {
  it('returns AdminAICost[]', async () => {
    const payload = [{ id: 'ai-1', model: 'gpt-4', tenant_id: null, tenant_name: null, prompt_tokens: 500, completion_tokens: 200, total_tokens: 700, cost: 0.05, operation: 'search', created_at: '2026-07-16' }]
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { listAdminAICosts } = require('../api')
...
describe('getAdminAIUsage — contract', () => {
  it('returns AdminAIUsage', async () => {
    const payload = { total_prompt_tokens: 5000, total_completion_tokens: 2000, total_tokens: 7000, by_model: [], by_tenant: [] }
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { getAdminAIUsage } = require('../api')
...
describe('getAdminHealthHistory — contract', () => {
  it('returns health history entries', async () => {
    const payload = [{ timestamp: '2026-07-16T00:00:00Z', overall_status: 'healthy', components: { database: 'healthy' } }]
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { getAdminHealthHistory } = require('../api')
...
describe('listAdminPermissions — contract', () => {
  it('returns AdminPermission[]', async () => {
    const payload = [{ id: 'perm-1', key: 'companies.read', name: 'Read Companies', description: null, group: 'companies' }]
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { listAdminPermissions } = require('../api')
...
describe('listAdminRoles — contract', () => {
  it('returns AdminRole[]', async () => {
    const payload = [{ id: 'role-1', name: 'Admin', description: 'Full access', permissions: ['*'], is_system: true, user_count: 3, created_at: '2026-01-01', updated_at: '2026-07-16' }]
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { listAdminRoles } = require('../api')
...
describe('createAdminRole — contract', () => {
  it('returns created AdminRole', async () => {
    const payload = { id: 'role-new', name: 'Manager', description: 'Manager role', permissions: ['companies.read'], is_system: false, user_count: 0, created_at: '2026-07-16', updated_at: '2026-07-16' }
    mockAxios.post.mockResolvedValueOnce(mockResponse(payload))

    const { createAdminRole } = require('../api')
    const result = await createAdminRole({ name: 'Manager', permissions: ['companies.read'] })

    expect(result.name).toBe('Manager')
    expect(mockAxios.post).toHaveBeenCalledWith('/api/v1/admin/roles', { name: 'Manager', permissions: ['companies.read'] })
  })
})

describe('updateAdminRole — contract', () => {
  it('returns updated AdminRole', async () => {
    const payload = { id: 'role-1', name: 'Super Admin', description: null, permissions: ['*'], is_system: true, user_count: 3, created_at: '2026-01-01', updated_at: '2026-07-16' }
    mockAxios.put.mockResolvedValueOnce(mockResponse(payload))

    const { updateAdminRole } = require('../api')
...
describe('deleteAdminRole — contract', () => {
  it('sends DELETE and returns void', async () => {
    ...
    const { deleteAdminRole } = require('../api')
...
describe('getAdminConfig — contract', () => {
  it('returns AdminConfigResponse', async () => {
    const payload = { content: '# Config', version: 1, versions: [{ version: 1, content: '# Config', created_at: '2026-07-16', created_by: 'admin' }] }
    mockAxios.get.mockResolvedValueOnce(mockResponse(payload))

    const { getAdminConfig } = require('../api')
...
describe('saveAdminConfig — contract', () => {
  it('PUTs and returns updated config', async () => {
    ...
    const { saveAdminConfig } = require('../api')
    const result = await saveAdminConfig('# New Config')

    expect(result.version).toBe(2)
    expect(mockAxios.put).toHaveBeenCalledWith('/api/v1/admin/config', { content: '# New Config' })
  })
})
...
describe('validateAdminConfig — contract', () => {
  it('returns validation result', async () => {
    ...
    const { validateAdminConfig } = require('../api')
