...
export async function createApiKey(name: string, tenantId: string): Promise<{ key: string; record: ApiKeyRecord }> {
 const response = await client.post("/api/v1/settings/api-keys", { name }, {
 headers: {"X-Tenant-Id": tenantId },
 })
 return response.data
}
...
export async function updateNotificationPreferences(data: Partial<NotificationPreferences>, tenantId: string): Promise<NotificationPreferences> {
