...
describe("useAdvanceOpportunity", () => {
  ...
  it("advances opportunity stage", async () => {
    mockedApi.advanceOpportunity.mockResolvedValue({
      id: "o-1",
      stage: "developing",
    });
    const { result } = renderHook(() => useAdvanceOpportunity(), {
      wrapper: createWrapper(),
    });
...
describe("useCloseWon", () => {
  ...
  it("closes won with amount", async () => {
    ...
    const { result } = renderHook(() => useCloseWon(), {
      wrapper: createWrapper(),
    });
...
describe("useCloseLost", () => {
  ...
  it("closes lost with reason", async () => {
    ...
    const { result } = renderHook(() => useCloseLost(), {
      wrapper: createWrapper(),
    });
