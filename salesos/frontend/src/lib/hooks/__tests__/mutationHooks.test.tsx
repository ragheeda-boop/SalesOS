...
describe("useLogin", () => {
  ...
  it("stores tokens on login", async () => {
    mockedApi.post.mockResolvedValue({
      data: { access_token: "at-1", refresh_token: "rt-1", tenant_id: "t-1" },
    });
...
describe("useRegister", () => {
  ...
  it("stores tokens on register", async () => {
    mockedApi.post.mockResolvedValue({
      data: { access_token: "at-2", refresh_token: "rt-2", tenant_id: "t-2" },
    });
...
describe("useDeleteCompany", () => {
  ...
  it("deletes a company", async () => {
    ...
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
    expect(mockedApi.delete).toHaveBeenCalledWith("/api/v1/companies/c-1", {
      headers: { "X-Tenant-Id": "tenant-1" },
    });
  });
});
