...
import {
 ...
 getAdminConfig, saveAdminConfig, validateAdminConfig,
} from"@/lib/api"
...
export function useUpdateAdminFeatureFlag() {
 const qc = useQueryClient()
 return useMutation({
 mutationFn: ({ id, ...data }: { id: string } & Record<string, unknown>) => updateAdminFeatureFlag(id, data),
 onSuccess: (_data, vars) => {
 qc.invalidateQueries({ queryKey: adminKeys.featureFlags() })
 qc.invalidateQueries({ queryKey: adminKeys.featureFlagTenants(vars.id) })
 },
 })
}
...
export function useSaveAdminConfig() {
 const qc = useQueryClient()
 return useMutation({
 mutationFn: (content: string) => saveAdminConfig(content),
 onSuccess: () => { qc.invalidateQueries({ queryKey: adminKeys.config() }) },
 })
}

export function useValidateAdminConfig() {
 return useMutation({
 mutationFn: (content: string) => validateAdminConfig(content),
 })
}
