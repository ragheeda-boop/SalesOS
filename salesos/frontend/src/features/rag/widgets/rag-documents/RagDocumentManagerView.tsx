...
import type { DocumentSourceType } from "@/lib/ragQueries";
...
interface RagDocumentManagerViewProps {
  documents: RagDoc[] | undefined;
  isLoading: boolean;
  error: boolean;
  showIngest: boolean;
  ...
  setSourceType: (v: DocumentSourceType) => void;
...
export function RagDocumentManagerView({
  documents,
  isLoading,
  error,
  showIngest,
    ...
    <div className="flex h-full flex-col rounded-xl border border-[var(--border-default)] bg-[var(--bg-primary)] overflow-hidden">
      <div className="flex items-center justify-between border-b border-[var(--border-default)] px-4 py-3">
        ...
        <button
          ...
          className="rounded-lg bg-[var(--muhide-orange)] px-2 py-1 text-xs text-white hover:opacity-90"
        >
          + إضافة
        </button>
      </div>
      ...
      <div className="flex-1 overflow-auto p-4 space-y-2">
        ...
        {!isLoading && !error && (!documents || documents.length === 0) && (
          <div className="flex h-full items-center justify-center">
            <div className="text-center">
              ...
              <button
                onClick={() => setShowIngest(true)}
                className="mt-2 text-xs text-[var(--muhide-orange)] hover:underline"
              >
                إضافة مستند جديد
              </button>
            </div>
        ...
        {documents?.map((doc) => (
          <div
            key={doc.id}
            className="rounded-lg border border-[var(--border-default)] p-3 space-y-1"
          >
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 min-w-0">
                ...
                <span
                  className={`rounded px-1.5 py-0.5 text-[10px] font-medium ${SOURCE_COLORS[doc.source_type] || SOURCE_COLORS.note}`}
                >
              ...
              {deleteConfirm === doc.id ? (
                ...
                <button
                  ...
                  className="text-xs text-[var(--text-muted)] hover:text-danger-600 shrink-0"
                >
                  حذف
                </button>
              )}
