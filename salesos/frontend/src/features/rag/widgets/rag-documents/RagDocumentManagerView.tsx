...
export function RagDocumentManagerView({
  documents, isLoading, error, showIngest, setShowIngest,
  title, setTitle, content, setContent, sourceType, setSourceType,
  deleteConfirm, setDeleteConfirm, onIngest, onDelete, isIngesting, isDeleting,
            ...
            <div className="flex items-center justify-between">
              ...
              {deleteConfirm === doc.id ? (
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => onDelete(doc.id)} disabled={isDeleting} className="text-xs text-danger-600 hover:underline">
                    {isDeleting ? "..." : "تأكيد"}
                  </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="w-full max-w-md rounded-xl bg-[var(--bg-primary)] p-6 shadow-xl space-y-4 m-4">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button onClick={onIngest} disabled={!title.trim() || !content.trim() || isIngesting} className="rounded-lg bg-[var(--muhide-orange)] px-3 py-1.5 text-sm text-white hover:opacity-90 disabled:opacity-50">
