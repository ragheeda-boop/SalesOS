...
export function RagChatView({ messages, input, setInput, expandedCitations, setExpandedCitations, isPending, onSend, onRetry }: RagChatViewProps) {
        ...
        {messages.map((msg) => (
          <div key={msg.id} className={`flex ${msg.role === "user" ? "justify-end" : "justify-start"}`}>
            <div className={`max-w-[80%] rounded-xl px-3 py-2 text-sm ${
              ...
              {msg.error && (
                <button onClick={() => onRetry(msg)} className="mt-1 text-xs text-[var(--muhide-orange)] hover:underline">
                  إعادة المحاولة
                </button>
              )}
            </div>
