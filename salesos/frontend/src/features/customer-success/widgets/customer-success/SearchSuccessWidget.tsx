...
export function SearchSuccessWidget({ total_searches, searches_with_action, success_rate }: SearchSuccessWidgetProps) {
 ...
 {t("cs.actions_from_searches", { actions: searches_with_action, total: total_searches })}
 </span>
 </div>
 ...
 <p className="mt-1 text-xs text-[var(--text-muted)]">{t("cs.success_rate", { rate: success_rate.toFixed(0) })}</p>
 </div>
 )
}
