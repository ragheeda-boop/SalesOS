        ...
        <div key={item.feature}>
          <div className="flex items-center justify-between text-sm mb-1">
            ...
            <span className="text-xs text-[var(--text-muted)]">
              {item.adoption_pct.toFixed(0)}%
            </span>
          </div>
          <div
            className="h-4 bg-[var(--bg-tertiary)] rounded-full overflow-hidden"
            role="progressbar"
            aria-valuenow={item.adoption_pct}
            aria-valuemin={0}
            aria-valuemax={100}
            aria-label={`${item.label}: ${item.adoption_pct.toFixed(0)}%`}
          >
            <div
              className={cn(
