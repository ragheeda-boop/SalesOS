...
export function CustomerSuccessContainer() {
 ...
 if (error) {
 return <div className="text-xs text-[var(--color-danger-600)] p-3 bg-[var(--color-danger-50)] rounded-lg">{t("cs.loading_error")}</div>
 }

 if (!data) {
 return <div className="text-xs text-[var(--text-muted)] p-3 text-center">{t("cs.no_data")}</div>
 }
