      ...
      {rule.conditions.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {rule.conditions.map((c, i) => (
            <span
              key={i}
              className="text-xs bg-[var(--bg-secondary)] text-[var(--text-muted)] rounded px-2 py-0.5"
            >
        ...
        {isDeleting ? (
          <>
            <button
              ...
              className="px-2 py-1 text-xs rounded-lg text-[var(--status-danger-text)] hover:bg-[var(--status-danger-bg)]"
            >
              {t("common.confirm")}
            </button>
            <button
        ...
        {form.conditions.length > 0 && (
          <div className="flex flex-wrap gap-1.5">
            {form.conditions.map((c, i) => (
              <span
                key={i}
                className="inline-flex items-center gap-1 text-xs bg-[var(--bg-secondary)] text-[var(--text-muted)] rounded px-2 py-0.5"
              >
                {c.field} {c.operator} {c.value}
                <button
                  onClick={() => removeCondition(i)}
                  className="ml-1 text-[var(--status-danger-text)] hover:text-[var(--status-danger-text)]"
                >
                  ×
                </button>
              </span>
