...
function RulesForm({
 ...
 {c.field} {c.operator} {c.value}
            <button onClick={() => removeCondition(i)} className="ml-1 text-[var(--status-danger-text)] hover:text-[var(--status-danger-text)]">×</button>
 </span>
 ))}
 </div>
 )}
 ...
 {a.type}: {String(a.params.target ||"")}
            <button onClick={() => removeAction(i)} className="ml-1 text-[var(--status-danger-text)] hover:text-[var(--status-danger-text)]">×</button>
 </span>
 ))}
 </div>
 )}
