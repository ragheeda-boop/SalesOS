...
const STATUS_CONFIG: Record<string, { color: string; labelKey: string }> = {
 ...
 unhealthy: { color:"bg-danger-500", labelKey:"monitoring.status.unhealthy" },
}
...
export function MonitoringWidget() {
 ...
 {[...Array(6)].map((_, i) => <div key={i} className="h-20 bg-[var(--bg-tertiary)] rounded-xl" />)}
 </div>
 </div>
 )
 }
 ...
 <h2 className="text-sm font-semibold text-[var(--text-primary)]">{t("monitoring.service_health")}</h2>
