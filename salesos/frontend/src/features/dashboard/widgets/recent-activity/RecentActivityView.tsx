...
export function RecentActivityView({ items, total, onItemClick }: RecentActivityViewProps) {
 if (items.length === 0) {
 return <EmptyState icon={<span className="text-2xl">📭</span>} title="لا توجد نشاطات حديثة" description="سيتم عرض النشاطات الجديدة هنا فور ظهورها" />
 }

 const displayed = items.slice(0, 10)

 return (
 <div role="region" aria-label="النشاطات الحديثة" className="space-y-1/20 dark:rounded-lg dark:p-1">
