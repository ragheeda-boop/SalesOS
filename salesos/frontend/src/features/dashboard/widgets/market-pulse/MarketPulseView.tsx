...
const DIRECTION_CONFIG = {
  ...
  stable: { icon: "→", color: "text-[var(--text-muted)]", label: "مستقر" },
} as const;
...
export function MarketPulseView({
  ...
  if (trends.length === 0 && topMovers.length === 0) {
    return (
      <EmptyState
        ...
        title="بيانات السوق غير متاحة حالياً"
        description="يمكنك المحاولة لاحقاً للحصول على أحدث البيانات"
      />
    );
  }
  ...
  return (
    <div role="region" aria-label="نبض السوق" className="space-y-4">
      {trends.length > 0 && (
        <div>
          <h3 className="mb-1 px-1 text-xs font-semibold text-[var(--text-muted)]">
            اتجاهات السوق
          </h3>
