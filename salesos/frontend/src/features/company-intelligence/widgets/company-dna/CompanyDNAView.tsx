...
const TREND_C = { up: 'text-[var(--color-success)] dark:text-[var(--color-success)]', stable: 'text-[var(--color-warning)] dark:text-[var(--color-warning)]', down: 'text-[var(--color-danger)] dark:text-[var(--color-danger)]' }

function Gauge({ label, value, max = 100, color }: { label: string; value: number; max?: number; color?: string }) {
 const pct = Math.min(100, (value / max) * 100)
 const bar = color ?? (pct >= 70 ? 'bg-[var(--color-success)]' : pct >= 40 ? 'bg-[var(--color-warning)]' : 'bg-[var(--color-danger)]')
 return (
 <div>
