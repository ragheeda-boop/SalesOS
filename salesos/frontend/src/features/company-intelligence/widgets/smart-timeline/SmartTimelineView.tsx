...
const TYPE_CFG: Record<string, { icon: React.ReactNode; label: string; color: string }> = {
 signal: { icon: <Activity className="h-3 w-3" />, label: 'إشارة', color: 'text-[var(--chart-purple)] bg-[var(--chart-purple-bg)] dark:bg-[var(--bg-primary)]/20 dark:text-[var(--text-muted)]' },
 news: { icon: <Newspaper className="h-3 w-3" />, label: 'خبر', color: 'text-blue-600 bg-blue-50 dark:bg-blue-900/20 dark:text-blue-300' },
 government: { icon: <Shield className="h-3 w-3" />, label: 'حكومي', color: 'text-[var(--status-success-text)] bg-[var(--status-success-bg)]' },
 email: { icon: <Mail className="h-3 w-3" />, label: 'بريد', color: 'text-[var(--status-warning-text)] bg-[var(--status-warning-bg)]' },
 ...
 funding: { icon: <DollarSign className="h-3 w-3" />, label: 'تمويل', color: 'text-[var(--status-success-text)] bg-[var(--status-success-bg)]' },
 ai: { icon: <Sparkles className="h-3 w-3" />, label: 'AI', color: 'text-[var(--chart-purple)] bg-[var(--chart-purple-bg)] dark:bg-[var(--bg-primary)]/20 dark:text-[var(--text-muted)]' },
}
...
export function SmartTimelineView({ events }: SmartTimelineViewProps) {
 ...
 return (
 <div role="region" aria-label="الجدول الزمني الذكي" className="space-y-1/20 dark:rounded-lg dark:p-1">
 {sorted.slice(0, 20).map((evt) => {
 const cfg = TYPE_CFG[evt.type] ?? TYPE_CFG.signal
 return (
 <div key={evt.id} className={cn('flex items-start gap-3 rounded-lg px-3 py-2 transition hover:bg-[var(--bg-tertiary)]', evt.aiHighlighted && 'bg-[var(--chart-purple-bg)]/30 dark:bg-[var(--bg-primary)]/5')}>
