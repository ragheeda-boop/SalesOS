...
function StrengthBar({ value }: { value: number }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-[10px] font-medium text-neutral-500 w-8 text-left">{Math.round(value * 100)}%</span>
    </div>
  )
}
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-neutral-900 dark:text-neutral-100">شبكة العلاقات</span>
          </div>
        </CardHeader>
        <CardContent>
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <span className="text-xs text-neutral-500">{relatedEntities.length} كيان</span>
        </div>
      </CardHeader>
      <CardContent>
              ...
              <div className="space-y-2">
                {entities.map((entity) => (
                  <Link
                    ...
                    className={cn(
                      "group flex items-center gap-3 rounded-lg border p-3 transition-colors hover:bg-neutral-50 dark:hover:bg-neutral-800/50",
                      RELATION_COLORS[entity.type as RelationshipType] || "border-neutral-200 dark:border-neutral-700"
                    )}
                  >
                    <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white dark:bg-neutral-800">
                    ...
                    <ExternalLink className="h-4 w-4 shrink-0 text-neutral-400 opacity-0 transition-opacity group-hover:opacity-100" />
                  </Link>
                ))}
              </div>
            </div>
