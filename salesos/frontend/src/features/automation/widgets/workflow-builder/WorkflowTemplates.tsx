...
const TEMPLATES: Template[] = [
 ...
 trigger_type:"event",
 steps: [
 { id:"lf-1", type:"send_email" as const, config: { send_email: { to:"{{lead.email}}", subject:"شكراً لتواصلك", body:"نحن سعداء بتواصلك..." } }, order: 0 },
 { id:"lf-2", type:"create_task" as const, config: { create_task: { title:"متابعة العميل {{lead.name}}", priority:"high", assignee:"{{owner}}" } }, order: 1 },
 ],
 },
 {
 id:"deal-review",
 ...
 steps: [
 { id:"dr-1", type:"send_email" as const, config: { send_email: { to:"team@example.com", subject:"مراجعة الصفقات", body:"يرجى مراجعة الصفقات الكبيرة..." } }, order: 0 },
 { id:"dr-2", type:"update_crm" as const, config: { update_crm: { field:"review_status", value:"pending_review" } }, order: 1 },
 ],
 },
 {
 id:"meeting-prep",
 ...
 trigger_type:"event",
 steps: [
 { id:"mp-1", type:"nba_recommend" as const, config: { nba_recommend: { action_type:"meeting_prep", reason:"اجتماع مرتقب" } }, order: 0 },
 { id:"mp-2", type:"send_email" as const, config: { send_email: { to:"{{owner.email}}", subject:"تحضير الاجتماع", body:"إليك ملخص التحضير..." } }, order: 1 },
 ],
 },
 {
 id:"lost-deal",
 ...
 trigger_type:"event",
 steps: [
 { id:"ld-1", type:"create_task" as const, config: { create_task: { title:"تحليل خسارة {{deal.name}}", priority:"medium", assignee:"{{owner}}" } }, order: 0 },
 { id:"ld-2", type:"send_email" as const, config: { send_email: { to:"{{owner.email}}", subject:"تحليل الصفقة الخاسرة", body:"يرجى توثيق أسباب الخسارة..." } }, order: 1 },
 ],
 },
]
...
export function WorkflowTemplates() {
 ...
 trigger_config: {},
 steps: tmpl.steps.map((s) => ({ ...s, config: s.config, condition_expression: undefined })),
 status:"draft",
 })
 }
 ...
 <h2 className="text-lg font-display text-[var(--text-primary)]">{t("automation.templates")}</h2>
