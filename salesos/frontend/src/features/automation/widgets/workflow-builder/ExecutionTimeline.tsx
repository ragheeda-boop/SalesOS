...
import { useWorkflowExecutions, type WorkflowExecution } from '@/lib/workflowQueries'
import type { StepType } from '@/lib/workflowQueries'
...
export function ExecutionTimeline({ workflowId }: ExecutionTimelineProps) {
 ...
 return <div className="text-xs text-[var(--text-muted)] p-4 text-center">لا يوجد سجل تنفيذ بعد</div>
 }
 ...
 {executions.map((exec) => {
 const status = STATUS_LABELS[exec.status] || { label: exec.status, color: '#9CA3AF' }
 ...
 <span className="text-[10px] px-1.5 py-0.5 rounded" style={{ backgroundColor: status.color + '20', color: status.color }}>
 {status.label}
 </span>
 {exec.completed_at != null && (
 <span className="text-[10px] text-[var(--text-muted)]">
 {Math.round((new Date(exec.completed_at).getTime() - new Date(exec.started_at).getTime()) / 1000)}ث
 </span>
 )}
 </div>
 </button>
 ...
 const s = STATUS_LABELS[sr.status] || { label: sr.status, color: '#9CA3AF' }
 return (
 <div key={i} className="flex items-center gap-2 text-[10px]">
 <span className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: s.color }} />
 <span className="text-[var(--text-secondary)] w-6 tabular-nums">{i + 1}</span>
 <span className="text-[var(--text-primary)]">{sr.step_type || '—'}</span>
 <span className="text-[var(--text-muted)] ml-auto">{s.label}</span>
 {sr.duration_ms != null && (
 <span className="text-[var(--text-muted)] tabular-nums">{sr.duration_ms}ms</span>
 )}
 </div>
 )
 })}
