...
export function SearchResultCard({ result, onClick, highlighted }: SearchResultCardProps) {
 ...
 >
 <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[var(--bg-tertiary)] text-[var(--text-muted)]">
 {icon}
 </div>
 ...
 <span className={cn('rounded px-1 py-0.5 font-medium', result.score > 0.8 ? 'text-[var(--status-success-text)]' : result.score > 0.5 ? 'text-[var(--status-warning-text)]' : 'text-[var(--status-danger-text)]')}>
