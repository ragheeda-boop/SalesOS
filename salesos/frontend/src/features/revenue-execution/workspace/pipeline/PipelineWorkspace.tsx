...
import { MetricCard } from "@salesos/charts";
import {
  DollarSign,
  TrendingUp,
  Clock,
      ...
      <div className="mt-1.5 flex items-center gap-1">
        <span
          className={cn(
            ...
            STAGE_CONFIG[opportunity.stage as StageKey]?.color ??
              "bg-neutral-300",
          )}
        />
      </div>
  ...
  return (
    <div
      ...
      className={cn(
        ...
        config.bg,
        dragOver &&
          "ring-2 ring-[var(--muhide-orange)]/30 bg-[var(--muhide-orange)]/5",
      )}
    >
      ...
      <div className="flex flex-col gap-2 p-2 overflow-y-auto max-h-[calc(100vh-280px)]">
        {opportunities.map((opp) => (
          <DealCard
            ...
            opportunity={opp}
            healthScore={
              healthMap.find((h) => h.opportunity_id === opp.id)?.health_score
            }
          />
        ))}
...
export function PipelineWorkspace() {
  ...
  const [_analytics, setAnalytics] = useState<PipelineAnalytics | null>(null);
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [oppsRes, healthRes, forecastRes, analyticsRes] =
        ...
        if (oppsRes.status === "fulfilled")
          setOpportunities(safeArray(oppsRes.value.data));
        if (healthRes.status === "fulfilled")
          setHealthMap(safeArray(healthRes.value.data));
      ...
      <div className="space-y-6 animate-pulse">
        ...
        <div className="flex gap-4">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div
              key={i}
              className="h-96 w-[280px] shrink-0 rounded-xl bg-[var(--bg-tertiary)]"
            />
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <MetricCard
          ...
          value={formatCurrency(
            openOpps.length > 0 ? totalValue / openOpps.length : 0,
          )}
        />
        <MetricCard
          label="Weighted"
      ...
      {view === "kanban" ? (
        ...
        <div className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-primary)] overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              {opportunities.map((opp) => {
                const score = healthMap.find(
                  (h) => h.opportunity_id === opp.id,
                )?.health_score;
                const stageConfig =
                  STAGE_CONFIG[(opp.stage || "lead") as StageKey];
                return (
                  <tr
                    key={opp.id}
                  ...
                  >
                    <td className="px-4 py-3">
                      <Link
                        ...
                        className="text-[var(--text-primary)] hover:text-[var(--muhide-orange)] font-medium"
                      >
                        {opp.name}
                      </Link>
                    </td>
                    <td className="px-4 py-3 text-[var(--text-secondary)]">
                      <Link
                        ...
                        className="hover:text-[var(--muhide-orange)]"
                      >
                        {opp.company_name ?? "—"}
                      </Link>
                    </td>
                      ...
                      <span className="flex items-center gap-1.5">
                        <span
                          className={cn(
                            ...
                            stageConfig?.dot ?? "bg-neutral-300",
                          )}
                        />
