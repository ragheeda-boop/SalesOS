  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-1.5">
        <div className="rounded-lg bg-[var(--bg-tertiary)] p-2">
          <p className="flex items-center gap-1 text-[9px] text-[var(--text-muted)]">
            <Users className="h-3 w-3" /> الحضور
          </p>
        ...
        <div className="rounded-lg bg-[var(--bg-tertiary)] p-2">
          <p className="flex items-center gap-1 text-[9px] text-[var(--text-muted)]">
            <Activity className="h-3 w-3" /> الإشارات
          </p>
          {brief.recentSignals.map((s, i) => (
            <p
              key={i}
              className="text-[10px] text-[var(--chart-purple)] dark:text-[var(--chart-purple)]"
            >
              • {s}
            </p>
          ))}
      ...
      {brief.risks.length > 0 && (
        <div className="rounded-lg bg-[var(--status-danger-bg)]/50 p-2">
          <p className="flex items-center gap-1 text-[9px] font-medium text-[var(--status-danger-text)]">
            <AlertTriangle className="h-3 w-3" /> المخاطر
          </p>
          {brief.risks.map((r, i) => (
            <p key={i} className="text-[10px] text-[var(--status-danger-text)]">
              • {r}
            </p>
          ))}
        </div>
      ...
      <div className="space-y-1">
        <p className="flex items-center gap-1 text-[9px] font-medium text-[var(--text-muted)]">
          <MessageSquare className="h-3 w-3" /> نقاط النقاش
        </p>
        {brief.talkingPoints.map((tp, i) => (
          <div key={i} className="rounded-lg bg-[var(--bg-tertiary)] px-2 py-1">
      ...
      <div className="flex items-center gap-2 rounded-lg bg-primary-50 p-2 dark:bg-primary-900/10">
        ...
        <p className="text-[10px] font-medium text-primary-700 dark:text-primary-300">
