...
export function MeetingView({ brief }: MeetingViewProps) {
 ...
 return (
 <div role="region" aria-label="ذكاء الاجتماعات" className="space-y-2/20 dark:rounded-lg dark:p-1">
 ...
 {brief.recentSignals.map((s, i) => (
          <p key={i} className="text-[10px] text-[var(--chart-purple)] dark:text-[var(--chart-purple)]">• {s}</p>
 ))}
 </div>
 </div>
 ...
 <p className="flex items-center gap-1 text-[9px] font-medium text-[var(--status-danger-text)]"><AlertTriangle className="h-3 w-3" /> المخاطر</p>
 {brief.risks.map((r, i) => <p key={i} className="text-[10px] text-[var(--status-danger-text)]">• {r}</p>)}
 </div>
 )}
