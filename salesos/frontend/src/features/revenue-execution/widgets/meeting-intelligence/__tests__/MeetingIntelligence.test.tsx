...
const sample: MeetingBrief = {
  ...
  meetingTitle: "اجتماع المتابعة",
  date: "2026-07-15",
  attendees: [{ name: "د. أحمد", role: "CEO", influence: "عالي" }],
  recentSignals: ["توظيف كبير"],
  risks: ["مخاطر"],
  opportunities: ["فرصة"],
  talkingPoints: ["نقطة نقاش 1"],
  recommendedAction: "تقديم عرض",
};
function renderView(b: MeetingBrief | null = sample) {
  return render(<MeetingView brief={b} />);
}
...
describe("MeetingView", () => {
  ...
  it("renders company name", () => {
    renderView();
    expect(screen.getByText(/شركة الطاقة/)).toBeInTheDocument();
  });
  it("renders attendees", () => {
    renderView();
    expect(screen.getByText(/د. أحمد/)).toBeInTheDocument();
  });
  it("renders signals", () => {
    renderView();
    expect(screen.getByText(/توظيف كبير/)).toBeInTheDocument();
  });
  ...
  it("renders talking points", () => {
    renderView();
    expect(screen.getByText("نقطة نقاش 1")).toBeInTheDocument();
  });
  ...
  it("shows empty state", () => {
    renderView(null);
    expect(screen.getByText("لا يوجد إيجاز اجتماع")).toBeInTheDocument();
  });
  it('has role="region"', () => {
    renderView();
    expect(
...
describe("MeetingIntelligenceWidget", () => {
  it("is a valid widget", () => {
    expect(MeetingIntelligenceWidget).toBeDefined();
  });
});
