...
function renderView(d: ChurnData = sample) {
  return render(<ChurnView data={d} />);
}

describeWidgetContract({
  name: "ChurnIntelligence",
  defaultData: sample,
  config: {
    metadata: {
      id: "churnIntelligence",
      title: "مخاطر التوقف",
      permissions: ["churn:read"],
...
describe("ChurnView", () => {
  it("renders company name", () => {
    renderView();
    expect(screen.getByText("شركة البترول")).toBeInTheDocument();
  });
  it("renders at-risk count", () => {
    renderView();
    const c = screen.getAllByText("1");
    expect(c.length).toBeGreaterThanOrEqual(1);
  });
  it("renders total revenue", () => {
    renderView();
    const v = screen.getAllByText(/\$1\.2M/);
    expect(v.length).toBeGreaterThanOrEqual(1);
  });
  it("renders reason", () => {
    renderView();
    expect(screen.getByText("لا نشاط")).toBeInTheDocument();
  });
  it("renders days inactive", () => {
    renderView();
    expect(screen.getByText(/45 يوم/)).toBeInTheDocument();
  });
  it('has role="region"', () => {
    renderView();
...
describe("ChurnIntelligenceWidget", () => {
  it("is a valid widget", () => {
    expect(ChurnIntelligenceWidget).toBeDefined();
  });
});
