...
const STAGE_STYLE: Record<string, string> = {
 ...
 developing: 'bg-[var(--chart-purple-bg)] text-[var(--text-secondary)] dark:bg-[var(--bg-primary)]/20 dark:text-[var(--text-muted)]',
...
const RISK_S = { low: 'text-[var(--status-success-text)]', medium: 'text-[var(--status-warning-text)]', high: 'text-[var(--status-danger-text)]' }
...
export function OpportunityDetailView({ opportunity, onStageChange }: OpportunityDetailViewProps) {
 ...
 return (
 <div role="region" aria-label={`تفاصيل الفرصة: ${opp.title}`} className="space-y-3/20 dark:rounded-lg dark:p-1">
 {/* Header */}
 <div>
 ...
 <p className="text-lg font-bold text-[var(--status-success-text)]">%{Math.round(opp.winProbability * 100)}</p>
 </div>
 </div>
 ...
 isPast ? 'bg-[var(--status-success-bg)] text-[var(--status-success-text)]' :
