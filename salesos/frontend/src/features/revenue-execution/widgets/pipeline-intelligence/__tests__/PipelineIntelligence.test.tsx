...
function renderView(p = sample) {
  return render(<PipelineView pipeline={p} />);
}

describeWidgetContract({
  name: "PipelineIntelligence",
  defaultData: sample,
  config: {
    metadata: {
...
describe("PipelineView", () => {
  ...
  it("shows stalled days with reason", () => {
    renderView();
    expect(screen.getByText(/21 يوم/)).toBeInTheDocument();
...
describe("PipelineIntelligenceWidget", () => {
  it("is a valid widget component", () => {
    expect(PipelineIntelligenceWidget).toBeDefined();
  });
});
