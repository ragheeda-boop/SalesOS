...
export function PipelineView({ pipeline }: PipelineViewProps) {
 ...
 <p className="text-sm font-bold text-[var(--status-success-text)]">%{Math.round(pipeline.winRate * 100)}</p>
 </div>
 </div>
 ...
 <span className="text-[10px] font-medium text-[var(--status-danger-text)]">صفقات متوقفة ({pipeline.stalledDeals.length})</span>
 </div>
 {pipeline.stalledDeals.slice(0, 3).map((deal) => (
 <div key={deal.id} className="flex items-center justify-between rounded-lg bg-[var(--status-danger-bg)]/50 px-2 py-1.5">
 <div>
