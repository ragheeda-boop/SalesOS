      ...
      <div className="grid grid-cols-4 gap-2">
        ...
        <div className="rounded-lg bg-[var(--bg-tertiary)] p-2">
          ...
          <p className="text-sm font-bold text-[var(--text-primary)]">
            $
            {pipeline.totalValue >= 1e6
              ? (pipeline.totalValue / 1e6).toFixed(1) + "M"
              : (pipeline.totalValue / 1e3).toFixed(0) + "K"}
          </p>
        </div>
        <div className="rounded-lg bg-[var(--bg-tertiary)] p-2">
          ...
          <p className="text-sm font-bold text-[var(--text-primary)]">
            $
            {pipeline.weightedValue >= 1e6
              ? (pipeline.weightedValue / 1e6).toFixed(1) + "M"
              : (pipeline.weightedValue / 1e3).toFixed(0) + "K"}
          </p>
        </div>
        <div className="rounded-lg bg-[var(--bg-tertiary)] p-2">
        ...
        {pipeline.stages.map((stage) => {
          ...
          return (
            <div key={stage.id} className="flex items-center gap-2">
              ...
              <span className="w-16 text-right text-[10px] font-medium text-[var(--text-primary)]">
                {stage.deals}
              </span>
              <span className="w-16 text-right text-[10px] text-[var(--text-muted)]">
                $
                {stage.value >= 1e6
                  ? (stage.value / 1e6).toFixed(1) + "M"
                  : (stage.value / 1e3).toFixed(0) + "K"}
              </span>
            </div>
          );
        })}
      ...
      {pipeline.stalledDeals.length > 0 && (
        <div>
          <div className="flex items-center gap-1.5 mb-1">
            <AlertTriangle className="h-3 w-3 text-[var(--status-danger-text)]" />
            <span className="text-[10px] font-medium text-[var(--status-danger-text)]">
              صفقات متوقفة ({pipeline.stalledDeals.length})
            </span>
          </div>
          {pipeline.stalledDeals.slice(0, 3).map((deal) => (
            <div
              key={deal.id}
              className="flex items-center justify-between rounded-lg bg-[var(--status-danger-bg)]/50 px-2 py-1.5"
            >
              <div>
                ...
                <p className="text-[9px] text-[var(--text-muted)]">
                  متوقفة منذ {deal.daysStalled} يوم
