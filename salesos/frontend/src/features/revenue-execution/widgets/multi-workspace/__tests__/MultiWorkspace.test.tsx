...
function renderView(d: WorkspaceData = sample) {
  return render(<MultiWorkspaceView data={d} />);
}
...
describe("MultiWorkspaceView", () => {
  ...
  it("shows active count", () => {
    renderView();
    expect(screen.getByText(/1 نشطة/)).toBeInTheDocument();
  });
  it('has role="region"', () => {
    renderView();
    expect(
...
describe("MultiWorkspaceWidget", () => {
  it("is a valid widget", () => {
    expect(MultiWorkspaceWidget).toBeDefined();
  });
});
