...
export function SecurityView({ data }: { data: SecurityData }) {
 ...
 return (
 <div role="region" aria-label="الأمان" className="space-y-2/20 dark:rounded-lg dark:p-1">
 <div className="flex gap-1.5">
 {features.map((f) => (
 <div key={f.label} className={cn('flex items-center gap-1 rounded-lg px-2 py-1 text-[9px] font-medium', f.enabled ? 'bg-[var(--status-success-bg)] text-[var(--status-success-text)]' : 'bg-[var(--bg-tertiary)] text-[var(--text-disabled)]')}>
 ...
 <div className={cn('rounded-lg p-2', data.pendingActions > 0 ? 'bg-[var(--status-warning-bg)]' : 'bg-[var(--bg-tertiary)]')}>
 <p className="text-[9px] text-[var(--text-muted)]">معلق</p>
 <p className="text-sm font-bold text-[var(--status-warning-text)]">{data.pendingActions}</p>
 </div>
 </div>
