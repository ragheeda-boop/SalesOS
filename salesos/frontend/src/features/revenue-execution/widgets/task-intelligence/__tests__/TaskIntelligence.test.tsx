...
function renderView(t: RevenueTask[] = sample) {
  return render(<TaskView tasks={t} />);
}
...
describe("TaskView", () => {
  ...
  it("renders priority labels", () => {
    renderView();
    const p = screen.getAllByText("حرج");
  ...
  it('has role="region"', () => {
    renderView();
    expect(screen.getByRole("region", { name: "المهام" })).toBeInTheDocument();
  });
});
...
describe("TaskView interactions", () => {
  ...
  it("renders due date when present", () => {
    renderView();
    expect(document.body.textContent).toContain("١٥‏/٧");
  });
});
describe("TaskIntelligenceWidget", () => {
  it("is a valid widget", () => {
    expect(TaskIntelligenceWidget).toBeDefined();
  });
});
