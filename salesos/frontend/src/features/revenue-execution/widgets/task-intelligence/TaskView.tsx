...
const PRIORITY_CFG = { critical: { color: 'text-[var(--status-danger-text)]', label: 'حرج' }, high: { color: 'text-orange-500', label: 'عالي' }, medium: { color: 'text-[var(--status-warning-text)]', label: 'متوسط' }, low: { color: 'text-[var(--text-disabled)]', label: 'منخفض' } }
...
export function TaskView({ tasks, onComplete }: TaskViewProps) {
 ...
 {task.completed ? <CheckCircle2 className="h-4 w-4 text-[var(--status-success-text)]" /> : <Circle className="h-4 w-4 text-[var(--text-muted)] hover:text-primary-500" />}
 </button>
