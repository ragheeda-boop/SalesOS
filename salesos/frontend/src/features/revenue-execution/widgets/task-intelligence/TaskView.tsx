...
const PRIORITY_CFG = {
  critical: { color: "text-[var(--status-danger-text)]", label: "حرج" },
  high: { color: "text-orange-500", label: "عالي" },
  medium: { color: "text-[var(--status-warning-text)]", label: "متوسط" },
  low: { color: "text-[var(--text-disabled)]", label: "منخفض" },
};
const SOURCE_ICON = {
  nba: <Sparkles className="h-3 w-3" />,
  meeting: <Clock className="h-3 w-3" />,
  email: <Target className="h-3 w-3" />,
  manual: <Circle className="h-3 w-3" />,
  signal: <AlertTriangle className="h-3 w-3" />,
};
            ...
            >
              ...
              >
                {task.completed ? (
                  ...
                  <Circle className="h-4 w-4 text-[var(--text-muted)] hover:text-primary-500" />
                )}
              </button>
