...
export function RevenueTimelineView({ events }: RevenueTimelineViewProps) {
  ...
  return (
    <div
      role="region"
      aria-label="الجدول الزمني للإيرادات"
    ...
    >
      {sorted.slice(0, 25).map((evt) => {
        ...
        return (
          <div
            key={evt.id}
            className="flex items-start gap-3 rounded-lg px-3 py-2 transition hover:bg-[var(--bg-tertiary)]"
          >
            <div
              className={cn(
              ...
              <div className="flex items-center gap-2 text-[8px] text-[var(--text-muted)]">
                ...
                {evt.value && (
                  <span>
                    $
                    {evt.value >= 1e6
                      ? (evt.value / 1e6).toFixed(1) + "M"
                      : (evt.value / 1e3).toFixed(0) + "K"}
                  </span>
                )}
              </div>
            </div>
