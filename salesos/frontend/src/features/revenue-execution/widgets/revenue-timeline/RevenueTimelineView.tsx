...
const TYPE_CFG: Record<string, { icon: React.ReactNode; label: string; color: string }> = {
 signal: { icon: <Activity className="h-3 w-3" />, label: 'إشارة', color: 'text-[var(--chart-purple)] bg-[var(--chart-purple-bg)] dark:bg-[var(--bg-primary)]/20' },
 ...
 task: { icon: <CheckCircle2 className="h-3 w-3" />, label: 'مهمة', color: 'text-[var(--status-success-text)] bg-[var(--status-success-bg)]' },
 deal: { icon: <DollarSign className="h-3 w-3" />, label: 'صفقة', color: 'text-[var(--status-warning-text)] bg-[var(--status-warning-bg)]' },
 note: { icon: <FileText className="h-3 w-3" />, label: 'ملاحظة', color: 'text-[var(--text-secondary)] bg-[var(--bg-secondary)]' },
}
...
export function RevenueTimelineView({ events }: RevenueTimelineViewProps) {
 ...
 return (
 <div role="region" aria-label="الجدول الزمني للإيرادات" className="space-y-1/20 dark:rounded-lg dark:p-1">
