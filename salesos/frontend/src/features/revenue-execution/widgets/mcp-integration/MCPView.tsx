...
export function MCPView({ data }: { data: MCPData }) {
 return (
 <div role="region" aria-label="اتصالات MCP" className="space-y-2/20 dark:rounded-lg dark:p-1">
 ...
 <p className="text-sm font-bold text-[var(--status-success-text)]">{data.activeConnections}</p>
 </div>
 <div className="rounded-lg bg-[var(--bg-tertiary)] p-2">
 ...
 {c.status === 'connected' ? <CheckCircle className="h-3 w-3 text-[var(--status-success-text)]" /> : c.status === 'error' ? <XCircle className="h-3 w-3 text-[var(--status-danger-text)]" /> : <Clock className="h-3 w-3 text-[var(--status-warning-text)]" />}
 </div>
