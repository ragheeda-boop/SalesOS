...
function renderView(d: APIData = sample) {
  return render(<APIView data={d} />);
}
...
describeWidgetContract({
  ...
  config: {
    metadata: {
      id: "apiPlatform",
      title: "منصة API",
      permissions: ["enterprise:api:read"],
...
describe("APIView", () => {
  ...
  it("renders total calls", () => {
    renderView();
    const c = screen.getAllByText("100");
    expect(c.length).toBeGreaterThanOrEqual(1);
  });
  it("renders latency", () => {
    renderView();
    const l = screen.getAllByText("45ms");
...
describe("APIPlatformWidget", () => {
  it("is a valid widget", () => {
    expect(APIPlatformWidget).toBeDefined();
  });
});
