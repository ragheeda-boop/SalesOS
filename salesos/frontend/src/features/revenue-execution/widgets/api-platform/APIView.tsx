...
import { Code, Activity, Clock, AlertTriangle } from "lucide-react";
import type { APIData } from "./types";

const METH_C: Record<string, string> = {
  GET: "text-[var(--status-success-text)]",
  POST: "text-blue-600",
  PUT: "text-[var(--status-warning-text)]",
  DELETE: "text-[var(--status-danger-text)]",
};
  ...
  return (
    ...
    >
      <div className="grid grid-cols-4 gap-2">
        ...
        <div className="rounded-lg bg-[var(--bg-tertiary)] p-2">
          <p className="text-[9px] text-[var(--text-muted)]">
            <Activity className="h-3 w-3 inline" /> الاستدعاءات
          </p>
          <p className="text-sm font-bold text-[var(--text-primary)]">
      ...
      {data.endpoints.map((ep, i) => (
        <div
          key={i}
          className="flex items-center gap-2 rounded-lg px-3 py-1.5 transition hover:bg-[var(--bg-tertiary)]"
        >
          <span
            className={cn(
