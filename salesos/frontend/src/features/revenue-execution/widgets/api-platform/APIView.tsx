...
const METH_C: Record<string, string> = { GET: 'text-[var(--status-success-text)]', POST: 'text-blue-600', PUT: 'text-[var(--status-warning-text)]', DELETE: 'text-[var(--status-danger-text)]' }
...
export function APIView({ data }: { data: APIData }) {
 ...
 <div className={cn('rounded-lg p-2', data.errorRate > 5 ? 'bg-[var(--status-danger-bg)]' : 'bg-[var(--bg-tertiary)]')}>
 ...
 <span className={cn('w-12 text-[9px] font-mono font-bold', METH_C[ep.method] ?? 'text-[var(--text-muted)]')}>{ep.method}</span>
