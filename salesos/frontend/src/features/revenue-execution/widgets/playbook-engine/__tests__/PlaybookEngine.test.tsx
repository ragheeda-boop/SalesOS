...
function renderView(p: Playbook | null = sample, ind = "test") {
  return render(<PlaybookView playbook={p} industry={ind} />);
}
...
describe("PlaybookView", () => {
  it("renders playbook name", () => {
    renderView();
    expect(screen.getByText("دليل الاختبار")).toBeInTheDocument();
  });
...
describe("PlaybookWidget", () => {
  it("is a valid widget component", () => {
    expect(PlaybookWidget).toBeDefined();
  });
});
