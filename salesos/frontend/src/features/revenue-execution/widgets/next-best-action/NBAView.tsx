...
const PRIORITY_CFG = {
 critical: { label: 'حرج — تحرك فوري', color: 'bg-[var(--status-danger-bg)] text-red-700 ring-1 ring-red-200 dark:ring-red-800', icon: <Zap className="h-4 w-4" /> },
 ...
 medium: { label: 'متوسط — هذا الشهر', color: 'bg-[var(--status-warning-bg)] text-amber-700 ring-1 ring-amber-200 dark:ring-amber-800', icon: <Target className="h-4 w-4" /> },
 low: { label: 'منخفض — مراقبة', color: 'bg-[var(--bg-secondary)] text-[var(--text-secondary)] ring-1 ring-neutral-200 dark:ring-neutral-700', icon: <Clock className="h-4 w-4" /> },
}
...
export function NBAView({ action, onExecute }: NBAViewProps) {
 ...
 return (
 <div role="region" aria-label="أفضل إجراء تالي" className="space-y-3/20 dark:rounded-lg dark:p-1">
 {/* Priority Badge */}
