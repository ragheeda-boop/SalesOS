...
export function MarketplaceView({ data }: MarketplaceViewProps) {
  ...
  const filtered =
    filter === "all" ? data.plugins : data.plugins.filter((p) => p.installed);
      ...
      <div className="flex items-center justify-between">
        <div className="flex gap-1">
          {(["all", "installed"] as const).map((f) => (
            ...
            >
              {f === "all" ? "الكل" : "مثبت"} (
              {f === "all" ? data.available : data.installed})
            </button>
          ))}
        </div>
        <span className="text-[9px] text-[var(--text-muted)]">
          {data.installed}/{data.available} مثبت
        </span>
      </div>
      {filtered.map((p) => (
        <div
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-1.5">
              ...
              {p.installed && (
                <CheckCircle className="h-3 w-3 shrink-0 text-[var(--status-success-text)]" />
              )}
            </div>
