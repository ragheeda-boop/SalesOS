...
const sample: TerritoryData = {
  ...
  coverage: [],
  gaps: [{ region: "القصيم", potentialValue: 1800000, reason: "لا يوجد" }],
};

function renderView(d: TerritoryData = sample) {
  return render(<TerritoryView data={d} />);
}
...
describe("TerritoryView", () => {
  it("renders territory names", () => {
    renderView();
    expect(screen.getByText("الرياض")).toBeInTheDocument();
  });
  it("renders deal count", () => {
    renderView();
    expect(screen.getByText(/8 صفقات/)).toBeInTheDocument();
  });
...
describe("TerritoryIntelligenceWidget", () => {
  it("is a valid widget", () => {
    expect(TerritoryIntelligenceWidget).toBeDefined();
  });
});
