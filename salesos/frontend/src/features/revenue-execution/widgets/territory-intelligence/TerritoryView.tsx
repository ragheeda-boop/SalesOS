...
export function TerritoryView({ data }: { data: TerritoryData }) {
 ...
 {data.gaps.length > 0 && (
 <div>
 <p className="mb-1 flex items-center gap-1 text-[10px] font-medium text-[var(--status-warning-text)]"><AlertTriangle className="h-3 w-3" /> فجوات التغطية</p>
 {data.gaps.map((g, i) => (
 <div key={i} className="flex items-center justify-between rounded-lg bg-[var(--status-warning-bg)]/50 px-2 py-1">
