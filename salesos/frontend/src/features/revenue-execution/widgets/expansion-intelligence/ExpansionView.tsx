  ...
  return (
    <div role="region" aria-label="فرص التوسع" className="space-y-2/20 dark:rounded-lg dark:p-1">
      <div className="grid grid-cols-2 gap-2">
        <div className="rounded-lg bg-[var(--bg-tertiary)] p-2">
          ...
          <p className="text-sm font-bold text-[var(--text-primary)]">
            $
            {data.totalValue >= 1e6
              ? (data.totalValue / 1e6).toFixed(1) + "M"
              : (data.totalValue / 1e3).toFixed(0) + "K"}
          </p>
        </div>
        <div className="rounded-lg bg-[var(--bg-tertiary)] p-2">
      ...
      {data.opportunities.map((o, i) => (
        <div
          key={i}
          className="flex items-start gap-2 rounded-lg px-2 py-1.5 transition hover:bg-[var(--bg-tertiary)]"
        >
          <TrendingUp className="mt-0.5 h-3.5 w-3.5 shrink-0 text-[var(--status-success-text)]" />
