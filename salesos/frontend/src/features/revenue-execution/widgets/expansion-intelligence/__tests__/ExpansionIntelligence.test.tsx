...
function renderView(d: ExpansionData = sample) {
  return render(<ExpansionView data={d} />);
}

describeWidgetContract({
  name: "ExpansionIntelligence",
  defaultData: sample,
  config: {
    metadata: {
...
describe("ExpansionView", () => {
  ...
  it("renders total value", () => {
    renderView();
    const v = screen.getAllByText(/\$2\.5M/);
    expect(v.length).toBeGreaterThanOrEqual(1);
  });
  it("renders reason", () => {
    renderView();
    expect(screen.getByText("طلب عالي")).toBeInTheDocument();
  });
  it('has role="region"', () => {
    renderView();
...
describe("ExpansionIntelligenceWidget", () => {
  it("is a valid widget", () => {
    expect(ExpansionIntelligenceWidget).toBeDefined();
  });
});
