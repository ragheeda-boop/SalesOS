...
export function ForecastView({ data }: { data: ForecastData }) {
 ...
 <p className="text-sm font-bold text-[var(--status-success-text)]">${data.currentQuarter.actual >= 1e6 ? (data.currentQuarter.actual / 1e6).toFixed(1) + 'M' : (data.currentQuarter.actual / 1e3).toFixed(0) + 'K'}</p>
 </div>
 <div className="rounded-lg bg-[var(--bg-tertiary)] p-2">
 <p className="text-[9px] text-[var(--text-muted)]">المتوقع</p>
 <p className="text-sm font-bold text-[var(--status-warning-text)]">${data.currentQuarter.projected >= 1e6 ? (data.currentQuarter.projected / 1e6).toFixed(1) + 'M' : (data.currentQuarter.projected / 1e3).toFixed(0) + 'K'}</p>
 </div>
 </div>
 <div>
 ...
 {data.risks.length > 0 && (
 <div>
 <p className="mb-1 flex items-center gap-1 text-[10px] font-medium text-[var(--status-danger-text)]"><AlertTriangle className="h-3 w-3" /> المخاطر</p>
 {data.risks.map((r, i) => (
 <div key={i} className="flex items-center justify-between rounded-lg bg-[var(--status-danger-bg)]/50 px-2 py-1">
 <span className="text-[10px] text-[var(--text-primary)]">{r.label}</span>
 <span className="text-[9px] text-[var(--status-danger-text)]">%{Math.round(r.probability * 100)}</span>
 </div>
 ))}
 </div>
 )}
