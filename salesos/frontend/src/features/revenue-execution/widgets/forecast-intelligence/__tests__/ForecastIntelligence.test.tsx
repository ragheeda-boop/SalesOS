...
const sample: ForecastData = {
  ...
  monthlyTrend: [],
  risks: [{ label: "تباطؤ", impact: 500000, probability: 0.35 }],
};

function renderView(d: ForecastData = sample) {
  return render(<ForecastView data={d} />);
}

describeWidgetContract({
  name: "ForecastIntelligence",
  defaultData: sample,
  config: {
    metadata: {
      id: "forecastIntelligence",
      title: "التوقعات",
...
describe("ForecastView", () => {
  ...
  it("renders projected", () => {
    renderView();
    expect(screen.getByText(/\$7\.8M/)).toBeInTheDocument();
  });
  it("renders risks", () => {
    renderView();
    expect(screen.getByText("تباطؤ")).toBeInTheDocument();
  });
  it("renders risk probability", () => {
    renderView();
  ...
  it("renders projected percentage of target", () => {
    renderView();
...
describe("ForecastView edge cases", () => {
  ...
  it("renders $0 for all metrics when all zero", () => {
    const allZero: ForecastData = {
      currentQuarter: { target: 0, actual: 0, projected: 0, confidence: 0 },
      monthlyTrend: [],
      risks: [],
    };
    renderView(allZero);
...
describe("ForecastIntelligenceWidget", () => {
  it("is a valid widget", () => {
    expect(ForecastIntelligenceWidget).toBeDefined();
  });
});
