...
export function AuditLogView({ items, total, loading, filters, onFilterChange, onExport, onRefresh }: AuditLogViewProps) {
 ...
 <tbody>
 {items.map((entry) => (
 <tr key={entry.id} className="border-b hover:bg-[var(--bg-secondary)] dark:hover:bg-[var(--bg-primary)]">
 <td className="p-2 text-xs text-[var(--text-muted)] font-mono" data-label="التاريخ">
 ...
 <td className="p-2 text-xs text-[var(--text-secondary)] max-w-[200px] truncate" data-label="التفاصيل">
