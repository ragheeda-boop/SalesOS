...
export function TenantList() {
 ...
 <Card className="p-4 space-y-3">
  ...
  <Input placeholder={t("admin.tenant_list.name_placeholder")} value={createName} onChange={(e) => setCreateName(e.target.value)} />
  <Input placeholder={t("admin.tenant_list.slug_placeholder")} value={createSlug} onChange={(e) => setCreateSlug(e.target.value)} />
  </div>
  <div className="flex gap-2">
  <Button onClick={handleCreate} disabled={createMutation.isPending}>{t("admin.tenant_list.create_btn")}</Button>
  <Button variant="ghost" onClick={() => setShowCreate(false)}>{t("admin.tenant_list.cancel")}</Button>
 </div>
 </Card>
 )}

 {isLoading ? (
  <div className="py-20 text-center text-[var(--text-muted)]"><Spinner /> {t("admin.tenant_list.loading")}</div>
 ) : !tenants?.length ? (
 ...
 <tr key={tenant.id} className="border-b dark:border-[var(--border-default)] hover:bg-[var(--bg-secondary)] dark:hover:bg-[var(--bg-primary)]">
  <td className="p-2 font-medium" data-label={t("admin.tenant_list.name")}>{tenant.name}</td>
  <td className="p-2 text-xs text-[var(--text-muted)] font-mono" data-label={t("admin.tenant_list.slug")}>{tenant.slug}</td>
  ...
  <td className="p-2" data-label={t("admin.tenant_list.users")}>{tenant.user_count}</td>
  <td className="p-2" data-label={t("admin.tenant_list.status")}>
  {tenant.is_active ? (
  <span className="flex items-center gap-1 text-success-600"><CheckCircle className="h-3.5 w-3.5" /> {t("admin.tenant_list.active")}</span>
  ) : (
  <span className="flex items-center gap-1 text-danger-600"><XCircle className="h-3.5 w-3.5" /> {t("admin.tenant_list.inactive")}</span>
  )}
  </td>
  <td className="p-2 text-xs text-[var(--text-muted)]" data-label={t("admin.tenant_list.created_at")}>
  ...
  onClick={() => updateTenantActive.mutate({ id: tenant.id, is_active: !tenant.is_active })}
  >
  {tenant.is_active ?t("admin.tenant_list.deactivate") :t("admin.tenant_list.activate")}
  </Button>
  </div>
  </td>
 </tr>
