      ...
      <div className="flex items-center justify-between">
        ...
        {health && (
          <div className="flex items-center gap-2">
            <span
              className={`h-3 w-3 rounded-full ${health.overall_status === "healthy" ? "bg-success-500" : "bg-danger-500"}`}
            />
            <span className="font-medium">
              {health.overall_status === "healthy"
                ? t("admin.health.system_ok")
                : t("admin.health.issue_detected")}
            </span>
          </div>
        )}
      </div>
              ...
              <div className="flex-1">
                ...
                {comp.latency_ms != null && (
                  <p className="text-xs text-[var(--text-muted)] mt-1">
                    {t("admin.health.component_latency", {
                      ms: comp.latency_ms,
                    })}
                  </p>
                )}
                    ...
                    <span
                      ...
                      className={`text-xs px-1.5 py-0.5 rounded ${
                        status === "healthy"
                          ...
                          : "bg-danger-50 text-danger-700 dark:bg-danger-900/20"
                      }`}
                    >
                      {name}: {status}
                    </span>
