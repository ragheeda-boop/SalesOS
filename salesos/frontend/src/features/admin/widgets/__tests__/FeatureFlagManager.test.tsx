...
function setupFlags(
  ...
  mockUseToggleAdminFlagForTenant.mockReturnValue({
    mutate: jest.fn(),
    isPending: false,
  });
}
...
describe("FeatureFlagManager", () => {
  ...
  it("calls create mutation on submit", async () => {
    render(<FeatureFlagManager />);
    fireEvent.click(screen.getByText("ميزة جديدة"));
    fireEvent.change(screen.getByPlaceholderText("المفتاح (key)"), {
      target: { value: "test_flag" },
    });
    fireEvent.change(screen.getByPlaceholderText("الاسم"), {
      target: { value: "ميزة اختبار" },
    });
