...
export function RoleManagerView({ roles, permissions, loading, onRefresh, onCreateRole, onUpdateRole, onDeleteRole }: RoleManagerViewProps) {
 ...
 <p className="text-xs font-semibold text-[var(--text-muted)]">{PERMISSION_GROUP_LABELS[group] || group}</p>
 {perms.map((p) => (
 <label
 key={p.id}
 className={`flex items-center gap-2 px-2 py-1 rounded cursor-pointer text-xs transition ${
 selectedPerms.includes(p.key) ?"bg-[var(--muhide-orange)]/10 text-[var(--muhide-orange)]" :"hover:bg-[var(--bg-tertiary)] dark:hover:bg-[var(--bg-secondary)]"
 }`}
 >
 <input
 type="checkbox"
 ...
 className="p-1.5 rounded hover:bg-[var(--bg-tertiary)] dark:hover:bg-[var(--bg-secondary)]"
 ...
 <span key={perm} className="text-[10px] px-1.5 py-0.5 rounded bg-[var(--bg-tertiary)] text-[var(--text-secondary)] font-mono">
 {perm}
 </span>
 ))
 )}
 {role.permissions.length > 8 && (
 <span className="text-[10px] text-[var(--text-disabled)]">+{role.permissions.length - 8}</span>
 )}
 </div>
 </Card>
 ))}
