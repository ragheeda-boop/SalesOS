...
export function JobList() {
 ...
 {isLoading ? (
 <div className="py-20 text-center text-[var(--text-muted)]"><Spinner /></div>
 ) : !jobs?.length ? (
 ...
 selectedJob === job.id ?"border-[var(--muhide-orange)] bg-[var(--muhide-orange)]/5" :"border-[var(--border-default)] hover:bg-[var(--bg-secondary)] dark:hover:bg-[var(--bg-secondary)]"
 }`}
 >
 ...
 {job.status ==="running" && <span className="text-xs text-[var(--text-disabled)]">{job.progress}%</span>}
 </div>
 </button>
 ))}
 </div>
...
function StatusIcon({ status }: { status: string }) {
 ...
 case"running": return <RefreshCw className="h-5 w-5 text-[var(--status-warning-text)] animate-spin" />
 default: return <Clock className="h-5 w-5 text-[var(--text-disabled)]" />
 }
}
...
function JobDetailPanel({ jobId }: { jobId: string }) {
 ...
 <div><span className="text-[var(--text-muted)]">الحالة:</span> <Badge>{job.status}</Badge></div>
 <div><span className="text-[var(--text-muted)]">التقدم:</span> {job.progress}%</div>
 <div><span className="text-[var(--text-muted)]">المحاولات:</span> {job.retry_count}/{job.max_retries}</div>
 {job.tenant_id && <div><span className="text-[var(--text-muted)]">العميل:</span> {job.tenant_id}</div>}
 </div>
 ...
 <div key={i} className={`text-xs p-1.5 rounded ${log.level ==="ERROR" ?"bg-danger-50 dark:bg-danger-900/20 text-danger-600" :"bg-[var(--bg-secondary)]"}`}>
 <span className="font-mono text-[var(--text-disabled)]">{new Date(log.timestamp).toLocaleTimeString("ar-SA")}</span>
