...
export function UserList() {
 ...
 {isLoading ? (
  <div className="py-20 text-center text-[var(--text-muted)]"><Spinner /> {t("admin.user_list.loading")}</div>
 ) : !users?.length ? (
 ...
 <tr key={user.id} className="border-b hover:bg-[var(--bg-secondary)] dark:hover:bg-[var(--bg-primary)]">
  ...
  <td className="p-2 text-xs" data-label={t("admin.user_list.email")}>{user.email}</td>
  ...
  <td className="p-2 text-xs text-[var(--text-muted)]" data-label={t("admin.user_list.tenant")}>{user.tenant_name}</td>
  <td className="p-2" data-label={t("admin.user_list.status")}>
  {user.is_active ? (
  <span className="flex items-center gap-1 text-success-600"><CheckCircle className="h-3.5 w-3.5" /> {t("admin.user_list.active")}</span>
  ) : (
  <span className="flex items-center gap-1 text-danger-600"><XCircle className="h-3.5 w-3.5" /> {t("admin.user_list.inactive")}</span>
  )}
  </td>
  <td className="p-2 text-xs text-[var(--text-muted)]" data-label={t("admin.user_list.last_login")}>
