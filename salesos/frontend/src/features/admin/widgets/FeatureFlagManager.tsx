...
export function FeatureFlagManager() {
 ...
 <input className="border rounded px-3 py-2 text-sm" placeholder="المفتاح (key)" value={createKey} onChange={(e) => setCreateKey(e.target.value)} />
 <input className="border rounded px-3 py-2 text-sm" placeholder="الاسم" value={createName} onChange={(e) => setCreateName(e.target.value)} />
 </div>
 ...
 selectedFlag === flag.id ?"border-[var(--muhide-orange)] bg-[var(--muhide-orange)]/5" :"border-[var(--border-default)] hover:bg-[var(--bg-secondary)] dark:hover:bg-[var(--bg-secondary)]"
 }`}
 >
 <div>
 <p className="font-medium text-sm">{flag.name}</p>
 <p className="text-xs text-[var(--text-muted)] font-mono">{flag.key}</p>
 {flag.description && <p className="text-xs text-[var(--text-disabled)] mt-0.5">{flag.description}</p>}
 </div>
...
function FlagTenantManager({ flagId }: { flagId: string }) {
 ...
 {!tenants?.length ? (
 <p className="text-sm text-[var(--text-muted)] text-center py-4">لا توجد تجاوزات لهذه الميزة</p>
 ) : (
 <div className="space-y-2">
 {tenants.map((t: { tenant_name: string; tenant_id: string; enabled: boolean }) => (
 <div key={t.tenant_id} className="flex items-center justify-between p-2 rounded-lg border">
 <span className="text-sm">{t.tenant_name}</span>
 <button
 onClick={() => toggleMutation.mutate({ tenantId: t.tenant_id, enabled: !t.enabled })}
 className={`px-3 py-1 rounded text-xs font-medium transition ${
 t.enabled ?"bg-success-100 text-success-700 dark:bg-success-900/30" :"bg-[var(--bg-tertiary)] text-[var(--text-muted)]"
 }`}
 >
 {t.enabled ?"مفعل" :"معطل"}
 </button>
