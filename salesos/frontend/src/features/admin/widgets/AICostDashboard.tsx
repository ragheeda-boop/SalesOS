...
export function AICostDashboard() {
 ...
 <div key={o.operation} className="flex items-center justify-between p-2 rounded-lg border">
 <Badge>{o.operation}</Badge>
 <div className="text-left">
 <p className="text-sm font-mono">${o.cost.toFixed(4)}</p>
 <p className="text-xs text-[var(--text-muted)]">{o.tokens.toLocaleString()} توكن</p>
 </div>
 </div>
 ))}
 </div>
 ...
 <td className="p-2 text-xs text-[var(--text-muted)]">{c.tenant_name ||"-"}</td>
 <td className="p-2 font-mono text-xs">${c.cost.toFixed(4)}</td>
 <td className="p-2 text-xs text-[var(--text-muted)]">{new Date(c.created_at).toLocaleDateString("ar-SA")}</td>
 </tr>
 ))}
 </tbody>
 </table>
