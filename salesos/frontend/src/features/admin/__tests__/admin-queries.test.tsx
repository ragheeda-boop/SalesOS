...
jest.mock("@/lib/api", () => ({
  ...
  getAdminDetailedHealth: jest.fn().mockResolvedValue({
  ...
  getAdminAICostSummary: jest.fn().mockResolvedValue({
    total_cost: 0,
    total_tokens: 0,
    by_model: [],
    by_tenant: [],
  ...
  getAdminAIUsage: jest.fn().mockResolvedValue({
  ...
  getAdminTenant: jest.fn().mockResolvedValue({
    id: "1",
    name: "Test",
    slug: "test",
    plan: "free",
  ...
  getAdminTenantUsage: jest.fn().mockResolvedValue({
    tenant_id: "1",
    tenant_name: "Test",
    api_calls: 100,
    storage_mb: 50,
  ...
  getAdminTenantSubscription: jest.fn().mockResolvedValue(null),
  listAdminBillingCatalog: jest.fn().mockResolvedValue([]),
  createAdminStripeCheckoutSession: jest.fn(),
  createAdminStripePortalSession: jest.fn(),
  listAdminPlatformInvoices: jest.fn().mockResolvedValue([]),
  listAdminUsageMeters: jest.fn().mockResolvedValue([]),
...
describe("Admin Query Hooks", () => {
  ...
  it("useAdminDetailedHealth fetches health", async () => {
    const { result } = renderHook(() => useAdminDetailedHealth(), {
      wrapper: createWrapper(),
    });
    await waitFor(() => expect(result.current.isSuccess).toBe(true));
