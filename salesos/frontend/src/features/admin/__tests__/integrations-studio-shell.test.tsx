...
import { IntegrationsStudioShell, STUDIO_STEPS } from "../IntegrationsStudioShell";
...
describe("IntegrationsStudioShell — STORY-08-07 pointer", () => {
  it("points Owner Console prep to live tenant Studio", () => {
    ...
    expect(screen.getByTestId("integrations-studio-tenant-link")).toHaveAttribute(
      "href",
      "/integrations"
    );
    for (const step of STUDIO_STEPS) {
      expect(screen.getByTestId(`integrations-studio-step-${step.id}`)).toHaveAttribute(
        "href",
        `/integrations?step=${step.id}`
      );
    }
  });
});
