...
export function FeedbackWidget({ className }: FeedbackWidgetProps) {
 ...
 {t("feedback.quick_rate")}
 </button>
 )
 }

 return (
 <div className="fixed bottom-4 left-4 z-40 w-80 rounded-2xl border border-[var(--border-color)] bg-[var(--bg-primary)] p-4 shadow-2xl">
 {submitted ? (
 <div className="flex flex-col items-center py-4 text-center">
 <Star className="mb-2 h-8 w-8 text-[var(--status-warning-text)]" />
 <p className="text-sm font-medium text-[var(--text-primary)]">{t("feedback.thank_you")}</p>
 </div>
 ) : (
 <>
 <p className="mb-3 text-sm font-medium text-[var(--text-primary)]">{t("feedback.question")}</p>
 ...
 placeholder={t("feedback.tell_us_more")}
 rows={2}
 className="mb-2 w-full resize-none rounded-lg border border-[var(--border-color)] bg-transparent p-2 text-xs text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:border-primary-500 focus:outline-none"
 />
 <div className="flex gap-2">
 <button onClick={() => setOpen(false)} className="flex-1 rounded-lg px-3 py-1.5 text-xs text-[var(--text-muted)] hover:bg-[var(--bg-tertiary)]">{t("common.cancel")}</button>
 <button
