...
export function FeedbackWidget({ className }: FeedbackWidgetProps) {
  ...
  const handleSubmit = useCallback(() => {
    track({
      type: "pilot.feedback_submitted",
      metadata: { nps, comment: comment.slice(0, 100) },
    });
    setSubmitted(true);
    setTimeout(() => {
      setOpen(false);
        ...
        <>
          ...
          <div className="flex gap-2">
            <button
              ...
              className="flex-1 rounded-lg px-3 py-1.5 text-xs text-[var(--text-muted)] hover:bg-[var(--bg-tertiary)]"
            >
              {t("common.cancel")}
            </button>
            <button
