...
const sample: AnalyticsData = {
  ...
  pipeline: {
    totalValue: 8500000,
    weightedValue: 4200000,
    dealCount: 24,
    winRate: 0.33,
  },
  widgets: {
    mostUsed: "company-dna",
    usageCount: 120,
    widgets: [{ id: "dna", name: "Company DNA", count: 120 }],
  },
  search: { totalQueries: 420, avgResults: 12, topQueries: ["شركات الطاقة"] },
  nba: { shown: 85, executed: 38, acceptanceRate: 45 },
};

function renderView(d: AnalyticsData = sample) {
  return render(<AnalyticsView data={d} />);
}

describe("AnalyticsView", () => {
  it("renders active users", () => {
    renderView();
    const u = screen.getAllByText("12");
  ...
  it("renders NBA acceptance rate", () => {
    renderView();
    expect(screen.getByText(/%45/)).toBeInTheDocument();
  });
  it("renders widget name", () => {
    renderView();
    expect(screen.getByText("Company DNA")).toBeInTheDocument();
  });
  it("renders top query", () => {
    renderView();
...
describe("CommercialAnalyticsWidget", () => {
  it("is a valid widget", () => {
    expect(CommercialAnalyticsWidget).toBeDefined();
  });
});
