...
export function AnalyticsView({ data }: AnalyticsViewProps) {
 ...
 <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
    <MetricCard icon={<Users className="h-4 w-4 text-blue-600" />} label={t("analytics.active_users")} value={data.users.active.toString()} sub={t("analytics.of_total", { total: data.users.total })} color="bg-blue-50 dark:bg-blue-900/20" />
 <MetricCard icon={<Activity className="h-4 w-4 text-[var(--status-success-text)]" />} label={t("analytics.daily_sessions")} value={data.usage.dailyActiveUsers.toString()} sub={t("analytics.avg_duration", { minutes: Math.round(data.usage.avgSessionDuration / 60) })} color="bg-[var(--status-success-bg)]" />
 <MetricCard icon={<DollarSign className="h-4 w-4 text-[var(--status-warning-text)]" />} label={t("analytics.pipeline_value")} value={`$${data.pipeline.totalValue >= 1e6 ? (data.pipeline.totalValue / 1e6).toFixed(1) + 'M' : (data.pipeline.totalValue / 1e3).toFixed(0) + 'K'}`} sub={t("analytics.deal_count", { count: data.pipeline.dealCount })} color="bg-[var(--status-warning-bg)]" />
    <MetricCard icon={<Target className="h-4 w-4 text-[var(--chart-purple)]" />} label={t("analytics.win_rate")} value={`%${Math.round(data.pipeline.winRate * 100)}`} sub={`${t("executive.weighted")} $${data.pipeline.weightedValue >= 1e6 ? (data.pipeline.weightedValue / 1e6).toFixed(1) + 'M' : (data.pipeline.weightedValue / 1e3).toFixed(0) + 'K'}`} color="bg-[var(--chart-purple-bg)] dark:bg-[var(--bg-primary)]/20" />
 </div>
 ...
 <span className="text-sm font-semibold text-[var(--text-primary)]">{t("analytics.most_used_widgets")}</span>
 </div>
 ...
 <span className="text-sm font-semibold text-[var(--text-primary)]">{t("analytics.search_analytics")}</span>
 </div>
 ...
 <div className="space-y-3">
 <div>
 <p className="text-xs text-[var(--text-muted)]">{t("analytics.shown_recommendations")}</p>
 <p className="text-lg font-bold text-[var(--text-primary)]">{data.nba.shown}</p>
 </div>
 <div>
 <p className="text-xs text-[var(--text-muted)]">{t("analytics.executed_recommendations")}</p>
 <p className="text-lg font-bold text-[var(--status-success-text)]">{data.nba.executed}</p>
 </div>
 <div>
