...
export function SearchHistory({ onReRun, currentQuery, currentStrategy }: SearchHistoryProps) {
 ...
 const updated = [entry, ...history.filter((h) => h.query !== query || h.strategy !== strategy)].slice(0, MAX_HISTORY)
 setHistory(updated)
 saveHistory(updated)
 },
 [history]
...
"rounded-md px-2.5 py-1 text-xs font-medium transition-colors",
 ...
 <span className="text-[10px] text-[var(--text-muted)] whitespace-nowrap">{entry.strategy}</span>
 {entry.resultCount !== undefined && (
 <span className="text-[10px] text-[var(--text-muted)]">({entry.resultCount})</span>
 )}
 </button>
