...
export function TourOverlay() {
  ...
  return (
    <>
      ...
      <div
        ref={tooltipRef}
        className="fixed z-[9999] w-[320px] sm:w-[360px] rounded-xl bg-[var(--bg-primary)] shadow-muhide-6 overflow-hidden"
        style={{
          top: tooltipPos.top,
      ...
      >
        <div className="relative">
          ...
          <button
            onClick={endTour}
            className="absolute top-2 left-2 rounded-full bg-[var(--bg-primary)]/80 p-1 hover:bg-[var(--bg-primary)] transition-colors"
        ...
        <div className="flex items-center justify-between border-t border-[var(--border-subtle)] px-4 py-3">
          ...
          <div className="flex items-center gap-2">
            {!isFirst && (
              <button
                onClick={prevStep}
                className="flex items-center gap-1 rounded-lg px-3 py-1.5 text-sm font-medium text-[var(--text-secondary)] hover:bg-[var(--bg-tertiary)] transition-colors"
              >
            ...
            {!isFirst && (
              <button
                onClick={endTour}
                className="text-xs text-[var(--text-disabled)] hover:text-[var(--text-secondary)] transition-colors px-2"
              >
                تخطي
              </button>
            )}
