...
const actionConfig: Record<string, { icon: typeof Mail; color: string }> = {
 ...
 note_added: { icon: MessageSquare, color:"text-[var(--text-secondary)] bg-[var(--bg-tertiary)]" },
 note_updated: { icon: Edit3, color:"text-[var(--text-secondary)] bg-[var(--bg-tertiary)]" },
...
export function TimelineWidget({ entityType, entityId, title, limit = 20, className }: TimelineWidgetProps) {
 ...
 <p className="py-8 text-center text-sm text-[var(--text-muted)]">لا توجد نشاطات حتى الآن</p>
 ) : (
 <div className="space-y-0">
 {activities.map((activity, idx) => {
 const config = actionConfig[activity.action] || { icon: Clock, color:"text-[var(--text-secondary)] bg-[var(--bg-tertiary)]" };
