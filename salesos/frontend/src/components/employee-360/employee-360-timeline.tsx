...
export function EmployeeTimeline({ employeeId }: { employeeId: string }) {
  ...
  >([]);
  const [_cursorStack, setCursorStack] = useState<string[]>([]);
  ...
  return (
    <div className="space-y-4">
              ...
              <div className="flex gap-4">
                <div>
                  <label className="mb-1 block text-xs font-medium text-[var(--text-secondary)]">
                    {t("emp360.filter_from")}
                  </label>
                  <input
                    type="date"
                    value={dateFrom}
                    onChange={(e) => setDateFrom(e.target.value)}
                    className="rounded-lg border border-[var(--border-default)] bg-[var(--bg-primary)] px-3 py-1.5 text-xs"
                  />
                </div>
                <div>
                  <label className="mb-1 block text-xs font-medium text-[var(--text-secondary)]">
      ...
      {data && !data.has_next && allEvents.length > 0 && (
        <p className="text-center text-xs text-[var(--text-disabled)] pt-2">
          {t("emp360.all_events_loaded")}
        </p>
      )}
    </div>
  );
