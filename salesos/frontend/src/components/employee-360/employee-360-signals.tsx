            ...
            {signals.by_type.map((s) => (
              <div key={s.type} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs font-medium text-[var(--text-primary)] w-6 text-right">{s.count}</span>
                </div>
              </div>
            ))}
            {signals.by_type.length === 0 && <p className="text-xs text-[var(--text-disabled)]">{t("emp360.no_data")}</p>}
          </div>
        </CardContent>
      </Card>
          ...
          <div className="space-y-2">
            {signals.by_source.map((s) => (
              <div key={s.source} className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  <div className="h-2 w-24 overflow-hidden rounded-full bg-[var(--bg-tertiary)]">
                    <div className="h-full rounded-full bg-[var(--chart-purple)]" style={{ width: `${Math.min(100, (s.count / signals.total) * 100)}%` }} />
                  </div>
                  <span className="text-xs font-medium text-[var(--text-primary)] w-6 text-right">{s.count}</span>
                </div>
              </div>
            ))}
            {signals.by_source.length === 0 && <p className="text-xs text-[var(--text-disabled)]">{t("emp360.no_data")}</p>}
          </div>
        </CardContent>
      </Card>
            ...
            {signals.trend.map((p) => (
              <div key={p.date} className="flex items-center justify-between text-xs">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span className="font-medium text-[var(--text-secondary)] w-6 text-right">{p.count}</span>
                </div>
              </div>
            ))}
            {signals.trend.length === 0 && <p className="text-xs text-[var(--text-disabled)]">{t("emp360.no_data")}</p>}
          </div>
          <p className="mt-3 text-center text-[10px] text-[var(--text-disabled)]">{t("emp360.signals_total", { count: signals.total })}</p>
        </CardContent>
      </Card>
    </div>
  )
