...
import {
  Mail, Calendar, CheckCircle, Phone, Clock,
} from "lucide-react"
...
export function ScoreBadge({ score }: { score: number | null }) {
  if (score === null || score === undefined) return <span className="text-[var(--text-disabled)]">-</span>
...
export function StatBox({ icon: Icon, label, value, color }: {
  icon: ElementType
  label: string
  value: ReactNode
  color: string
...
export const actionConfig: Record<string, { icon: typeof Mail; color: string }> = {
  ...
  note_added: { icon: Mail, color: "text-[var(--text-secondary)] bg-[var(--bg-tertiary)]" },
  contract_signed: { icon: CheckCircle, color: "text-[var(--chart-purple)] bg-[var(--chart-purple-bg)] dark:text-[var(--chart-purple)] dark:bg-[var(--bg-primary)]/50" },
}

export function getActionConfig(action: string) {
  return actionConfig[action] || { icon: Clock, color: "text-[var(--text-secondary)] bg-[var(--bg-tertiary)]" }
}

export const SOURCE_OPTIONS = ["crm", "timeline", "workflow", "email", "calendar", "manual"]
export const TYPE_OPTIONS = ["email_sent", "email_received", "meeting_created", "meeting_completed", "call", "task_created", "task_completed", "note_added", "contract_signed"]
