...
const STAGES = [
 { key:"prospecting", labelKey:"pipeline.stage.prospecting", color:"bg-info-500" },
 { key:"qualification", labelKey:"pipeline.stage.qualification", color:"bg-indigo-500" },
 { key:"proposal", labelKey:"pipeline.stage.proposal", color:"bg-warning-500" },
 { key:"negotiation", labelKey:"pipeline.stage.negotiation", color:"bg-orange-500" },
];
...
export function PipelineColumn({
 ...
 stage: { key: string; labelKey: string; color: string };
...
"flex w-[260px] shrink-0 flex-col rounded-xl border border-[var(--border-default)] bg-[var(--bg-secondary)]/80",
 ...
 <span className={cn("h-2.5 w-2.5 rounded-full", stage.color)} />
      <span className="text-sm font-semibold text-[var(--text-secondary)]">{t(stage.labelKey)}</span>
 ...
 <p className="py-8 text-center text-xs text-[var(--text-disabled)]">
 {t("pipeline.drag_here")}
 </p>
 )}
 </div>
 </div>
...
function CreateOpportunityModal() {
 ...
 <ModalContent title={t("opportunities.create_title")}>
 ...
 className="text-[var(--text-disabled)] hover:text-danger-500"
 >
 <X className="h-4 w-4" />
 </button>
 </div>
 ...
 <>
 <Input
 placeholder={t("opportunities.search_company")}
 value={companySearch}
 ...
 <Input
 placeholder={t("opportunities.name_example")}
 value={name}
...
export function PipelineKanban() {
 ...
 {t("opportunities.load_error")} {error.message}
 </div>
 );
 }
 ...
 if (isLoading) {
 return (
 <div className="flex items-center justify-center py-20 text-[var(--text-disabled)]">
 {t("common.loading")}
 </div>
 );
 }
 ...
 <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-full max-w-md -translate-x-1/2 -translate-y-1/2 rounded-xl bg-[var(--bg-primary)] p-0 shadow-muhide-6">
 <div className="border-b border-[var(--border-default)] px-4 py-3">
 <Dialog.Title className="text-base font-semibold text-[var(--text-primary)]">
 {t("opportunities.close_won_title")}
 </Dialog.Title>
 </div>
 ...
 >
 {closeWon.isPending ?"..." : t("opportunities.confirm_win")}
 </Button>
 <Button
 variant="ghost"
 ...
 <div className="border-b border-[var(--border-default)] px-4 py-3">
 <Dialog.Title className="text-base font-semibold text-[var(--text-primary)]">
 {t("opportunities.close_lost_title")}
 </Dialog.Title>
 </div>
 <div className="flex flex-col gap-3 p-4">
 <p className="text-sm text-[var(--text-secondary)]">{selectedOpp?.name}</p>
 ...
 <Input
 placeholder={t("opportunities.loss_reason_example")}
 value={lossReason}
