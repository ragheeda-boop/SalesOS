...
function HealthScoreRing({ score, label }: { score: number; label: string }) {
 ...
 <circle cx="34" cy="34" r={radius} fill="none" stroke="currentColor" className="text-[var(--text-primary)]" strokeWidth="5" />
...
export function CompanyWorkspace({ companyId }: CompanyWorkspaceProps) {
 ...
 <p className="text-lg font-semibold text-[var(--text-primary)]">{t("company.load_error")}</p>
...
"inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium",
 ...
 <div key={i} className="flex items-center gap-2 rounded-lg border border-[var(--border-default)] px-3 py-2 text-sm">
 <Avatar
 size="sm"
 fallback={String(emp.full_name || emp.name ||"").split("").map((n: string) => n[0]).join("").slice(0, 2).toUpperCase()}
 className="h-7 w-7 text-xs"
 />
 <span className="text-[var(--text-primary)]">{String(emp.full_name || emp.name)}</span>
 {emp.role ? <span className="text-xs text-[var(--text-muted)]">{String(emp.role)}</span> : null}
 </div>
 ))}
 </div>
 </div>
