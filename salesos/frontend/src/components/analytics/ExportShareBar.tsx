...
import { Badge, Modal, ModalTrigger, ModalContent, ModalHeader, ModalBody, ModalFooter, Input } from"@salesos/ui"
import {
 Download,
 Share2,
 FileText,
...
export function ExportShareBar({ reportName }: { reportName?: string }) {
...
"flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-medium transition",
 ...
 <p className="text-xs text-[var(--text-muted)]">Recipients ({recipients.length})</p>
 {recipients.map((r) => (
 <div key={r.email} className="flex items-center justify-between rounded-lg bg-[var(--bg-secondary)] px-3 py-2">
 <div className="flex items-center gap-2">
 <Mail className="h-3.5 w-3.5 text-[var(--text-muted)]" />
 <span className="text-xs text-[var(--text-primary)]">{r.email}</span>
 <Badge variant={r.permission ==="admin" ?"danger" : r.permission ==="edit" ?"warning" :"outline"}>
 {r.permission}
 </Badge>
 </div>
 <button
...
"rounded-lg px-3 py-1.5 text-xs font-medium transition",
 ...
 <div key={email} className="flex items-center justify-between rounded-lg bg-[var(--bg-secondary)] px-3 py-2">
 <span className="text-xs text-[var(--text-primary)]">{email}</span>
 <button
 onClick={() => setScheduleRecipients(scheduleRecipients.filter((e) => e !== email))}
 className="text-[var(--text-muted)] hover:text-[var(--status-danger-text)] transition"
 >
 <X className="h-3.5 w-3.5" />
 </button>
