...
function LatencyChart({ data }: { data: LatencyBucket[] }) {
 ...
 <h3 className="text-sm font-semibold text-[var(--text-primary)]">Latency Distribution (p95)</h3>
...
function LatencyTable({ data }: { data: LatencyBucket[] }) {
 ...
 <th className="text-right px-3 py-2 text-xs font-medium text-[var(--text-muted)]">Strategy</th>
 <th className="text-right px-3 py-2 text-xs font-medium text-[var(--text-muted)]">p50</th>
 <th className="text-right px-3 py-2 text-xs font-medium text-[var(--text-muted)]">p95</th>
 <th className="text-right px-3 py-2 text-xs font-medium text-[var(--text-muted)]">p99</th>
 </tr>
 </thead>
 <tbody>
 {data.map((item) => (
...
"rounded-md px-3 py-1.5 text-xs font-medium transition-colors",
 ...
 <LineChart
 series={[{ name:"Queries", color:"#F57C1E", data: volumeChart.map((v) => v.value) }]}
 height={220}
 />
 ) : (
 ...
 <div className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-primary)] p-4">
 <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-3">Top Queries Detail</h3>
 <div className="rounded-lg border border-[var(--border-default)] overflow-hidden">
 <table className="w-full text-sm">
 <thead>
 <tr className="bg-[var(--bg-secondary)] border-b border-[var(--border-default)]">
 <th className="text-right px-3 py-2 text-xs font-medium text-[var(--text-muted)]">#</th>
 <th className="text-right px-3 py-2 text-xs font-medium text-[var(--text-muted)]">Query</th>
 <th className="text-right px-3 py-2 text-xs font-medium text-[var(--text-muted)]">Count</th>
 <th className="text-right px-3 py-2 text-xs font-medium text-[var(--text-muted)]">Avg Results</th>
 </tr>
 </thead>
 <tbody>
 ...
 <td className="px-3 py-2 text-[var(--text-secondary)]">{q.avg_results.toFixed(0)}</td>
 </tr>
 ))}
 </tbody>
 </table>
