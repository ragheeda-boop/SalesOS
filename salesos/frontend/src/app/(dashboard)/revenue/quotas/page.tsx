...
function StatusBadge({ status }: { status: QuotaItem["status"] }) {
  const config = {
    on_track: {
      label: "On Track",
      color: "bg-[var(--status-success-bg)] text-[var(--status-success-text)]",
    },
  ...
  const fetchQuotas = useCallback(async () => {
    ...
    try {
      ...
      if (workspace?.opportunities?.recent?.length) {
        ...
        for (const [i, o] of (
          workspace.opportunities.recent as Array<{
            id: string;
            name: string;
            value: number;
          }>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-[var(--border-default)] bg-[var(--bg-secondary)]">
                <th className="text-right px-4 py-3 text-xs font-medium text-[var(--text-muted)]">
                  Rep
                </th>
                <th className="text-right px-4 py-3 text-xs font-medium text-[var(--text-muted)]">
                  Target
