...
import { Input, Button, Badge, Card, Spinner, Modal, ModalTrigger, ModalContent, ModalHeader, ModalBody, ModalFooter, useToast } from"@salesos/ui"
import { Plus, Flag, ToggleLeft, ToggleRight, Loader2, Edit3 } from"lucide-react"
import { useAdminFeatureFlags, useCreateAdminFeatureFlag, useUpdateAdminFeatureFlag, useAdminFlagTenants, useToggleAdminFlagForTenant, useAdminTenants } from"@/lib/hooks/adminQueries"
...
export default function AdminFlagsPage() {
 ...
 const [createForm, setCreateForm] = useState({ key:"", name:"", description:"", enabled: false })
 const [editForm, setEditForm] = useState({ name:"", description:"", enabled: false, rollout_percent: 100 })
 ...
 <p className="mt-1 text-sm text-[var(--text-muted)]">Manage feature toggles, rollout percentages, and per-tenant overrides.</p>
 </div>
 ...
 {isLoading ? (
 <Card className="p-12 text-center"><Spinner className="mx-auto h-6 w-6" /><p className="mt-2 text-sm text-[var(--text-muted)]">Loading flags...</p></Card>
 ) : !flags?.length ? (
 ...
 <p className="mt-1 text-sm text-[var(--text-disabled)]">Create your first flag to start managing feature rollouts.</p>
 </Card>
 ) : (
 ...
 selectedFlag === flag.id ?"bg-[var(--muhide-orange)]/5" :"hover:bg-[var(--bg-secondary)] dark:hover:bg-[var(--bg-secondary)]/50"
 }`}
 >
 <div className="flex items-center gap-3">
 <div className={`flex h-10 w-10 items-center justify-center rounded-lg ${flag.enabled ?"bg-success-100 text-success-600 dark:bg-success-900/30" :"bg-[var(--bg-tertiary)] text-[var(--text-disabled)]"}`}>
 ...
 {flag.description && <p className="text-xs text-[var(--text-disabled)] mt-0.5 max-w-md truncate">{flag.description}</p>}
 </div>
 </div>
 <div className="flex items-center gap-3">
 ...
 onClick={(e) => {
 e.stopPropagation()
 setEditForm({ name: flag.name, description: flag.description ||"", enabled: flag.enabled, rollout_percent: 100 })
 setShowEdit(flag)
 }}
 >
 ...
 <p className="text-sm text-[var(--text-muted)]">Select a flag to manage per-tenant overrides</p>
 </Card>
 )}
 </div>
 </div>
 ...
 <div>
 <label className="mb-1 block text-sm font-medium text-[var(--text-secondary)]">Key *</label>
 <Input value={createForm.key} onChange={(e) => setCreateForm({ ...createForm, key: e.target.value })} placeholder="new_dashboard" className="font-mono" />
 <p className="mt-1 text-xs text-[var(--text-disabled)]">Unique identifier for this flag (snake_case)</p>
 </div>
 <div>
 <label className="mb-1 block text-sm font-medium text-[var(--text-secondary)]">Name *</label>
 <Input value={createForm.name} onChange={(e) => setCreateForm({ ...createForm, name: e.target.value })} placeholder="New Dashboard" />
 </div>
 <div>
 <label className="mb-1 block text-sm font-medium text-[var(--text-secondary)]">Description</label>
 <Input value={createForm.description} onChange={(e) => setCreateForm({ ...createForm, description: e.target.value })} placeholder="Enables the new dashboard UI" />
 </div>
 <div className="flex items-center gap-3">
 <button
 type="button"
 ...
 >
 <span className={`pointer-events-none inline-block h-5 w-5 rounded-full bg-[var(--bg-primary)] shadow transition-transform ${
 ...
 <Button onClick={handleCreate} disabled={!createForm.key || !createForm.name || createMutation.isPending} leftIcon={createMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : undefined}>
 ...
 <div>
 <label className="mb-1 block text-sm font-medium text-[var(--text-secondary)]">Rollout %</label>
 <div className="flex items-center gap-3">
 <input
 type="range"
 min={0}
 ...
 onChange={(e) => setEditForm({ ...editForm, rollout_percent: Number(e.target.value) })}
 className="flex-1 accent-[var(--muhide-orange)]"
 />
 <span className="w-12 text-right text-sm font-mono font-medium text-[var(--text-primary)]">{editForm.rollout_percent}%</span>
 </div>
 </div>
 <div className="flex items-center gap-3">
 <button
 ...
 <span className="text-sm text-[var(--text-secondary)]">{editForm.enabled ?"Enabled" :"Disabled"}</span>
 </div>
 </div>
 </ModalBody>
 <ModalFooter>
...
function FlagTenantOverride({ flagId }: { flagId: string }) {
 ...
 const handleToggle = useCallback(async (tenantId: string, currentEnabled: boolean) => {
 try {
 await toggleMutation.mutateAsync({ tenantId, enabled: !currentEnabled })
 toast({ variant:"success", title: currentEnabled ?"Disabled for tenant" :"Enabled for tenant" })
 } catch {
 ...
 {isLoading ? (
 <div className="py-8 text-center"><Spinner className="mx-auto h-5 w-5" /></div>
 ) : !overrides?.length ? (
 <p className="py-4 text-center text-sm text-[var(--text-muted)]">No tenant overrides configured</p>
 ) : (
 <div className="space-y-2">
 {overrides.map((o) => (
 <div key={o.tenant_id} className="flex items-center justify-between rounded-lg border border-[var(--border-default)] p-3">
 <div>
