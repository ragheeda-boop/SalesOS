...
import { Database, RefreshCw, CheckCircle2, AlertTriangle, XCircle, Clock, ArrowLeft, History, Zap, Link2 } from"lucide-react"
...
const STATUS_CONFIG: Record<string, { color:"success" |"warning" |"danger" |"default"; icon: typeof CheckCircle2; label: string }> = {
...
export default function ConnectorsPage() {
 ...
 setConnectors([
 { id:"conn-crm-1", name:"CRM Connector", type:"crm", status:"active", last_sync: new Date(Date.now() - 3600000).toISOString(), sync_count: 142 },
 { id:"conn-erp-1", name:"ERP Connector", type:"erp", status:"active", last_sync: new Date(Date.now() - 7200000).toISOString(), sync_count: 87 },
 { id:"conn-mkt-1", name:"Market Feed", type:"market", status:"error", last_sync: new Date(Date.now() - 86400000).toISOString(), sync_count: 23, error_message:"API rate limit exceeded" },
 ])
 } finally {
 setLoading(false)
 }
 ...
 setConnectors((prev) =>
 prev.map((c) => (c.id === connectorId ? { ...c, last_sync: new Date().toISOString(), status:"active" as const } : c))
 )
 if (selectedConnector === connectorId) loadSyncHistory(connectorId)
 } catch {
 setConnectors((prev) =>
 prev.map((c) => (c.id === connectorId ? { ...c, status:"error" as const, error_message:"Sync failed" } : c))
 )
 } finally {
 setSyncingId(null)
 }
 ...
 <Link href="/knowledge" className="p-1.5 rounded-md text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-secondary)] transition-colors">
 <ArrowLeft className="h-4 w-4" />
 </Link>
 <div>
 <h1 className="text-2xl font-bold text-[var(--text-primary)]">Data Fabric Connectors</h1>
 <p className="mt-1 text-sm text-[var(--text-secondary)]">Manage data source connections and synchronization</p>
 </div>
 </div>
 <Button onClick={loadConnectors} variant="outline" leftIcon={<RefreshCw className="h-4 w-4" />}>
 Refresh
 </Button>
 </div>
 ...
 <span>{error}</span>
 <button onClick={loadConnectors} className="underline text-xs">Retry</button>
 </div>
 )}
 ...
 return (
 <div key={status} className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-primary)] p-4">
 ...
 <p className="mt-1.5 text-xs text-[var(--status-danger-text)]">{connector.error_message}</p>
 )}
 </div>
 ...
 disabled={isSyncing}
 leftIcon={isSyncing ? <Spinner className="h-3.5 w-3.5" /> : <RefreshCw className="h-3.5 w-3.5" />}
 >
 ...
 <h4 className="text-xs font-semibold text-[var(--text-secondary)] uppercase tracking-wider">Sync History</h4>
 </div>
 ...
 <p className="text-xs text-[var(--text-muted)] py-4">No sync history available</p>
 ) : (
 ...
 sync.status ==="success" ?"bg-green-500" : sync.status ==="failed" ?"bg-red-500" :"bg-yellow-500"
 }`} />
 <div className="flex-1 min-w-0">
 ...
 <Badge variant={sync.status ==="success" ?"success" : sync.status ==="failed" ?"danger" :"warning"}>
 {sync.status}
 </Badge>
 {sync.error && (
