...
import { Spinner, Tooltip, Badge, Button, Input, EmptyState } from"@salesos/ui"
import { Search, X, ZoomIn, ZoomOut, Maximize2, Network, ChevronRight, Building2, User, Package, Calendar } from"lucide-react"
...
function getDemoData(): { nodes: KNode[]; edges: KEdge[] } {
 return {
 nodes: [
 { id:"c1", label:"أرامكو السعودية", type:"company", x: 0, y: 0, vx: 0, vy: 0, fx: null, fy: null, radius: 28 },
 { id:"c2", label:"سابك", type:"company", x: 0, y: 0, vx: 0, vy: 0, fx: null, fy: null, radius: 28 },
 { id:"c3", label:"STC", type:"company", x: 0, y: 0, vx: 0, vy: 0, fx: null, fy: null, radius: 28 },
 { id:"c4", label:"الراجحي", type:"company", x: 0, y: 0, vx: 0, vy: 0, fx: null, fy: null, radius: 28 },
 { id:"c5", label:"دار الأركان", type:"company", x: 0, y: 0, vx: 0, vy: 0, fx: null, fy: null, radius: 28 },
 { id:"p1", label:"محمد العلي", type:"person", x: 0, y: 0, vx: 0, vy: 0, fx: null, fy: null, radius: 20 },
 { id:"p2", label:"فهد الخالدي", type:"person", x: 0, y: 0, vx: 0, vy: 0, fx: null, fy: null, radius: 20 },
 { id:"p3", label:"سارة المطيري", type:"person", x: 0, y: 0, vx: 0, vy: 0, fx: null, fy: null, radius: 20 },
 { id:"pr1", label:"نظام CRM", type:"product", x: 0, y: 0, vx: 0, vy: 0, fx: null, fy: null, radius: 22 },
 { id:"pr2", label:"منصة التحليلات", type:"product", x: 0, y: 0, vx: 0, vy: 0, fx: null, fy: null, radius: 22 },
 { id:"ev1", label:"مؤتمر التقنية 2026", type:"event", x: 0, y: 0, vx: 0, vy: 0, fx: null, fy: null, radius: 18 },
 ],
 edges: [
...
export default function KnowledgeGraphPage() {
 ...
 ids = new Set(
 layoutNodes.filter((n) => ids.has(n.id) && (n.label.toLowerCase().includes(lower) || n.type.toLowerCase().includes(lower))).map((n) => n.id)
 )
 }
 return ids
 ...
 if (highlighted.size > 0 && searchFilter) return new Set([...highlighted].filter((id) => filteredNodeIds.has(id)))
 ...
 return { target: otherId, targetLabel: other?.label || otherId, relation: e.label }
 })
 setSelected({ id: node.id, label: node.label, type: node.type, properties: {}, relationships })
 setHighlighted(new Set([node.id, ...rels.map((e) => e.source), ...rels.map((e) => e.target)]))
 },
 [edges, nodes]
 )

 const handleToggleEntityFilter = useCallback((type: string) => {
 setEntityFilter((prev) => (prev.includes(type) ? prev.filter((t) => t !== type) : [...prev, type]))
 }, [])
 ...
 setNodes((prev) => prev.map((n) => (n.id === draggingNode ? { ...n, x: nx, y: ny, fx: nx, fy: ny } : n)))
 return
 }
 ...
 if (draggingNode) {
 setNodes((prev) => prev.map((n) => (n.id === draggingNode ? { ...n, fx: null, fy: null } : n)))
 ...
 style={active ? { borderColor: getColor(type), color: getColor(type), backgroundColor: `${getColor(type)}15` } : undefined}
 >
 ...
 {searchFilter && (
 <button
 onClick={() => { setSearchFilter(""); setHighlighted(new Set()) }}
 ...
 <button onClick={() => setZoom((z) => Math.min(3, z + 0.2))} className="p-1.5 rounded-md bg-[var(--bg-primary)] border border-[var(--border-default)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:border-[var(--border-hover)] transition-colors shadow-sm">
 <ZoomIn className="h-4 w-4" />
 </button>
 </Tooltip>
 <Tooltip content="Zoom out" side="left">
 <button onClick={() => setZoom((z) => Math.max(0.3, z - 0.2))} className="p-1.5 rounded-md bg-[var(--bg-primary)] border border-[var(--border-default)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:border-[var(--border-hover)] transition-colors shadow-sm">
 <ZoomOut className="h-4 w-4" />
 </button>
 </Tooltip>
 <Tooltip content="Reset view" side="left">
 <button onClick={handleResetView} className="p-1.5 rounded-md bg-[var(--bg-primary)] border border-[var(--border-default)] text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:border-[var(--border-hover)] transition-colors shadow-sm">
 ...
 {hl && edge.label && (
 <>
 <rect x={mx - lw / 2} y={my - 10} width={lw} height={14} rx={3} fill="var(--bg-primary)" stroke="var(--border-default)" strokeWidth={0.5} opacity={0.9} />
 <text x={mx} y={my} textAnchor="middle" dominantBaseline="central" className="fill-[var(--text-secondary)]" style={{ fontSize:"9px", fontFamily:"var(--font-ui)" }}>
 {edge.label}
 </text>
 </>
 )}
 ...
 onClick={(e) => { e.stopPropagation(); if (!isDragging) handleNodeClick(node) }}
 onMouseDown={(e) => handleNodeMouseDown(e, node.id)}
 onDoubleClick={(e) => { e.stopPropagation(); handleExpandNode(node.id) }}
 onMouseEnter={() => setHoveredNode(node.id)}
 onMouseLeave={() => setHoveredNode(null)}
 >
 {isActive && (
 <circle r={node.radius + 8} fill="none" stroke={color} strokeWidth={2} opacity={0.3}>
 <animate attributeName="r" values={`${node.radius + 6};${node.radius + 10};${node.radius + 6}`} dur="2s" repeatCount="indefinite" />
 </circle>
 )}
 ...
 />
 <text textAnchor="middle" dominantBaseline="central" className="pointer-events-none select-none" style={{ fontSize: node.radius > 22 ?"11px" :"9px", fontFamily:"var(--font-ui)", fontWeight: 600, fill:"white" }}>
 {getInitials(node.label)}
 </text>
 <text textAnchor="middle" y={node.radius + 14} className="pointer-events-none select-none" style={{ fontSize:"11px", fontFamily:"var(--font-ui)", fontWeight: isActive || isHovered ? 600 : 400, fill: isDimmed ?"transparent" : isActive ? color :"var(--text-primary)" }}>
 ...
 description={hasSearched ?"Try a different search query" :"Search for entities to visualize relationships, or load demo data."}
 action={!hasSearched ? { label:"Load Demo Data", onClick: handleLoadDemo } : undefined}
 />
 )}
 </svg>
 ...
 <button onClick={() => { setSelected(null); setHighlighted(new Set()) }} className="p-1 rounded-md text-[var(--text-muted)] hover:text-[var(--text-primary)] hover:bg-[var(--bg-secondary)] transition-colors">
 <X className="h-4 w-4" />
 </button>
 </div>
 ...
 <div className="w-10 h-10 rounded-full flex items-center justify-center text-white text-sm font-bold shrink-0" style={{ background: getColor(selected.type) }}>
 {getInitials(selected.label)}
 </div>
 <div className="min-w-0">
 <p className="text-sm font-semibold text-[var(--text-primary)] truncate">{selected.label}</p>
 <Badge variant="default" className="capitalize mt-0.5">{selected.type}</Badge>
 </div>
 </div>
 </div>
 ...
 <button
 key={i}
 onClick={() => { if (targetNode) handleNodeClick(targetNode) }}
 ...
 >
 <div className="flex items-center gap-2">
 <div className="w-6 h-6 rounded-full flex items-center justify-center text-white text-[9px] font-bold shrink-0" style={{ background: getColor(relType) }}>
 {getInitials(rel.targetLabel)}
 </div>
 <div className="min-w-0 flex-1">
 <p className="text-xs font-medium text-[var(--text-primary)] truncate">{rel.targetLabel}</p>
 <p className="text-[10px] text-[var(--text-muted)] font-mono">{rel.relation}</p>
 </div>
 ...
 disabled={loading}
 leftIcon={loading ? <Spinner className="h-4 w-4" /> : <Maximize2 className="h-4 w-4" />}
 className="w-full"
 >
 Expand Subgraph
 </Button>
