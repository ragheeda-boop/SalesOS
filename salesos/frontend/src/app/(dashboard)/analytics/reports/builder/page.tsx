...
import { Badge, Button, Modal, ModalTrigger, ModalContent, ModalHeader, ModalBody, ModalFooter, Input } from"@salesos/ui"
...
const METRICS: MetricOption[] = [
 { id:"revenue", label:"Total Revenue", domain:"Revenue", format:"currency" },
 { id:"arr", label:"ARR", domain:"Revenue", format:"currency" },
 { id:"nrr", label:"Net Revenue Retention", domain:"Revenue", format:"percentage" },
 { id:"churn_rate", label:"Churn Rate", domain:"Revenue", format:"percentage" },
 { id:"pipeline_value", label:"Pipeline Value", domain:"Pipeline", format:"currency" },
 { id:"win_rate", label:"Win Rate", domain:"Pipeline", format:"percentage" },
 { id:"avg_deal_size", label:"Avg Deal Size", domain:"Pipeline", format:"currency" },
 { id:"deals_closed", label:"Deals Closed", domain:"Sales", format:"number" },
 { id:"conversion_rate", label:"Conversion Rate", domain:"Sales", format:"percentage" },
 { id:"employee_count", label:"Employee Count", domain:"Employees", format:"number" },
 { id:"avg_score", label:"Avg Performance Score", domain:"Employees", format:"number" },
 { id:"workflow_runs", label:"Workflow Executions", domain:"Automation", format:"number" },
 { id:"completion_rate", label:"Completion Rate", domain:"Automation", format:"percentage" },
]
...
export default function ReportBuilderPage() {
 ...
 <p className="text-sm text-[var(--text-muted)]">Configure metrics, dimensions, and filters</p>
 </div>
 </div>
 <div className="flex items-center gap-2">
 {saved && (
 <Badge variant="success">Report saved!</Badge>
 )}
 ...
 {config.metrics.length} metrics, {config.dimensions.length} dimensions, {config.viz_type} view
 </div>
 </div>
 </ModalBody>
 <ModalFooter>
...
"flex items-center gap-2 rounded-lg px-3 py-2 cursor-pointer transition",
 ...
 {[
 { id:"chart" as const, label:"Chart", icon: <BarChart3 className="h-4 w-4" /> },
 { id:"table" as const, label:"Table", icon: <Table className="h-4 w-4" /> },
 { id:"both" as const, label:"Both", icon: <Eye className="h-4 w-4" /> },
 ].map((v) => (
 <button
 key={v.id}
...
"flex items-center gap-1.5 rounded-lg px-3 py-2 text-xs font-medium transition",
 ...
 <div key={m.label} className="rounded-lg border border-[var(--border-default)] bg-[var(--bg-secondary)] p-3">
 <p className="text-[10px] text-[var(--text-muted)]">{m.label}</p>
 <p className="text-lg font-bold text-[var(--text-primary)]">{m.value}</p>
 {m.trend !== undefined && (
 <p className={cn("text-[10px]", m.trend >= 0 ?"text-[var(--status-success-text)]" :"text-[var(--status-danger-text)]")}>
 {m.trend >= 0 ?"+" :""}{m.trend}%
 </p>
 )}
 </div>
 ...
 <th key={key} className="text-right px-3 py-2 text-xs font-medium text-[var(--text-muted)] capitalize">
 {key}
 </th>
 ))}
 </tr>
 ...
 <tr key={i} className="border-b border-[var(--border-default)] last:border-0 hover:bg-[var(--bg-secondary)]">
 ...
 <p className="text-sm text-[var(--text-muted)]">Select metrics to preview your report</p>
 </div>
 )}
 </div>
 </div>
