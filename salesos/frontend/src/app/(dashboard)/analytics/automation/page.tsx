...
export default function AutomationAnalyticsOverviewPage() {
 ...
 const draft = workflows.filter((w: Workflow) => w.status ==="draft").length
 setAnalytics({
 ...
 const wonLostData = [
 { label:"Successful", value: analytics.successful_executions, color:"#10B981" },
 ...
 <p className="text-sm text-[var(--text-muted)]">Workflow execution, completion rates, and performance</p>
 </div>
 </div>
...
"px-3 py-1.5 text-xs font-medium transition",
 ...
 <p className="text-2xl font-bold text-[var(--status-success-text)]">{analytics.successful_executions}</p>
 </div>
 <div className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-primary)] p-4">
 <p className="text-xs text-[var(--text-muted)]">Failed Executions</p>
 <p className="text-2xl font-bold text-[var(--status-danger-text)]">{analytics.failed_executions}</p>
 </div>
 <div className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-primary)] p-4">
 ...
 <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
 <div className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-primary)] p-4">
 <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-4">Successful Executions Over Time</h3>
 <BarChart data={executionsData} height={250} />
 </div>

 <div className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-primary)] p-4">
 <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-4">Failed Executions Over Time</h3>
 ...
 <div className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-primary)] p-4">
 <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-4">Success/Failure Ratio</h3>
 ...
 {/* Top Workflows Table */}
 <div className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-primary)] p-4">
 <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-4">Workflow Performance</h3>
 <div className="rounded-lg border border-[var(--border-default)] overflow-hidden">
 <table className="w-full text-sm">
 <thead>
 <tr className="bg-[var(--bg-secondary)] border-b border-[var(--border-default)]">
 <th className="text-right px-3 py-2 text-xs font-medium text-[var(--text-muted)]">Workflow</th>
 <th className="text-right px-3 py-2 text-xs font-medium text-[var(--text-muted)]">Runs</th>
 <th className="text-right px-3 py-2 text-xs font-medium text-[var(--text-muted)]">Success Rate</th>
 <th className="text-right px-3 py-2 text-xs font-medium text-[var(--text-muted)]">Performance</th>
 </tr>
 </thead>
 <tbody>
 {analytics.top_workflows.map((wf) => (
 <tr key={wf.id} className="border-b border-[var(--border-default)] last:border-0 hover:bg-[var(--bg-secondary)]">
