...
export default function PipelineAnalyticsOverviewPage() {
 ...
 const wonLostData = [
 { label:"Won", value: Math.round(analytics.total_pipeline * (analytics.win_rate / 100)), color:"#10B981" },
 { label:"Lost", value: Math.round(analytics.total_pipeline * (1 - analytics.win_rate / 100)), color:"#EF4444" },
 ]
 ...
 <p className="text-sm text-[var(--text-muted)]">Conversion, velocity, and pipeline health</p>
 </div>
 </div>
...
"px-3 py-1.5 text-xs font-medium transition",
 ...
 <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-4">Conversion Funnel</h3>
 ...
 >
 <span className="text-[10px] font-medium text-white whitespace-nowrap">{item.count}</span>
 </div>
 </div>
 ...
 <span className="font-semibold text-[var(--muhide-orange)]">{analytics.conversion_rate_lead_to_close}%</span>
 </span>
 </div>
 )}
 </div>
 ...
 {/* Velocity Chart */}
 <div className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-primary)] p-4">
 <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-4">Velocity (Avg Days per Stage)</h3>
 ...
 <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
 <div className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-primary)] p-4">
 <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-4">Pipeline Value Over Time</h3>
 <LineChart
 series={[{ name:"Pipeline Value", color:"#F57C1E", data: valueOverTime.map((v) => v.value) }]}
 height={250}
 />
 </div>

 <div className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-primary)] p-4">
 <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-4">Win/Loss Distribution</h3>
 ...
 <div className="rounded-xl border border-[var(--border-default)] bg-[var(--bg-primary)] p-4">
 <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-4">Stage Duration Breakdown</h3>
 <div className="rounded-lg border border-[var(--border-default)] overflow-hidden">
 <table className="w-full text-sm">
 <thead>
 <tr className="bg-[var(--bg-secondary)] border-b border-[var(--border-default)]">
 <th className="text-right px-3 py-2 text-xs font-medium text-[var(--text-muted)]">Stage</th>
 <th className="text-right px-3 py-2 text-xs font-medium text-[var(--text-muted)]">Median (p50)</th>
 <th className="text-right px-3 py-2 text-xs font-medium text-[var(--text-muted)]">p95</th>
 <th className="text-right px-3 py-2 text-xs font-medium text-[var(--text-muted)]">Distribution</th>
 </tr>
 </thead>
 <tbody>
 {stageDuration.map((item) => (
 <tr key={item.stage} className="border-b border-[var(--border-default)] last:border-0 hover:bg-[var(--bg-secondary)]">
 ...
 />
 <span className="text-[var(--text-primary)]">{STAGE_LABELS[item.stage] || item.stage}</span>
 </span>
 </td>
