...
const STEP_TYPES: {
 ...
 { value:"send_email", label:"إرسال بريد", icon: <Mail className="h-4 w-4" />, color:"#3B82F6" },
 { value:"update_crm", label:"تحديث سجل", icon: <Database className="h-4 w-4" />, color:"#10B981" },
 { value:"create_task", label:"إنشاء مهمة", icon: <CheckSquare className="h-4 w-4" />, color:"#F59E0B" },
 { value:"webhook", label:"Webhook", icon: <Webhook className="h-4 w-4" />, color:"#8B5CF6" },
 { value:"nba_recommend", label:"توصية ذكية", icon: <Brain className="h-4 w-4" />, color:"#EC4899" },
]
...
function emptyStep(order: number, prevId: string | null): VisualStep {
 return {
 id: crypto.randomUUID?.() || `step-${Date.now()}-${order}`,
 type:"send_email",
 label:"خطوة جديدة",
 config: {},
 conditionExpression:"",
...
function StepConfigPanel({
 ...
 <div>
 <label className="mb-1 block text-xs font-medium text-[var(--text-muted)]">نوع الخطوة</label>
 <select
 value={step.type}
 ...
 <div>
 <label className="mb-1 block text-xs font-medium text-[var(--text-muted)]">الموضوع</label>
 <Input
 ...
 <div>
 <label className="mb-1 block text-xs font-medium text-[var(--text-muted)]">المحتوى</label>
 <Textarea
 ...
 <>
 <div>
 <label className="mb-1 block text-xs font-medium text-[var(--text-muted)]">الحقل</label>
 <Input
 ...
 <div>
 <label className="mb-1 block text-xs font-medium text-[var(--text-muted)]">القيمة الجديدة</label>
 <Input
 ...
 <>
 <div>
 <label className="mb-1 block text-xs font-medium text-[var(--text-muted)]">عنوان المهمة</label>
 <Input
 ...
 <div>
 <label className="mb-1 block text-xs font-medium text-[var(--text-muted)]">المسؤول</label>
 <Input
 value={(step.config.assignee as string) ||""}
 onChange={(e) => onUpdate({ config: { ...step.config, assignee: e.target.value } })}
 placeholder="{{owner}}"
 />
 </div>
 </div>
 ...
 <>
 <div>
 <label className="mb-1 block text-xs font-medium text-[var(--text-muted)]">الرابط</label>
 <Input
 ...
 <div>
 <label className="mb-1 block text-xs font-medium text-[var(--text-muted)]">الطريقة</label>
 <select
 ...
 <>
 <div>
 <label className="mb-1 block text-xs font-medium text-[var(--text-muted)]">نوع الإجراء</label>
 <Input
 value={(step.config.action_type as string) ||""}
 onChange={(e) => onUpdate({ config: { ...step.config, action_type: e.target.value } })}
 placeholder="follow_up_call"
 />
 </div>
 <div>
 <label className="mb-1 block text-xs font-medium text-[var(--text-muted)]">السبب</label>
 <Textarea
 value={(step.config.reason as string) ||""}
 onChange={(e) => onUpdate({ config: { ...step.config, reason: e.target.value } })}
...
"absolute rounded-xl border-2 bg-[var(--bg-primary)] p-3 shadow-sm cursor-pointer transition-all",
 ...
 <Badge variant="success" className="text-[9px] px-1.5 py-0">بداية</Badge>
 </div>
 )}
...
export default function NewWorkflowPage() {
 ...
 const newStep = emptyStep(steps.length, lastStep?.id || null)
 ...
 <div className="space-y-3">
 <h3 className="text-xs font-semibold uppercase tracking-wider text-[var(--text-muted)]">إعدادات سير العمل</h3>

 <div>
 <label className="mb-1 block text-xs font-medium text-[var(--text-muted)]">الاسم *</label>
 <Input
 value={workflowName}
 ...
 <div>
 <label className="mb-1 block text-xs font-medium text-[var(--text-muted)]">الوصف</label>
 <Textarea
 value={workflowDesc}
 ...
 <div>
 <label className="mb-1 block text-xs font-medium text-[var(--text-muted)]">نوع المشغل</label>
 <Select
 ...
 <div>
 <label className="mb-1 block text-xs font-medium text-[var(--text-muted)]">الحالة</label>
 <Select
 ...
 <h3 className="text-xs font-semibold uppercase tracking-wider text-[var(--text-muted)] mb-2">إضافة خطوة</h3>
 ...
 {steps.length > 0 && (
 <div className="border-t border-[var(--border-default)] pt-3">
 <h3 className="text-xs font-semibold uppercase tracking-wider text-[var(--text-muted)] mb-2">الخطوات ({steps.length})</h3>
 ...
 <button
 key={s.id}
 onClick={() => { setSelectedStepId(s.id); setShowConfig(true) }}
 className={cn(
"flex w-full items-center gap-2 rounded-lg px-2 py-1.5 text-right text-xs transition-colors",
 ...
 onSelectStep={(id) => { setSelectedStepId(id); setShowConfig(!!id) }}
 ...
 onClose={() => { setSelectedStepId(null); setShowConfig(false) }}
 />
 )}
 </div>
 </div>
