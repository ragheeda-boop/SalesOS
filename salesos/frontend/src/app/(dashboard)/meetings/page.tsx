...
export default function MeetingsPage() {
 ...
 className="flex items-center gap-2 w-full max-w-md px-4 py-2.5 border rounded-lg bg-[var(--bg-primary)] text-sm hover:border-[var(--muhide-orange)] transition"
 >
 {selectedOpp ? (
 ...
 {o.company_name && <span className="text-[var(--text-disabled)] ms-2">— {o.company_name}</span>}
 </button>
 ))}
 </div>
 )}
 ...
 {brief && (
 <div className="rounded-lg border p-4 bg-[var(--bg-primary)] space-y-4">
 ...
 <ul className="mt-1 space-y-1">
 {brief.risks.map((r, i) => (
 <li key={i} className="text-sm text-[var(--status-danger-text)]">• {r}</li>
 ))}
 </ul>
 </div>
 )}
 ...
 {meetings.map(m => (
 <div key={m.id} className="flex items-center justify-between p-3 rounded-lg bg-[var(--bg-secondary)]">
 <div>
 <p className="font-medium text-sm">{m.title}</p>
 <p className="text-xs text-[var(--text-muted)]">{m.date}{m.duration_minutes && ` · ${m.duration_minutes}min`}</p>
 </div>
 <span className={cn(
"px-2 py-0.5 rounded text-xs font-medium",
 m.status ==="completed" &&"bg-[var(--status-success-bg)] text-[var(--status-success-text)]",
 ...
 <div className="rounded-lg border p-4 bg-[var(--bg-primary)] h-fit">
 <h3 className="font-bold mb-3 flex items-center gap-2">
 <Users className="h-4 w-4" /> {t("meetings.attendees")}
 </h3>
 <div className="space-y-2">
 {brief.attendees.map((a, i) => (
 <div key={i} className="flex items-center justify-between p-2 rounded bg-[var(--bg-secondary)]">
 <div>
 ...
 {!selected && !loading && (
 <div className="flex flex-col items-center justify-center min-h-[300px] text-[var(--text-muted)]">
 <Calendar className="h-12 w-12 mb-4 text-[var(--text-disabled)]" />
