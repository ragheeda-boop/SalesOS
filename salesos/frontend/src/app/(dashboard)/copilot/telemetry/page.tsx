...
import {
 ArrowLeft, Bot, Clock, TrendingUp, CheckCircle,
 BarChart3,
} from"lucide-react"
...
"rounded-md px-3 py-1.5 text-xs font-medium transition-colors",
 ...
 trend={data?.summary?.success_rate
 ...
 <LineChart
 series={[{ name:"Calls", color:"#F57C1E", data: volumeChartData.map((v) => v.value) }]}
 height={220}
 />
 ) : (
 <div className="flex h-[220px] items-center justify-center text-sm text-[var(--text-muted)]">
 ...
 <th className="text-start px-3 py-2 text-xs font-medium text-[var(--text-muted)]">{t("copilot.telemetry_tool_name")}</th>
 <th className="text-end px-3 py-2 text-xs font-medium text-[var(--text-muted)]">{t("copilot.telemetry_calls")}</th>
 <th className="text-end px-3 py-2 text-xs font-medium text-[var(--text-muted)]">{t("copilot.telemetry_success")}</th>
 <th className="text-end px-3 py-2 text-xs font-medium text-[var(--text-muted)]">{t("copilot.telemetry_failure")}</th>
 <th className="text-end px-3 py-2 text-xs font-medium text-[var(--text-muted)]">Rate</th>
 <th className="text-end px-3 py-2 text-xs font-medium text-[var(--text-muted)]">{t("copilot.telemetry_p50")}</th>
 <th className="text-end px-3 py-2 text-xs font-medium text-[var(--text-muted)]">{t("copilot.telemetry_p95")}</th>
 <th className="text-end px-3 py-2 text-xs font-medium text-[var(--text-muted)]">{t("copilot.telemetry_p99")}</th>
 </tr>
 </thead>
 <tbody>
 {data.tools.map((tool) => (
 ...
 >
 <td className="px-3 py-2 text-[var(--text-primary)] font-medium">{tool.tool_name}</td>
 <td className="px-3 py-2 text-[var(--text-secondary)] text-end">{tool.total_calls}</td>
 <td className="px-3 py-2 text-[var(--status-success-text)] text-end">{tool.success_count}</td>
 <td className="px-3 py-2 text-[var(--status-danger-text)] text-end">{tool.failure_count}</td>
...
"rounded-full px-2 py-0.5 text-[10px] font-medium",
 ...
 <td className="px-3 py-2 text-[var(--text-secondary)] text-end font-mono">{tool.latency_p50_ms.toFixed(0)}ms</td>
 <td className="px-3 py-2 text-[var(--text-secondary)] text-end font-mono">{tool.latency_p95_ms.toFixed(0)}ms</td>
 <td className="px-3 py-2 text-[var(--text-secondary)] text-end font-mono">{tool.latency_p99_ms.toFixed(0)}ms</td>
 </tr>
 ))}
 </tbody>
 </table>
...
function LatencyPercentileTable({ data }: { data: LatencyBucket[] }) {
 ...
 <thead>
 <tr className="bg-[var(--bg-secondary)] border-b border-[var(--border-default)]">
 <th className="text-start px-3 py-2 text-xs font-medium text-[var(--text-muted)]">Tool</th>
 <th className="text-end px-3 py-2 text-xs font-medium text-[var(--text-muted)]">{t("copilot.telemetry_p50")}</th>
 <th className="text-end px-3 py-2 text-xs font-medium text-[var(--text-muted)]">{t("copilot.telemetry_p95")}</th>
 <th className="text-end px-3 py-2 text-xs font-medium text-[var(--text-muted)]">{t("copilot.telemetry_p99")}</th>
 </tr>
 ...
 <tbody>
 {data.map((item) => (
 <tr key={item.label} className="border-b border-[var(--border-default)] last:border-0 hover:bg-[var(--bg-secondary)]">
 <td className="px-3 py-2 text-[var(--text-primary)]">{item.label}</td>
 <td className="px-3 py-2 text-[var(--text-secondary)] text-end font-mono">{item.p50.toFixed(1)}ms</td>
 <td className="px-3 py-2 text-[var(--text-secondary)] text-end font-mono">{item.p95.toFixed(1)}ms</td>
 <td className="px-3 py-2 text-[var(--text-secondary)] text-end font-mono">{item.p99.toFixed(1)}ms</td>
 </tr>
 ))}
 </tbody>
 </table>
