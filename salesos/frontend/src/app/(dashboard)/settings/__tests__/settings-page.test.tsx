...
jest.mock('@salesos/ui', () => ({
  ...
  TabsPanel: ({ children }: { children: React.ReactNode }) => <div data-testid="tabs-panel">{children}</div>,
  Input: ({ label, placeholder, ...props }: Record<string, unknown>) => (
    <div>
      {label && <label>{String(label)}</label>}
      <input data-testid={`input-${label || placeholder}`} placeholder={String(placeholder || '')} {...(props as React.InputHTMLAttributes<HTMLInputElement>)} />
    </div>
  ),
  Button: ({ children, ...props }: { children: React.ReactNode; [key: string]: unknown }) => <button {...(props as React.ButtonHTMLAttributes<HTMLButtonElement>)}>{children}</button>,
...
describe('SettingsPage', () => {
  ...
  it('shows inactive badge when profile is inactive', () => {
    mockUseQuery.mockReturnValue({ data: { ...mockProfile, is_active: false }, isLoading: false })
    mockUseMutation.mockReturnValue({ mutate: jest.fn(), isPending: false })
    render(<SettingsPage />)
