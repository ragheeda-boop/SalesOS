...
import { Tabs, TabsList, Tab, TabsPanel, Input, Button, Badge, Card, cn, useToast, Spinner } from"@salesos/ui";
import api, { getCurrentUser, changePassword, getNotificationPreferences, updateNotificationPreferences, getApiKeys, createApiKey, deleteApiKey, type NotificationPreferences } from"@/lib/api";
...
export default function SettingsPage() {
 ...
 onError: () => {
 toast({ variant:"error", title: t("settings.save_failed"), description: t("settings.save_error") });
 },
 });
 ...
 toast({ variant:"error", title: t("settings.password_update_failed"), description: msg });
 },
 });
 ...
 <p className="text-xs text-[var(--text-muted)]">{t(ROLE_KEYS[profile.role] ||"") || profile.role}</p>
 </div>
 <Badge className="mr-auto">{profile.is_active ? t("settings.active") : t("settings.inactive")}</Badge>
 </div>
 )}
 ...
 <span>{t("settings.role")}: {t(ROLE_KEYS[profile?.role ||""] ||"") || profile?.role}</span>
 <span>|</span>
 <span>{t("settings.member_since")}: {profile?.created_at ? new Date(profile.created_at).toLocaleDateString("ar-SA") :"\u2014"}</span>
 </div>
 <Button
 ...
 <Card className="p-6">
 <h2 className="mb-4 text-lg font-semibold text-[var(--text-primary)]">{t("settings.security_title")}</h2>
 <div className="space-y-4">
 <Input
 ...
 error={passwordForm.new_password && passwordForm.new_password.length < 12 ? t("settings.password_min_length") : undefined}
 />
 <Input
 ...
 <Card className="p-6">
 <h2 className="mb-4 text-lg font-semibold text-[var(--text-primary)]">{t("settings.notifications_title")}</h2>
 {notifLoading ? (
 ...
 {[
 { key:"email_notifications", labelKey:"settings.notif.email_notifications" },
 { key:"app_notifications", labelKey:"settings.notif.app_notifications" },
 { key:"opportunity_alerts", labelKey:"settings.notif.opportunity_alerts" },
 { key:"company_updates", labelKey:"settings.notif.company_updates" },
 { key:"weekly_summary", labelKey:"settings.notif.weekly_summary" },
 ].map((item) => (
 <label
 key={item.key}
...
"relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors",
 notifPrefs[item.key] ?"bg-[var(--muhide-orange)]" :"bg-[var(--bg-tertiary)]"
 )}
 role="switch"
...
"pointer-events-none inline-block h-5 w-5 rounded-full bg-[var(--bg-primary)] shadow transition-transform",
 ...
 <TabsPanel value="api">
 <Card className="p-6">
 <h2 className="mb-2 text-lg font-semibold text-[var(--text-primary)]">{t("settings.api_title")}</h2>
 ...
 <p className="py-4 text-center text-sm text-[var(--text-muted)]">{t("settings.no_api_keys")}</p>
 )}
 </div>
 ...
 className="flex-1"
 />
 <Button onClick={() => createKeyMutation.mutate(newKeyName.trim())} disabled={!newKeyName.trim() || createKeyMutation.isPending}>
 ...
 {activeTab ==="data" && (
 <Card className="p-6">
 <h2 className="mb-4 text-lg font-semibold text-[var(--text-primary)]">{t("settings.data_title")}</h2>
 <div className="space-y-4">
 <div>
 <label className="block text-sm font-medium text-[var(--text-secondary)]">{t("settings.data_sources")}</label>
