...
import {
 ...
 Breadcrumbs, Skeleton, EmptyState, DataTable,
 Card, CardContent, CardHeader, cn
} from"@salesos/ui"
import {
 BarChart3, Share2, DollarSign, Activity, Sparkles, Building2, MapPin,
 FileText, TrendingUp, Users, ChevronRight, AlertTriangle, CheckCircle,
 Clock, Hash, ArrowRight, Loader2
} from"lucide-react"
...
function HealthScoreRing({ score }: { score: number }) {
 ...
 <circle cx="42" cy="42" r={radius} fill="none" stroke="currentColor" className="text-[var(--text-primary)]" strokeWidth="6" />
 <circle cx="42" cy="42" r={radius} fill="none" stroke={color} strokeWidth="6" strokeDasharray={circumference} strokeDashoffset={offset} strokeLinecap="round" transform="rotate(-90 42 42)" className="transition-all duration-700" />
 </svg>
...
function MetricBox({ label, value, icon: Icon, color }: { label: string; value: string | number; icon: typeof BarChart3; color: string }) {
 return (
...
export default function Company360Page() {
 ...
 { label: company?.name_ar || company?.name_en ||"...", href: `/companies/${id}` },
 ...
 action={{ label: t("companies.back_to_list"), onClick: () => router.push("/companies") }}
 />
 </div>
 )
 }
 ...
 const contacts = company360?.contacts || []
 const opportunities = company360?.opportunities || []
 const contracts = company360?.contracts || []
 const invoices = company360?.invoices || []

 const enrichmentData = [
 { label:"إجمالي الإيرادات", value: overview?.total_revenue ? `$${(overview.total_revenue / 1000).toFixed(0)}K` :"-" },
 ...
 { label:"آخر تحديث", value: enrichment?.last_enriched_at ? new Date(enrichment.last_enriched_at).toLocaleDateString("ar-SA") :"-" },
 ]
 ...
 title:"الفروع",
 icon: Building2,
 items: organization.branches?.map((b: { id: string; name: string; city: string | null; region: string | null }) => ({
 id: b.id, label: b.name, subtitle: [b.city, b.region].filter(Boolean).join(","),
 })) || [],
 },
 {
 title:"الأقسام",
 icon: Users,
 items: organization.departments?.map((d: string, i: number) => ({ id: String(i), label: d })) || [],
 },
 ] : []

 const financialColumns: ColumnDef<Record<string, unknown>>[] = [
 { accessorKey:"source", header:"المصدر", cell: ({ getValue }) => String(getValue() ||"-") },
 { accessorKey:"type", header:"النوع", cell: ({ getValue }) => String(getValue() ||"-") },
 { accessorKey:"value", header:"القيمة", cell: ({ getValue }) => {
 const v = getValue()
 return typeof v ==="number" ? `$${v.toLocaleString()}` : String(v ||"-")
 }},
 { accessorKey:"status", header:"الحالة", cell: ({ getValue }) => String(getValue() ||"-") },
 { accessorKey:"date", header:"التاريخ", cell: ({ getValue }) => {
 const v = getValue()
 ...
 const financialData = [
 ...(contracts?.map((c: Record<string, unknown>) => ({ source:"عقد", ...c })) || []),
 ...(invoices?.map((i: Record<string, unknown>) => ({ source:"فاتورة", ...i })) || []),
 ...(opportunities?.map((o: Record<string, unknown>) => ({ source:"فرصة", ...o })) || []),
 ]
 ...
 <div className="flex flex-wrap items-center gap-4 px-6 py-4">
          <Link href={`/companies/${id}`} className="flex h-14 w-14 items-center justify-center rounded-xl bg-[var(--muhide-orange)]/10 text-[var(--muhide-orange)] hover:bg-[var(--muhide-orange)]/20 transition-colors">
 ...
 {company.name_ar || company.name_en}
                ...
                <span className="rounded bg-purple-100 px-1.5 py-0.5 text-[10px] font-medium text-purple-700 dark:bg-purple-900/30 dark:text-purple-400">360</span>
 </div>
...
"inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs font-medium",
 ...
 <MetricBox label="الإيرادات" value={`$${(overview.total_revenue / 1000).toFixed(0)}K`} icon={TrendingUp} color="bg-success-50 text-success-700 dark:bg-success-900/20 dark:text-success-400" />
 )}
 {overview?.active_contracts !== undefined && (
 <MetricBox label="العقود النشطة" value={overview.active_contracts} icon={FileText} color="bg-info-50 text-info-700 dark:bg-info-900/20 dark:text-info-400" />
 )}
 {overview?.total_contacts !== undefined && (
 <MetricBox label="جهات الاتصال" value={overview.total_contacts} icon={Users} color="bg-purple-50 text-purple-700 dark:bg-purple-900/20 dark:text-purple-400" />
 )}
 {overview?.total_opportunities !== undefined && (
 <MetricBox label="الفرص" value={overview.total_opportunities} icon={BarChart3} color="bg-warning-50 text-warning-700 dark:bg-warning-900/20 dark:text-warning-400" />
 )}
 </div>
 <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
 ...
 <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
 {hierarchySections.length > 0 ? hierarchySections.map((section) => (
 ...
 <EmptyState icon={<section.icon className="h-8 w-8" />} title={`لا توجد ${section.title}`} description={`لم يتم العثور على ${section.title}`} />
 ) : (
 <div className="space-y-1">
 {section.items.map((item: { id: string; label: string; subtitle?: string }) => (
 <div key={item.id} className="flex items-center gap-3 rounded-lg px-3 py-2 hover:bg-[var(--bg-secondary)] dark:hover:bg-[var(--bg-secondary)]/50">
 ...
 {item.subtitle && <p className="text-xs text-[var(--text-muted)]">{item.subtitle}</p>}
 </div>
 </div>
 ))}
 </div>
 ...
 <Card>
 <CardContent>
 <EmptyState icon={<Share2 className="h-10 w-10" />} title="لا تبيانات هرمية" description="لا توجد معلومات هرمية أو فروع لهذه الشركة" />
 </CardContent>
 </Card>
 </div>
 )}
 ...
 <span className="text-sm font-semibold text-[var(--text-primary)]">البيانات المالية والإثراء</span>
 </div>
 </CardHeader>
 <CardContent>
 <div className="mb-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
 {enrichmentData.map((d) => (
 <div key={d.label} className="rounded-lg bg-[var(--bg-secondary)] p-3/50">
 <p className="text-[10px] text-[var(--text-muted)]">{d.label}</p>
 <p className="mt-0.5 text-sm font-semibold text-[var(--text-primary)]">{String(d.value)}</p>
 </div>
 ))}
 </div>
 <h4 className="mb-2 text-xs font-semibold text-[var(--text-secondary)]">العقود والفواتير والفرص</h4>
 <DataTable
