...
interface AggregatedMetrics {
  ...
  errors: {
    ...
    recent: { message: string; time: string }[];
  };
  page_loads: {
    total: number;
    avg_load_ms: number;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          icon={Activity}
          label={t("monitoring.api_requests")}
          value={metrics?.api_calls?.total || 0}
          color="blue"
        />
        <StatCard
          icon={AlertTriangle}
        ...
        <StatCard
          icon={Clock}
          label={t("monitoring.page_load")}
          value={`${metrics?.page_loads?.avg_load_ms || 0}ms`}
          color="green"
        />
        <StatCard
          icon={MemoryStick}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card className="p-4">
          ...
          <div className="space-y-3">
            <LatencyBar
              label="p50"
              value={metrics?.api_calls?.p50_ms || 0}
              max={1000}
              color="bg-success-500"
            />
            <LatencyBar
              label="p95"
              value={metrics?.api_calls?.p95_ms || 0}
              max={1000}
              color="bg-warning-500"
            />
            <LatencyBar
              label="p99"
              value={metrics?.api_calls?.p99_ms || 0}
              max={1000}
              color="bg-danger-500"
            />
          </div>
        ...
        <Card className="p-4">
          ...
          <div className="space-y-3">
            <LatencyBar
              ...
              value={metrics?.page_loads?.avg_dom_interactive_ms || 0}
              max={5000}
              color="bg-info-500"
            />
            <LatencyBar
              label="LCP"
              value={metrics?.web_vitals?.lcp || 0}
              max={4000}
              color="bg-warning-500"
            />
            <LatencyBar
              label="FID"
              value={metrics?.web_vitals?.fid || 0}
              max={300}
              color="bg-info-500"
            />
            <LatencyBar
              label="CLS"
              value={(metrics?.web_vitals?.cls || 0) * 100}
              max={25}
              color="bg-[var(--chart-purple)]"
              unit="%"
            />
          </div>
        </Card>
        ...
        <Card className="p-4">
          ...
          <div className="space-y-2">
            {metrics.errors.recent.slice(0, 10).map((err, i) => (
              <div
                key={i}
                className="flex items-start gap-2 text-sm p-2 bg-danger-50 dark:bg-danger-900/20 rounded-lg"
              >
...
function LatencyBar({
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="w-28 text-sm text-[var(--text-secondary)]">{label}</span>
      ...
      <span className="text-sm font-mono w-20 text-left">
        {value}
        {unit}
      </span>
    </div>
