...
export default function MonitoringPage() {
 ...
 <button onClick={handleRefresh} className="rounded-lg p-1.5 hover:bg-[var(--bg-tertiary)] dark:hover:bg-[var(--bg-secondary)]">
 ...
 <LatencyBar label="CLS" value={(metrics?.web_vitals?.cls || 0) * 100} max={25} color="bg-[var(--chart-purple)]" unit="%" />
 </div>
 </Card>
 </div>
 ...
 <p className="text-xs text-[var(--text-muted)]">{new Date(err.time).toLocaleString('ar-SA')}</p>
 </div>
 </div>
 ))}
 </div>
...
function StatCard({ icon: Icon, label, value, color }: { icon: React.ComponentType<{ className?: string }>; label: string; value: string | number; color: string }) {
 ...
 purple:"bg-[var(--chart-purple-bg)] text-[var(--chart-purple)] dark:bg-[var(--bg-primary)]/30 dark:text-[var(--chart-purple)]",
 }
 return (
 <Card className="p-4">
