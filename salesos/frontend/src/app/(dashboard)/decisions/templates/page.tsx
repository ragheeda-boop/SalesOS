...
import {
 DataTable, Badge, Button, Modal, ModalContent, ModalHeader, ModalBody, ModalFooter,
 EmptyState, Input, Textarea, Select, useToast
} from"@salesos/ui"
import {
 ArrowLeft, Plus, Settings, Trash2, Edit3,
...
export default function DecisionTemplatesPage() {
 ...
 onSuccess: () => {
 queryClient.invalidateQueries({ queryKey: [...decisionKeys.all,"templates"] })
 ...
 <ModalFooter>
 <Button variant="outline" onClick={() => { setModalOpen(false); setEditTemplate(null); resetForm() }}>
 {t("common.cancel")}
 </Button>
 <Button
 onClick={handleSubmit}
 disabled={!formData.name.trim() || createTemplate.isPending || updateTemplate.isPending}
 leftIcon={(createTemplate.isPending || updateTemplate.isPending) ? <RefreshCw className="h-4 w-4 animate-spin" /> : undefined}
 >
 {(createTemplate.isPending || updateTemplate.isPending)
 ? t("common.saving")
 : editTemplate ? t("common.save") : t("common.create")}
 </Button>
 </ModalFooter>
 </ModalContent>
 </Modal>
 ...
 {t("decisions.delete_template_message", { name: deleteConfirm?.name ||"" })}
 </p>
 ...
 leftIcon={deleteTemplate.isPending ? <RefreshCw className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
 >
