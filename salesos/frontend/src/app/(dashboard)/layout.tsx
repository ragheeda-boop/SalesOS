...
function DashboardContent({ children }: { children: ReactNode }) {
 ...
 <kbd className="ms-auto rounded border border-[var(--border-hover)] px-1.5 py-0.5 text-[10px]">⌘K</kbd>
 </button>
 {aiCopilotEnabled && (
 <button
...
"flex items-center gap-3 rounded-lg px-3 py-2 text-sm transition",
 ...
 <main id="main-content" tabIndex={-1} className="min-w-0 flex-1 overflow-auto p-3 sm:p-4 lg:p-6">{children}</main>
 </div>
 <MobileNav />
 <LazyCommandBar open={commandOpen} onClose={() => setCommandOpen(false)} />
 <LazySearchPanel open={searchOpen} onClose={() => setSearchOpen(false)} />
 {aiCopilotEnabled && (
 <LazyCopilotPanel open={copilotOpen} onClose={() => setCopilotOpen(false)} entityType="company" />
 )}
 </div>
 )
}
