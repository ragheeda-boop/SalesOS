...
function PreviewPanel({
 ...
 <PreviewBadge />
 <span className="text-[12px] text-[var(--text-muted)]">Not wired — no invented controls</span>
 </div>
 <p>{children}</p>
...
function UsersPanel({
 ...
 {user.full_name || '—'}
 {user.full_name_ar ? (
 <p className="mt-0.5 text-[12px] font-normal text-[var(--text-muted)]" dir="auto">
 {user.full_name_ar}
 </p>
 ) : null}
 </td>
...
function FlagsPanel({ ready, hasToken }: { ready: boolean; hasToken: boolean }) {
 ...
 (<code className="mx-1 font-mono text-[12px]">AI_HONESTY.md</code>). Ask AI remains popup-only.
 </p>
 {aiFlags.length > 0 ? (
 <p className="text-[12px] text-[var(--text-muted)]">
