...
const REPORT_DIR =
  process.env.CRAWL_REPORT_DIR ||
  path.join(process.cwd(), 'test-results', 'full-ui-crawl')
...
async function clickVisibleControls(page: Page, originPath: string): Promise<{
  clicks: ClickResult[]
  skipped: string[]
}> {
  ...
  const buttons = page.locator('main button:visible, [role="main"] button:visible, main a[role="button"]:visible')
  ...
  for (let i = 0; i < btnCount && remaining > 0; i++) {
    ...
    try {
      ...
      if (await dialog.isVisible().catch(() => false)) {
        const close = dialog.getByRole('button', { name: /close|cancel|إلغاء|إغلاق|dismiss/i }).first()
