...
test.describe("Critical Path 14: Error States & Empty States", () => {
  ...
  test("dashboard shows widgets or empty state after login", async ({ page }) => {
    ...
    await expect(
      page
        .locator('[class*="widget"], [class*="empty"], [class*="placeholder"], [class*="skeleton"]')
        .first()
  ...
  test("search page handles empty query gracefully", async ({ page }) => {
    ...
    await expect(page.locator('main, [role="main"], article, div').first()).toBeVisible({
      timeout: 5_000,
    });
  });
  ...
  test("company detail shows content or empty state", async ({ page }) => {
    ...
    await page.waitForLoadState("networkidle");
    await expect(page.locator('main, [role="main"], article, table, div').first()).toBeVisible({
      timeout: 8_000,
    });
  });
