...
test.describe('Feature: Copilot Page', () => {
  ...
  test('page renders with title and AI icon', async ({ page }) => {
    ...
    await expect(page.locator('h1').or(page.locator('text=/مساعد|Assistant|Copilot/i').first())).toBeVisible()
  })

  test('chat input area is accessible', async ({ page }) => {
    await page.goto('/copilot')
    await page.waitForLoadState('networkidle')
    const input = page.getByRole('textbox').or(page.locator('textarea')).or(page.locator('input[type="text"]'))
  ...
  test('history sidebar toggles', async ({ page }) => {
    ...
    const sidebar = page.locator('div').filter({ hasText: /recent|محادثات|Conversations/i }).first()
