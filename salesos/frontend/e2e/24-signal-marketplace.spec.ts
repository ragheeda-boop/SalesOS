...
const mockSignals = [
  {
    ...
    ar_name: "صانع قرار جديد",
    description: "Detects new exec",
    domain: "company",
    category: "enrichment",
    severity: "info",
    source: "linkedin",
  ...
  },
  {
    ...
    ar_name: "زيادة الميزانية",
    description: "Budget change detected",
    domain: "opportunity",
...
const mockFeed = [
  {
    ...
    company_id: "comp-1",
    data: { role: "CEO" },
    detected_at: "2026-07-10T08:00:00Z",
    acknowledged: false,
  },
  {
    id: "evt-2",
...
test.describe("Feature: Signal Marketplace", () => {
  ...
  test("three tabs render on signals page", async ({ page }) => {
    ...
    const tabButtons = page
      .locator("button")
      .filter({ hasText: /marketplace|feed|subscriptions|Marketplace|Feed/i });
  ...
  test("marketplace tab shows signal list", async ({ page }) => {
    ...
    await page.waitForLoadState("networkidle");
    await expect(
      page.getByText("New Decision Maker").or(page.getByText("Budget Increase"))
  ...
  test("subscribe button triggers subscription", async ({ page }) => {
    ...
    await page.route("**/api/v1/signals/subscribe**", (route) =>
      route.fulfill({
        status: 200,
        contentType: "application/json",
  ...
  test("feed tab shows signal events", async ({ page }) => {
    await page.route("**/api/v1/signals/feed**", (route) =>
      route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({ events: mockFeed }),
      })
    );
    await page.goto("/signals");
    await page.waitForLoadState("networkidle");
  ...
  test("acknowledge button works on feed events", async ({ page }) => {
    ...
    await page.route("**/api/v1/signals/*/acknowledge**", (route) =>
      route.fulfill({
        status: 200,
        contentType: "application/json",
        body: JSON.stringify({ success: true }),
