...
from app.modules.revenue_execution.service import RevenueService, STAGE_WEIGHTS
...
class TestCreateOpportunity:
    ...
    async def test_create_opportunity_basic(self, service, mock_db):
        _setup_execute_with_row(mock_db, _mock_row(
            id="1", title="Deal A", stage="identified",
            estimated_value=50000, confidence=0.7,
            source="nba", risk_level="medium",
        ))
    ...
    async def test_create_opportunity_high_confidence_low_risk(self, service, mock_db):
        _setup_execute_with_row(mock_db, _mock_row(
            id="2", title="High Deal", stage="identified",
            estimated_value=100000, confidence=0.95,
            source="nba", risk_level="low",
        ))
        result = await service.create_opportunity(
            tenant_id=str(uuid.uuid4()),
            company_id=str(uuid.uuid4()),
    ...
    async def test_create_opportunity_low_confidence_high_risk(self, service, mock_db):
        _setup_execute_with_row(mock_db, _mock_row(
            id="3", title="Risky Deal", stage="identified",
            estimated_value=10000, confidence=0.2,
            source="nba", risk_level="high",
        ))
        result = await service.create_opportunity(
            tenant_id=str(uuid.uuid4()),
            company_id=str(uuid.uuid4()),
...
class TestUpdateStage:
    ...
    async def test_update_stage_success(self, service, mock_db):
        _setup_execute_with_row(mock_db, _mock_row(
            id="1", title="Deal", stage="qualifying",
            estimated_value=50000, confidence=0.7,
        ))
...
class TestCreateTask:
    ...
    async def test_create_task_basic(self, service, mock_db):
        _setup_execute_with_row(mock_db, _mock_row(
            id="1", title="Follow up", priority="medium",
            source="manual", completed=False,
        ))
    ...
    async def test_create_task_with_priority(self, service, mock_db):
        _setup_execute_with_row(mock_db, _mock_row(
            id="2", title="Urgent Task", priority="high",
            source="nba", completed=False,
        ))
        result = await service.create_task(
            tenant_id=str(uuid.uuid4()),
            title="Urgent Task",
    ...
    async def test_create_task_with_due_date(self, service, mock_db):
        _setup_execute_with_row(mock_db, _mock_row(
            id="3", title="Dated Task", priority="low",
            source="manual", completed=False,
        ))
        result = await service.create_task(
            tenant_id=str(uuid.uuid4()),
