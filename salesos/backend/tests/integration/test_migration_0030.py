...
class TestConfidenceScoreIndex:
    ...
    async def test_sort_by_confidence_desc(
        self, db_session: AsyncSession, ensure_index, test_tenant: str
    ):
        ...
        r = await db_session.execute(
            text(
                "SELECT confidence_score FROM companies WHERE tenant_id=:tid ORDER BY confidence_score DESC"  # noqa: E501
            ).bindparams(tid=tid)
        )
    ...
    async def test_filter_by_confidence_range(
        ...
        r = await db_session.execute(
            text(
                "SELECT confidence_score FROM companies WHERE tenant_id=:tid AND confidence_score >= 0.5 ORDER BY confidence_score DESC"  # noqa: E501
            ).bindparams(tid=tid)
        )
        fetched = [row[0] for row in r.fetchall()]
    ...
    async def test_index_used_in_explain(
        self, db_session: AsyncSession, ensure_index, test_tenant: str
    ):
        tid = uuid.UUID(test_tenant)
        r = await db_session.execute(
            text(
                "EXPLAIN SELECT confidence_score FROM companies WHERE tenant_id=:tid ORDER BY confidence_score DESC"  # noqa: E501
            ),
            {"tid": str(tid)},
        )
