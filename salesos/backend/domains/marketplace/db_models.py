...
class PluginModel(Base):
    ...
    # DB columns nullable with defaults (DEC-130d — ORM align; no SET NOT NULL)
    description: Mapped[str | None] = mapped_column(Text, nullable=True, default="")
    author: Mapped[str | None] = mapped_column(String(128), nullable=True, default="")
    license: Mapped[str | None] = mapped_column(String(32), nullable=True, default="MIT")
...
class PluginLifecycleEventModel(Base):
    ...
    # SQLAlchemy Declarative reserves attribute name "metadata" — column stays "metadata".
