...
# Static SQL only — never interpolate user input into statement text.
...
""")
_SQL_EMAIL_COUNT_EMPLOYEE_DIRECTION = sa_text("""
    SELECT COUNT(*) AS c FROM employee_email_events
    WHERE tenant_id = :tid AND employee_id = :eid AND direction = :direction
""")
...
class PostgresEmailReader(EmailReader):
    ...
    async def count_by_company(
        ...
        if direction:
            ...
            sql = _SQL_EMAIL_COUNT_DIRECTION
        else:
            sql = _SQL_EMAIL_COUNT
        row = (await self.db.execute(sql, params)).mappings().one()
    ...
    async def count_by_employee(
        ...
        if direction:
            params["direction"] = direction
            sql = _SQL_EMAIL_COUNT_EMPLOYEE_DIRECTION
        else:
            sql = _SQL_EMAIL_COUNT_EMPLOYEE
        row = (await self.db.execute(sql, params)).mappings().one()
        return int(row["c"] or 0)
