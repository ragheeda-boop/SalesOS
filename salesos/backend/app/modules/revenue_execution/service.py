...
class RevenueService:
    ...
    async def create_opportunity(
        ...
        result = await self.db.execute(
            text("""
                INSERT INTO opportunities (tenant_id, company_id, title, estimated_value,
                    confidence, win_probability, source, source_action_id, buying_intent,
                    relationship_strength, risk_level, stage)
                VALUES (:tenant_id, :company_id, :title, :value, :confidence, 0.10, 'nba',
                    :action_id, :buying_intent, :relationship, :risk, 'identified')
                RETURNING id, title, stage, estimated_value, confidence, source,
                    risk_level, created_at, last_activity_at
            """),
            {
                "tenant_id": tenant_id,
    ...
    async def update_stage(self, opportunity_id: str, stage: str, tenant_id: str):
        result = await self.db.execute(
            text(
                "UPDATE opportunities SET stage = :stage, stage_changed_at = NOW(), last_activity_at = NOW() WHERE id = :id AND tenant_id = :tenant_id RETURNING id, title, stage, estimated_value, confidence"  # noqa: E501
            ),
    ...
    async def complete_task(self, task_id: str, tenant_id: str):
        result = await self.db.execute(
            text(
                "UPDATE tasks SET completed = true WHERE id = :id AND tenant_id = :tenant_id RETURNING id, title, completed"  # noqa: E501
            ),
    ...
    async def get_pipeline(self, tenant_id: str):
        result = await self.db.execute(
            text("""
                ...
                GROUP BY stage ORDER BY array_position(
                    ARRAY['identified','qualifying','developing','proposing','negotiating','closing'],
                    stage)
            """),
            {"tenant_id": tenant_id},
        )
    ...
    async def list_opportunities(
        ...
        result = await self.db.execute(
            text(
                f"SELECT id, company_id, title, stage, estimated_value, confidence, win_probability, source, risk_level, created_at, last_activity_at FROM opportunities {where} ORDER BY created_at DESC LIMIT :limit OFFSET :offset"  # noqa: E501
            ),
            {**params, "limit": limit, "offset": offset},
        )
        rows = [dict(r._mapping) for r in result.fetchall()]
        count_result = await self.db.execute(
            text(f"SELECT COUNT(*) FROM opportunities {where}"), params
        )
