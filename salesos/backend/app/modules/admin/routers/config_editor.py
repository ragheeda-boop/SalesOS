...
async def get_config(tenant_id: str, key: str, db: AsyncSession = Depends(get_db_session)):
    ...
    return TenantConfigResponse(
        id=config.id, tenant_id=config.tenant_id, key=config.key,
        yaml_content=config.yaml_content, version=config.version,
...
async def save_config(tenant_id: str, body: TenantConfigCreate, db: AsyncSession = Depends(get_db_session)):
    svc = ConfigEditorService(db)
...
async def list_config_versions(tenant_id: str, key: str, db: AsyncSession = Depends(get_db_session)):
    svc = ConfigEditorService(db)
