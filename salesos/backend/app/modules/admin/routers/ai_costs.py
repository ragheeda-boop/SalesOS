...
async def list_ai_costs(
    ...
    records, total = await repos.ai.list(model=model, tenant_id=tenant_id, days=days, page=1, page_size=page_size + 1)
    ...
    items = [AICostResponse(
        id=r.id, model=r.model, tenant_id=r.tenant_id,
        tenant_name=await _resolve_tenant_name(db, str(r.tenant_id)) if r.tenant_id else "System",
