...
class InMemoryPlanRepository:
    ...
    async def list(self) -> builtins.list[Plan]:
...
class InMemoryLicenseRepository:
    ...
    async def find_by_tenant(self, tenant_id: str | uuid.UUID) -> builtins.list[License]:
...
class InMemoryInvoiceRepository:
    ...
    async def list_invoices(self, tenant_id: str | None = None) -> builtins.list[Invoice]:
    ...
    async def list_transactions(self, tenant_id: str | None = None) -> builtins.list[Transaction]:
    ...
    def _seed(self):
        flags = [
            ...
            FeatureFlag(
                ...
                key="ai_copilot",
                name="AI Copilot",
                description="AI copilot — disabled by default until runtime is production-validated",  # noqa: E501
                enabled=False,
            ),
            FeatureFlag(
                id=uuid.uuid4(),
                ...
                description="Enable semantic and hybrid search",
                enabled=True,
            ),
            FeatureFlag(
                id=uuid.uuid4(),
                key="crm_kanban",
                name="CRM Kanban Board",
                description="Enable Kanban view for CRM pipeline",
                enabled=False,
            ),
            FeatureFlag(
                id=uuid.uuid4(),
    ...
    def _seed(self):
        ...
        jobs = [
            Job(
                ...
                progress=100,
                tenant_id="tenant-1",
                payload={"source": "notion", "records": 1500},
            ...
            Job(
                ...
                progress=42,
                tenant_id="tenant-1",
                payload={"contacts": 500},
                error_message="Rate limit exceeded on provider API",
                retry_count=2,
                max_retries=3,
            ...
            Job(
                ...
                progress=0,
                tenant_id="tenant-3",
                payload={"report_type": "monthly", "period": "2026-06"},
            ...
            Job(
                ...
                completed_at=now - timedelta(days=1, hours=-23),
        ...
        for j in jobs:
            j.logs = [
                {
                    "level": "INFO",
                    "message": f"Job {j.id} initialized",
                    "timestamp": (now - timedelta(hours=2)).isoformat(),
                },
                {
                    "level": "INFO",
                    "message": f"Processing with payload: {j.payload}",
            ...
            if j.status == "completed":
                ...
                j.logs.append(
                    {
                        "level": "ERROR",
                        "message": j.error_message or "Unknown error",
                        "timestamp": now.isoformat(),
                    }
                )
            self._jobs[j.id] = j
...
class InMemoryAICostRepository:
    def __init__(self):
        self._records: builtins.list[AICostRecord] = []
        self._seed()
    ...
    async def list(
        ...
        page: int = 1,
        page_size: int = 20,
    ) -> tuple[builtins.list[AICostRecord], int]:
    ...
    async def get_summary(self, days: int = 30) -> dict:
        ...
        for r in recent:
            ...
            t = by_tenant.setdefault(
                str(r.tenant_id) if r.tenant_id else "system",
                {
                    "tenant_id": str(r.tenant_id) if r.tenant_id else "system",
                    "cost": 0,
                    "tokens": 0,
                },
            )
            ...
            o = by_operation.setdefault(
                r.operation, {"operation": r.operation, "cost": 0, "tokens": 0}
            )
            o["cost"] += r.cost
        ...
        return {
            ...
            "by_tenant": [
                {"tenant_id": k, "cost": round(v["cost"], 4), "tokens": v["tokens"]}
                for k, v in sorted(by_tenant.items(), key=lambda x: -x[1]["cost"])
            ],
            "by_operation": [
                {"operation": k, "cost": round(v["cost"], 4), "tokens": v["tokens"]}
                for k, v in sorted(by_operation.items(), key=lambda x: -x[1]["cost"])
            ],
        }
    ...
    async def get_usage(self, days: int = 30) -> dict:
        ...
        for r in recent:
            m = by_model.setdefault(
                r.model,
                {"model": r.model, "prompt_tokens": 0, "completion_tokens": 0, "total_tokens": 0},
            )
...
class InMemoryHealthRepository:
    _history: builtins.list[HealthSnapshot]
    ...
    async def get_history(self, hours: int = 24) -> builtins.list[HealthSnapshot]:
