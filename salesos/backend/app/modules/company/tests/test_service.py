...
@pytest.mark.asyncio
async def test_get_company_not_found(db_session: AsyncSession, test_tenant: str):
    service = CompanyService(db_session)
    with pytest.raises(NotFoundError):
        await service.get_company("00000000-0000-0000-0000-000000000000", test_tenant)
...
async def test_kg_engine_get_company_insights_basic(db_session: AsyncSession, test_tenant: str):
    ...
    await kg.create_edge(
        c1, c2, EdgeType.COMPETITOR_OF, {"reason": "same_industry"}, tenant_id=test_tenant
    )
