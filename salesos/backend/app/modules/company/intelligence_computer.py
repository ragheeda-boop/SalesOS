...
def _build_dna(resp: Company360Response, firmographic: CompanyFirmographicDTO) -> CompanyDNADTO:
    ...
    return CompanyDNADTO(
        ...
        riskLevel={
            "score": max(0, 100 - resp.health_score * 100),
            "level": "low" if resp.health_score > 0.7 else "medium",
        },
    ...
    if resp.golden_record_id:
        ...
        result.append(
            GoldenRecordEntryDTO(
                ...
                freshness="current",
                status="matched"
                if getattr(resp.enrichment, "is_golden_record", False)
