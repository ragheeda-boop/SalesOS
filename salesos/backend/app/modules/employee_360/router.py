...
async def my_employee_360(
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail=f"Employee {user_id} not found") from None
        ...
        if logger := getattr(request.app.state, "logger", None):
            logger.error(
                "employee_360.get_my_360_failed",
                user_id=user_id,
                error=str(e),
                traceback=full_tb[:1000],
            )
        raise HTTPException(
            status_code=500,
            detail=f"Failed to load employee 360 data: {type(e).__name__}: {str(e)[:150]}",
        ) from e
    try:
...
async def employee_360(
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail=f"Employee {employee_id} not found") from None
    except Exception as e:
        if logger := getattr(request.app.state, "logger", None):
            logger.error("employee_360.get_360_failed", employee_id=employee_id, error=str(e))
        raise HTTPException(status_code=500, detail="Failed to load employee 360 data") from e
    try:
        audit = EmployeeAuditLogger(db)
