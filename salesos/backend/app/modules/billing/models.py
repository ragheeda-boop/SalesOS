...
class DunningCaseModel(Base):
    ...
    __table_args__ = (
        Index("ix_dunning_cases_tenant_id", "tenant_id"),
        Index("ix_dunning_cases_status_grace", "status", "grace_ends_at"),
    )
    ...
    last_stripe_invoice_id: Mapped[str | None] = mapped_column(String(128), nullable=True)
    created_at: Mapped[datetime] = mapped_column(
        DateTime(timezone=True), server_default=func.now(), nullable=False
    )
    updated_at: Mapped[datetime] = mapped_column(
