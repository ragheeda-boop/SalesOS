...
def register_subscribers(event_runtime, session_factory, feature_store=None, log=None):
    ...
    async def on_opportunity_event(event):
        if event.event_type in ("opportunity.created", "opportunity.stage_changed"):
            try:
                ...
                logger.error("NBA recomputation failed for %s: %s", event.aggregate_id, exc)
