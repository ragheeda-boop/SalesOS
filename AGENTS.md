...
## 159. Session Summary (2026-09-26) — ExpansionScoreComputer UTC/local date bug (5th occurrence) + a real test-invocation safety gap closed

| Action | Result | Details |
|---|:---:|---|
| Bug found | **FIXED** | `ExpansionScoreComputer.compute()` (`runtime/feature_store/features.py:586`) computed `days_to_renewal` against `datetime.now(timezone.utc).date()` instead of `date.today()` — the same UTC/local calendar-boundary bug class found and fixed 4 times previously (reports 70, 73, 78). `renewal_date` is a plain DATE column; comparing it to the UTC calendar date silently shifts the renewal window by a day on a positive-UTC-offset host (this one included), which can flip a license's scoring tier. |
| Live reproduction | **DONE** | Reverted the fix, confirmed the exact predicted off-by-one against this host's real, currently-in-effect UTC/local offset, restored. |
| Permanent test | **ADDED** | `tests/integration/test_feature_store_licenses_table_db.py::test_expansion_score_computer_uses_local_calendar_date_not_utc` monkeypatches `date.today()` to a fixed value unrelated to the real UTC date, so a regression fails by ~2,400 days (not a subtle 1-day drift) rather than depending on a live timezone coincidence. Reverted the fix again: failed exactly as predicted (`assert -2429 == 30`); restored: PASS. |
| Real gap found + closed (this file) | **FIXED** | A bare host-side `pytest` run of this file, exporting only `DATABASE_URL`, silently targets `localhost:5432/salesos` — this repo's **persistent local dev Postgres** — instead of the intended disposable container, because `app.database.engine` is built from `settings.app_database_url`, and the checked-in `.env`'s `APP_POSTGRES_PASSWORD` takes precedence over a bare `DATABASE_URL` export. Caught live: one INSERT there failed on a stale pre-`p7q8r9s0t1u2` NOT NULL constraint (that DB's `alembic_version` is empty — never migrated) and rolled back before commit, so nothing was written. Added a `current_database() != "salesos"` refusal to the file's `autouse` fixture so it can't happen again here; correct invocation additionally sets `APP_POSTGRES_PASSWORD=""` to force the fallback to `DATABASE_URL`. |
| Wider gap | **DOCUMENTED, not blanket-fixed** | Every other `_db.py` integration test built on `app.database.engine`/`async_session` (not its own `asyncpg.connect`) shares this exposure — grepped several from this session's own recent work (DEC-157, persistent DLQ, attribution engine) and confirmed none has a `current_database()` safety check. Rolling a check out repo-wide is a separate, deliberate follow-up (each file's "expected database" isn't uniform), not attempted here. |
| Verification | **PASS** | New test genuine red→green; full file 3/3; combined `test_feature_store.py` + `test_feature_store_cache.py` + this file: 35/35; Ruff (E4/E7/E9/F/I) and compileall clean on both touched files (3 pre-existing, unrelated unused-import findings in `features.py` confirmed via `git diff` to predate this change). |
| Commit | **`0618e473`** | Explicit paths only (`runtime/feature_store/features.py`, `tests/integration/test_feature_store_licenses_table_db.py`); not yet pushed. |
| Production / Phase 7 | **UNCHANGED** | Only a disposable `sweep-pg` container (destroyed after) and, transiently with zero rows written, the persistent local `salesos` dev database (one read-only probe + one rolled-back INSERT) were touched. `salesos_test` and production untouched. No gate closed. |
| Loop status | **CONTINUING** | Standing "5 hours, all approvals" authorization from §157/§158; no auto-merge/auto-resolution/cluster-certify attempted. Next: continue the systematic sweep for further genuine code-level defects. |

Full evidence: `project-audit/118_EXPANSION_SCORE_UTC_DATE_BUG_AND_TEST_DB_MISDIRECTION_2026-09-26.md`.

---

## 160. Session Summary (2026-09-26) — D: disk-full emergency resolved (owner-approved cleanup) + LLMProvider.chat_stream Protocol typing fix

| Action | Result | Details |
|---|:---:|---|
| Disk-full emergency | **RESOLVED, owner-approved** | Mid-sweep, `D:\AISalesOS` (15GB FAT32) hit **0 bytes free** — every further edit failed with `ENOSPC`. Investigated before touching anything: Docker's storage lives in the WSL2 VM, not on `D:`, so irrelevant. A PowerShell size breakdown found the 8 nested-duplicate directories already flagged by reports 107/151 as "apparent copy accident, owner to decide" (`packages/packages` 1.6GB, `docs/docs` 471MB, plus 6 smaller ones) and 2 superseded pre-restore database dumps (`salesos_test_pre_repair_20260920.dump` + `salesos_test.dump`, ~504MB, both predating report 139's pinned-Global-ID restore). `packages/data` (3.95GB, real Notion/identity import data) was inspected and left untouched. Each outer directory was verified to hold its own independent real copy before deleting the nested duplicate. **Asked the owner directly** (destructive, hard-to-reverse, outside the standing loop authorization's scope) — approved both deletions. Freed **2.76GB**. Zero git impact: all 9 deleted paths were already untracked (`??`), confirmed via `git status --porcelain --ignore-submodules=all` before and after. |
| Protocol typing fix | **FIXED** | `intelligence/providers/protocol.py`'s `LLMProvider.chat_stream` Protocol stub was `async def ... -> AsyncIterator[StreamEvent]: ...` with no `yield` — mypy correctly infers this as returning a bare `Coroutine`, not an async-iterable, flagging `reliability.py:304`'s `async for` as "not async iterable." Checked reachability first: both real call sites (`llm.py:358`, `reliability.py:304`) already use the correct pattern, and every concrete provider is a genuine async generator — **not a live bug**, but a latent gap where a future missing-await regression at either call site would go uncaught by mypy. Fixed by declaring the Protocol method as `def` (no `async`), the correct PEP 544 convention; concrete implementations are unaffected. Not `@runtime_checkable`, so no `isinstance()` depends on this. |
| Verification | **PASS** | Full `test_ai_foundation_f1/f2/f3.py`: 95/95 PASS. Ruff (E4/E7/E9/F/I) and compileall clean. mypy no longer flags the `chat_stream`/`__aiter__` line. |
| Commit | **`a25fd1d7`** | `intelligence/providers/protocol.py` only. |
| Self-correction | **DISCLOSED** | That commit's message incorrectly characterized 2 adjacent `EmbeddingResponse` mypy findings (lines 314/341) as "the same" dict/list value-type-widening false-positive shape confirmed elsewhere this session. Checked separately afterward: they're a distinct mypy nuance (empty-list-literal `[[]]` inferred as `list[list[Never]]` inside a nested Union), independently confirmed benign via caller analysis (`llm.py:431-433` already handles the `[[]]` fallback correctly) — the conclusion (false positive) holds, but the commit message's stated mechanism was wrong. Recorded here rather than rewriting the local, unpushed commit, per the standing "new commits, not amends" instruction. |
| Two more mypy findings checked | **CONFIRMED, established false-positive shape** | `phase6/pipeline.py:552` and `muhide_adapter.py:457` (`"object" has no attribute "clear"/"append"`) — both the same heterogeneous dict/tuple-literal value-type-widening pattern already documented in reports 64/66/110/112. Not fixed (no behavior change possible). `benchmarks/runner.py:141-148` — same shape, dev-only tool, not pursued. |
| Production / Phase 7 | **UNCHANGED** | No production/`salesos_test` write, no provider call, no Phase 7 gate touched by either the cleanup or the code fix. |
| Loop status | **CONTINUING** | Standing "5 hours, all approvals" authorization; the disk-space decision was explicitly carved out and put to the owner first, since it was destructive and outside that authorization's scope. |

Full evidence: `project-audit/119_DISK_FULL_EMERGENCY_CLEANUP_AND_PROTOCOL_TYPING_FIX_2026-09-26.md`.

---

## 161. Session Summary (2026-09-26) — PostgresPipelineRepository stage-entry methods: guaranteed crash on every real call, 2 independent bugs fixed

| Action | Result | Details |
|---|:---:|---|
| Loop extended | **RECORDED** | User: "يلا كمل في لوب 6 ساعات" — continuing the standing loop, now framed as 6 hours, all approvals; same standing invariants (no production writes, no Apollo/Maps, no auto-merge/auto-adjudicate for G4/G2, salesos_test + disposable containers only, explicit-path commits, push under standing authorization). |
| Reachability | **Live, not dead code** | `PostgresPipelineRepository` is instantiated by `app/routers/commercial.py`'s `_get_pipe(db)` factory and used by real mounted endpoints (also referenced by `app/graphql/query.py`) — every real call to `PipelineService.enter_stage()`/`exit_stage()` would have crashed. |
| Bug 1 — contract/model field mismatch | **FIXED** | The `StageEntry` domain contract (`stage_name`, no `tenant_id`, no `from_stage`/`to_stage`) and the DB model `StageEntryModel` (`from_stage`/`to_stage`/`duration_hours`, required `tenant_id`, table `commercial_stage_entries`, FORCE RLS) model genuinely different shapes (a "stay in a stage" vs. a "stage transition"). The repository read `entry.from_stage`/`entry.to_stage` off the contract (`AttributeError` — confirmed as the actual crash) and constructed `StageEntry(from_stage=..., to_stage=..., duration_hours=...)` on reads (`TypeError`, unexpected keyword arguments); `tenant_id` defaulted to `""`, which would fail RLS's `WITH CHECK` even past the first crash. |
| Bug 2 — found while fixing bug 1 | **FIXED** | `save_stage_entry()` always did an unconditional INSERT; `enter_stage()`'s "close the previous entry" step re-saves the same already-persisted row (same id) to set `exited_at` — a primary-key violation on every stage transition after the first, even with bug 1's field names fixed. Fixed by upserting on `entry.id`. |
| Fix | **Additive, both bugs** | `StageEntry` gains `tenant_id: str = ""` and `from_stage: str = ""` (appended, all call sites use keyword args, no positional break). `PipelineService.enter_stage()` populates both from information it already has (`pipeline.tenant_id`, `prev.stage_name`). Repository upserts by id, maps `stage_name↔to_stage`/`from_stage↔from_stage` correctly, computes `duration_hours` from `entered_at`/`exited_at` via a shared helper, and reconstructs contract objects through one `_to_contract()` helper. |
| Scope decisions, disclosed not fixed | **DOCUMENTED** | `exit_reason` still has no DB column (silently lost on read-back — a data-loss gap, not a crash; needs a migration, out of scope here). `save_definition()`'s stage serialization is lossy (drops `entry_criteria`/`exit_criteria`/`sla_days`/etc.), silently disabling pipeline criteria enforcement — found as a side-effect, does not crash, not touched. |
| Verification | **Genuine red→green** | New `tests/integration/test_pipeline_stage_entry_persistence_db.py` drives the real `PipelineService`→`PostgresPipelineRepository` flow through two real stage transitions against a fresh, fully-migrated, RLS-enforced disposable DB. Reverted via a scoped `git stash` of exactly the 3 changed files: `AttributeError: 'StageEntry' object has no attribute 'from_stage'` — the exact predicted failure. Restored: PASS, plus direct-SQL assertions proving exactly 2 rows persist (no PK-violation duplicate) with correct `to_stage`/`from_stage`/`duration_hours`/`tenant_id`. |
| Regression | **50/50 PASS** | `test_pipeline.py` + `test_pipeline_analytics.py` + report 117's `test_pipeline_analytics_score_deal_db.py` (same table) + the new file. Ruff: 54 pre-existing findings in the 1,700-line file, identical count before/after this change (confirmed via scoped stash/pop) — 0 new; the new test file is Ruff-clean on its own. `compileall` and `git diff --check` clean. |
| Production / Phase 7 | **UNCHANGED** | Only a disposable, ephemeral Postgres container was used, torn down after. `salesos_test` and production untouched. No gate closed. |
| Loop status | **CONTINUING** | Next candidate in the same file/mypy batch: the `Quote`/`QuoteRevenueKPIs` field-mismatch findings in `postgres_repositories.py`, which look like the same bug class. |

Full evidence: `project-audit/120_PIPELINE_STAGE_ENTRY_CONTRACT_MODEL_MISMATCH_2026-09-26.md`.

---

## 162. Session Summary (2026-09-26) — PostgresQuoteRepository crash fix (same class as §161) + a real env-leak near-miss caught and fixed

| Action | Result | Details |
|---|:---:|---|
| Reachability | **Live** | `PostgresQuoteRepository` wired via `app/routers/commercial.py`'s `_get_quote(db)` factory — every real quote lifecycle call (`create_quote`/`add_line`/`submit_for_approval`/`approve`/`send`/`accept`) would have crashed. |
| Bugs — same class as §161's `StageEntry` | **FIXED** | `Quote` contract has `lines`/`approval` (nested `approved_by`/`approved_at`)/computed `grand_total`; `QuoteModel` denormalizes flat `total_value`/`sent_at`/`approved_by`/`approved_at`/`accepted_at` columns. `save()` read the flat names directly off the contract object — none exist (`AttributeError` every call, confirmed). `get()`/`get_by_opportunity()`/`list_by_tenant()` constructed `Quote(total_value=..., sent_at=..., ...)` — none of those are real constructor keywords. Independently: `quote.lines` was never persisted at all — `add_line()` (a real service method) would silently lose every added line. |
| Fix | **Upsert + status-transition inference** | `save()` upserts by id, maps `grand_total→total_value`, `approval.approved_by/approved_at→model` fields directly, and infers `sent_at`/`accepted_at` (which have no contract equivalent) from a status-transition check against `quote.updated_at` — not fabricated history, since `send()`/`accept()` already set `updated_at` to "now" immediately before saving. New `_sync_lines()` upserts/deletes `QuoteLineModel` rows to match `quote.lines`. `revenue_kpis()`'s field names fixed to mirror the in-memory reference exactly (0 live callers, fixed for contract alignment while already in the file). |
| Bonus fix, same file | **FIXED** | `QuoteService._record_approval_audit()` (P1-9 compliance audit trail) imported `from sdk.database import async_session_factory` — no such attribute exists anywhere in `sdk/database.py`; every call raised `ImportError`, silently swallowed by a bare `except Exception: pass`. This audit feature had likely never recorded a single entry. Fixed to `app.database`'s real `async_session` + `apply_tenant_guc` (`audit_logs` is FORCE-RLS). |
| Not fixed, disclosed | **DOCUMENTED** | `commercial_quote_lines` has no `discount_percent`/`tax_percent`/`description_ar`/`product_code`/`notes` columns — those per-line fields don't round-trip through this schema. Pre-existing, not introduced here. |
| **Real near-miss during verification** | **CAUGHT, FIXED, DISCLOSED** | A `pytest` run without re-exporting `DATABASE_URL`/`APP_POSTGRES_PASSWORD` in the same shell call (report 118's exact exposure) silently hit the persistent local dev database — and this time a real write actually landed: one `tenants` row (mid-test commit) and one `audit_logs` row (from the newly-fixed audit path's own separate session+commit) before the outer test's later rollback could undo anything. Both found and deleted immediately; confirmed zero other stray rows. Added report 118's `current_database() != "salesos"` guard to both this report's new test file and §161's (which hadn't received it yet) — verified the guard actually refuses the run before touching any table. |
| Second bug found while adding the guard | **FIXED** | Both test files used a `def test_...(): asyncio.run(_run())` wrapper (copied from an unrelated sibling test); combined with the new autouse async fixture, this created two different event loops sharing the same process-global `engine` (`RuntimeError: attached to a different loop`). Fixed by converting both to plain `async def test_...():` (this repo's `asyncio_mode = "auto"` needs no decorator), matching report 118's own file's correct pattern. |
| Verification | **Genuine red→green** | New `tests/integration/test_quote_repository_persistence_db.py` drives the real service lifecycle end to end. Reverting exactly the 2 changed files: `AttributeError: 'Quote' object has no attribute 'total_value'` — exact predicted failure. Restored: PASS, plus direct-SQL proof of correct `total_value`/`sent_at`/`accepted_at`/line persistence. |
| Regression | **67/67 PASS** | `test_quote.py` + `test_pipeline.py` + `test_pipeline_analytics.py` + report 117's DB test + both new integration tests. Ruff: `postgres_repositories.py` 52→50 findings (confirmed via scoped stash/pop — this fix removed 2 pre-existing issues, added 0). `compileall` and `git diff --check` clean. |
| Production / Phase 7 | **UNCHANGED** | `salesos_test` and production untouched; only a disposable container and (briefly, now cleaned + guarded against recurrence) the persistent local dev database were touched. No gate closed. |
