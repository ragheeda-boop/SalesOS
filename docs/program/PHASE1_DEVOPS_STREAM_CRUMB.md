...
| Field 2h soak | **OPTIONAL; r2 FAIL** | Wall-clock 7200s achieved; `all_iters_ok=false` (ITER 11–12 harness exit 2 — CSRF missing on POST `/load/run-all` + tip-line 401). Not invent PASS. Fix tip: CSRF harness + remint-retry. |
