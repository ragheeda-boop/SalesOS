...
**Date:** 2026-07-31
**Context:** CI-12 (triage #3) — CI `security-secrets-scan` job's `Run Trivy filesystem scan` failed (exit 1) but the log showed only `[npm] Detecting vulnerabilities...` → `[poetry] Detecting vulnerabilities...` → exit, with **no vulnerability table and no error** between them. Triage classified it "silent failure", requiring local reproduction with verbose output before any fix.
**Alternatives considered:** (a) Remove `exit-code: 1` to match `security-scan.yml`'s non-blocking `secret-scan` — rejected: weakens the CI gate (governance: never weaken without approval); (b) narrow `scan-ref` to `./salesos` to drop root scrapers — rejected: scan-scope change is policy, needs separate approval; (c) split the step into a blocking visible table gate + a non-blocking SARIF export leg — chosen.
**Decision:** Local reproduction on a clean `git archive` snapshot of `HEAD` with the exact CI flags (`--format sarif --severity CRITICAL,HIGH --exit-code 1`) reproduces the triage log **verbatim** (npm/poetry detecting lines → exit 1, no table). **Root cause:** with `--format sarif --output <file>`, Trivy writes findings only to the SARIF file; stdout carries progress lines only — the gate correctly blocks but is opaque. The findings are REAL and match already-tracked debt exactly: `salesos/backend/poetry.lock` → **10 HIGH** (ecdsa CVE-2024-23342 "Minerva attack" 0.19.2, etc. → **CI-16 / R-21**) + `salesos/frontend/package-lock.json` → **1 HIGH** (sharp GHSA-f88m-g3jw-g9cj, 0.34.5 → 0.35.0 → **CI-14**). Inconsistency confirmed: `security-scan.yml` `secret-scan` has **no** `exit-code` (always passes, SARIF-only); CI `security-secrets-scan` uses `exit-code: 1` (strict gate). Secondary: secret-scanner walks large tracked root-scraper JSONs (`balady_scraper/engineering_offices_full.json` 24 MB, `najiz_scraper/data/lawyers.json` 12 MB) with high-memory warnings — scan-scope note for later, not this story.
**Consequence:** Split the CI Trivy step (`.github/workflows/ci.yml` `security-secrets-scan`) into: (1) `Run Trivy filesystem scan (blocking gate, visible findings)` — `format: table`, `severity: CRITICAL,HIGH`, `exit-code: 1` → findings now PRINT to the log; (2) `Export Trivy SARIF for code scanning` — `format: sarif`, `output: trivy-results.sarif`, `exit-code: 0` → code-scanning upload preserved. Gate strictness unchanged; no security weakening. The job remains red while the 11 tracked findings exist — now visibly and actionably. Local proof: table+exit-1 run prints the table and exits 1; sarif+exit-0 run produces the file and exits 0. CI-12 **CLOSED**; program progress **13/19**.
