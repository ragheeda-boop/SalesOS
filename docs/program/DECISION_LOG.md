...
### DEC-159 — Gate /api/v1/cache/* behind the "admin" role (Accepted — CLOSED)

**Date:** 2026-09-23
**Context:** `project-audit/63_CACHE_SERVICE_GRACEFUL_FAILOVER_CLOSURE_2026-09-23.md` ("Next code-reachable work" item 1) flagged that `app/modules/cache/router.py` (`/api/v1/cache/*`) required only `verify_token` — any authenticated user, any role, any tenant — for raw caller-supplied-key `get`/`set`/`delete` plus a wildcard `flush(pattern="*")`, with no per-key tenant-prefix check. A wider blast radius than a typical per-tenant business endpoint.
**Decision:** **Accepted.** Router-level dependency changed from `Depends(verify_token)` to `Depends(require_role_dep("admin"))` (already depends on `verify_token` transitively) — matches this codebase's existing pattern for infra/ops-level routers (`app/routers/metrics.py`, `benchmarks.py`).
**Verification:** `tests/unit/test_cache_admin_router.py` updated (override `get_current_user_role`, not the factory-produced `require_role_dep` closure) + 2 new tests (non-admin role → 403 on all 5 endpoints; manager role also rejected). Genuine red→green: reverted, watched all 6 tests fail as predicted, restored, re-confirmed 6/6 PASS. Full `tests/unit/` regression: 3765 passed, 1 pre-existing unrelated failure.
**Consequence:** Closes the access-control gap. Does not add cache key-namespace tenant isolation (a caller with legitimate "admin" for their own tenant could still, in principle, reach another tenant's cache key if they can guess its name) — that is a separate, larger finding not addressed here. Does not touch `CacheService`'s failover behavior (already closed, report 62/63) or the unrelated circuit-breaker performance item.
**Status:** Accepted — CLOSED.

### DEC-158 — Owner-plane billing tables: accept app-layer RBAC, no RLS (Accepted — CLOSED)

**Date:** 2026-09-23
**Context:** `project-audit/60_BILLING_TABLES_OWNER_PLANE_RLS_FINDING_2026-09-23.md` found 6 owner-plane billing tables (`subscriptions`, `usage_meters`, `usage_meter_events`, `dunning_cases`, `platform_billing_invoices`, `stripe_webhook_events`) with `tenant_id` but no RLS — structurally different from DEC-157's 14 tables because the owner-admin billing dashboard has a real, already-shipped requirement to read across all tenants (`usage_meter_router.py`'s optional `tenant_id: uuid.UUID | None = Query(None)`), through the same `salesos_app` restricted role every tenant request uses. A standard per-tenant RLS policy cannot serve both "owner admin sees every tenant" and "a tenant never sees another tenant" through the same DB role at once.
**Alternatives considered:** (a) a second, distinct Postgres role for owner-plane DB access — real fix, but a materially larger architecture change touching connection management repo-wide; (b) a role-aware `OR is_owner_context()` RLS predicate — more fragile, a wrong implementation would be a worse hole than today's absence of RLS; (c) formally accept the existing app-layer RBAC (`require_owner_role_dep("admin")`) design as sufficient, matching this codebase's already-explicit "Owner-platform vs Tenant-API JWT audiences" two-plane design (`11_CAPABILITY_MATRIX.md` §1.6) — **accepted**.
**Confirmation before Accept:** re-ran and extended report 60's grep of every router/service file importing the 6 billing ORM model classes; found 2 additional import sites (`app/modules/admin/routers/billing.py`, `app/modules/admin/routers/tenants.py`), both confirmed owner-role-gated at the router level (`require_owner_role_dep("admin")`), and 2 apparent matches (`SignalSubscriptionModel`, `WebhookSubscriptionModel`) confirmed as unrelated classes (substring false positives). No tenant-JWT-gated route was found reaching any of the 6 tables. Not a formally exhaustive trace of every code path in the repository.
**Decision:** **Accepted, no code change.** These 6 tables intentionally have no `tenant_isolation_<table>` policy; app-layer RBAC is the isolation boundary. Conditional on every current and future read path staying owner-role-gated — a future tenant-JWT-gated route touching these tables would void this DEC's safety argument for that route and require re-review.
**Consequence:** Closes report 57/60's open finding as a deliberate, documented design, not a gap. Does not change the capability register (these tables were never a counted capability row) or production/Phase 7 status.
**Status:** Accepted — CLOSED.

### DEC-157 — Tenant GUC pinning + RLS for the 14 live orphan-keep tables (Accepted — CLOSED)

**Date:** 2026-09-23
**Context:** `project-audit/59_ORPHAN_KEEP_TABLES_RLS_GAP_2026-09-23.md` (capability-register mechanical audit) found 14 tables carrying `tenant_id` — the 9 Feature Store `company_*` tables, `company_policies`, `decisions`, `decision_feedback_loop`, `domain_events`, `activity_records` — that are live (read/written via raw SQL by five routed runtime engines: Feature Store, Policy Runtime, Decision Runtime, Event Runtime, Activity Runtime) but have **zero** database-level RLS. All 14 are already governed by DEC-130f's "no DROP without a dedicated DEC" KEEP register, so dropping was never an option; the finding here is narrower and different: adding RLS today, without first pinning the `app.tenant_id` session GUC at five call sites that currently never set it, would break live functionality outright (unpinned sessions would see zero rows / fail every write's `WITH CHECK`). Two open semantics questions (`activity_records`'s nullable tenant_id; `domain_events.read_by_type()`'s cross-tenant read) blocked a safe blanket policy until ruled on.
**Alternatives considered:** (a) add RLS now without GUC pinning, matching the `account_funnel`/`score_observations` fix in report 58 — rejected, that fix's tables were already GUC-compatible and these were not; (b) leave as permanent status quo — rejected as a final answer, acceptable only as a temporary hold; (c) rule on semantics, add GUC pinning, regression-prove, then add RLS, as one reviewed sequence — **accepted and executed**.
**Semantics rulings (§3 of the companion file):** (1) `activity_records.tenant_id` nullability — exhaustive grep found zero legitimate cross-tenant/tenant-less read callers; no carve-out, standard fail-closed policy applies (matches the accepted `admin_ai_costs`/`admin_jobs` precedent in `d1a8c35e7f09`, consistent with `b7e2f65a3f07`'s rejection of an `OR tenant_id IS NULL` bypass). (2) `domain_events.read_by_type()` cross-tenant read — zero callers anywhere in the codebase (dead code); interface now requires `tenant_id`, standard policy applies.
**Decision:** **Accepted and executed, all 4 steps, same session.** (1) Rulings above. (2) GUC pinning added to all 5 call sites (`runtime/feature_store/features.py` ×5, `runtime/policy_runtime/__init__.py`, `runtime/decision_runtime/feedback_loop.py`, `sdk/events/store.py` via a local `_pin_tenant_guc()` helper — duplicated, not imported, to avoid a circular import with `app.database` — `runtime/activity_runtime/__init__.py` ×4). (3) Regression proof: full local `tests/unit/` run, 3763 passed (1 pre-existing unrelated failure, a stale count assertion predating this session); `test_event_runtime_subscriber_bounds.py` updated red→green for the new GUC-pin call. (4) Migration `e1d1c1225d00` (new Alembic head, was `70193187420d`) adds `ENABLE`/`FORCE ROW LEVEL SECURITY` + the canonical policy to all 14 tables via `app.alembic.lib.rls.generate_policy_sql`; proven on a fresh ephemeral `pgvector/pgvector:pg16` container (built from current source, torn down after): full migration chain from zero, downgrade→upgrade round trip, and a new 9-test cross-tenant isolation suite (`tests/integration/test_dec157_orphan_keep_rls_db.py`) exercising every one of the 14 tables through its real GUC-pinned application code path (not just `pg_class` inspection) under the restricted non-superuser, non-BYPASSRLS `salesos_app` role — including a fail-closed proof that a NULL-tenant `activity_records` insert is rejected outright, no bypass.
**Side-effect fix:** found and fixed a real cross-tenant injection bug in `runtime/activity_runtime/router.py`'s `ingest-batch` endpoint (a client-supplied `tenant_id` in the request body could override the authenticated caller's tenant) — red→green verified, new regression test added.
**Consequence:** The 14-table RLS gap is closed. Does not affect DEC-130f (no DROP; RLS is not a schema-shape change, `alembic check`'s posture unaffected) or DEC-156 (different concern — MetaData island location, not RLS). No production/staging migration — this DEC's execution was verified only on ephemeral, disposable Postgres containers; `salesos_test` and any shared database were never touched.
**Evidence:** `project-audit/68_DEC157_ORPHAN_KEEP_RLS_CLOSURE_2026-09-23.md`; full ruling/validation detail in [`decisions/DEC-157-ORPHAN-KEEP-TENANT-GUC-RLS-REMEDIATION.md`](decisions/DEC-157-ORPHAN-KEEP-TENANT-GUC-RLS-REMEDIATION.md).
**Status:** Accepted — CLOSED.
...
**Status:** Accepted
**Orchestration follow-on (2026-08-02, Execution Orchestrator — not a new DEC; Watchdog tip `53a4aa7`):** **TRIGGER_POST_PHASE0_PLAN** confirmed. Tip checklist **54/54**. Operating State → **PHASE 1 PARALLEL EXECUTION ACTIVE**. Premature withdraw `a08d7c0` reversed. Streams **A Backend / B Frontend / C DevOps / D Validation** launched ([`PHASE1_STREAM_LAUNCH_CRUMB.md`](PHASE1_STREAM_LAUNCH_CRUMB.md)). D1 PASS. Never-stop: integrate returns → board/DAG → reassign next Sprint 04 stories. **No Production GO / GA GO invent.**
.
...
**Date:** 2026-08-02
**Context:** Under DEC-151 freeze + user-authorized independent ARB lane, Stream C required PASS/FAIL on B1–B7 (4.1) and a new validation report with zero CRITICAL (4.8). Evidence pack already assembled; historical FAIL remains in `.engineering/32_EOS_VALIDATION_AUDIT.md`.
**Alternatives considered:** (a) FAIL solely for fingerprint pin lag (33 commits) — **rejected as CRITICAL** (material claims still match tip; Active revalidation); (b) rubber-stamp without tip measure — **rejected**; (c) PASS with CRITICAL=0 + non-blocking residuals — **approved**.
**Decision:** Accept **4.1 PASS** and **4.8 PASS**. Report: [`.engineering/34_EOS_REAUDIT_2026-08-02.md`](../../.engineering/34_EOS_REAUDIT_2026-08-02.md) @ tip `74f698b`. Companion: [`decisions/DEC-153-CRITERION-4-1-4-8-ARB-REAUDIT-PASS.md`](decisions/DEC-153-CRITERION-4-1-4-8-ARB-REAUDIT-PASS.md). Pack: [`ARB_PHASE0_4_1_4_8_EVIDENCE_PACK.md`](ARB_PHASE0_4_1_4_8_EVIDENCE_PACK.md).
...
**Date:** 2026-08-02
**Context:** DEC-149 Accepted + DEC-149a CLOSED CONDITIONAL established canonical live deploy = Railway (BE) + Vercel (FE); staging deferred; `railway up` does **not** require GHCR. Checklist **3.6/3.9/3.10** and `ci.yml` Stage 6 previously required GHCR `:latest`/SHA push. Proposed land packaged A/B for ARB without deciding.
**Alternatives considered:** (A) Stage 6 remains required capability — keep 3.6/3.9/3.10 + CI-08 until push SUCCESS (DEC-104 Option A) — **rejected by ARB**; (B) Stage 6 GHCR obsolete vs DEC-149 live path — governance rewrite + Stage 6 quarantine — **approved**; (c) Cursor invent Option without ARB — **rejected**.
**Decision:** **Accepted — Option B (ARB Approved).** Stage 6 GHCR publish is **no longer** a required Phase 0 capability after DEC-149. Cross-ref DEC-149 / DEC-149a. Companion: [`decisions/DEC-150-STAGE-6-GHCR-POST-DEC-149.md`](decisions/DEC-150-STAGE-6-GHCR-POST-DEC-149.md). **3.6 / 3.10 → CLOSED — SUPERSEDED** (governance; not field GHCR green). **3.9** redefined = Stages 1–5 + Railway/Vercel deploy (no GHCR); remains OPEN. **3.7** decoupled from Stage 6 GHCR; remains OPEN (no E2E services). CI-08 → **GOVERNANCE COMPLETED** (superseded). `ci.yml` Stage 6 quarantined (`if: false`). DEC-104 Option A no longer Phase 0 gate.
**Consequence:** Phase 0 pin **46/54 → 48/54 NO-GO**. CI/CD Complete **7 → 9** / Blocked **1 → 0**. Residual GHCR 403 = legacy/non-blocking. App code / DEC-085 untouched. **Production GO not claimed. CI GREEN not met. No 4.1/4.8 invent.** Validation: **governance implementation only**; runtime validation pending.
**Status:** Accepted (Option B Approved).

### DEC-149 — Canonical deploy topology: Backend → Railway; Frontend → Vercel

**Date:** 2026-08-02
**Context:** Architecture Validation (hybrid) showed live/intended deploy = Railway (backend) + Vercel (frontend) while GitHub Actions `deploy.yml` / `deploy-staging.yml` still assume VPS/SSH. Phase 0 criterion **3.11** / CI-09 previously framed as ops provision of `VPS_HOST` / `VPS_USER` / `VPS_SSH_KEY`. User governance ruling approved the hybrid verdict and forbade creating a VPS or unused `VPS_*` solely to close the criterion. **Amend:** user ruling that the project currently operates with **one environment only** (production Railway); staging variables must not be required until staging is created.
**Alternatives considered:** (a) provision unused `VPS_*` to close CI-09 — rejected; (b) mark CI-09 obsolete / CLOSED without DEC — rejected; (c) modify workflows before governance DEC — rejected; (d) ADR instead of DEC — deferred (program DECs preferred for Phase 0 ops topology; permanent ADR optional later); (e) DEC Accepted naming Railway+Vercel + CI-09 BLOCKED BY GOVERNANCE + workflow migration deferred — approved for governance land; (f) Backend/DevOps workflow migration follow-on — assigned and landed; (g) keep requiring `RAILWAY_STAGING_*` on master push — rejected (single-env); (h) staging `workflow_dispatch` + soft-skip until staging exists — approved.
**Decision:** Accept canonical deploy **Backend → Railway**, **Frontend → Vercel**. Companion: [`decisions/DEC-149-CANONICAL-DEPLOY-RAILWAY-VERCEL.md`](decisions/DEC-149-CANONICAL-DEPLOY-RAILWAY-VERCEL.md) §1a/§6. Single-env amend: production secrets required for close path; staging secrets **optional/deferred**; `deploy-staging.yml` dispatch-only + soft-skip. Closed CONDITIONAL via DEC-149a after Validation PASS + Architecture prior PASS. DEC-085 intact.
**Consequence:** Master push no longer dual-fires failing staging deploy. Validation field-verifies Deploy Production after production §6 names provisioned. Phase 0 remains **NO-GO**. CI-08 ops BLOCKED separately. **Production GO not claimed. CI GREEN not met.**
**Status:** Accepted (+ single-env amend). Criterion **CLOSED CONDITIONAL via DEC-149a**.

**Validation field-verify (2026-08-02):** **VALIDATION_PASS** — Deploy Production [30723120473](https://github.com/ragheeda-boop/SalesOS/actions/runs/30723120473) @ c3507ed SUCCESS: Railway up SUCCESS; Backend Health Gate SUCCESS (HTTP 200); Frontend Vercel SUCCESS (Git primary). Repo secret names: RAILWAY_TOKEN, RAILWAY_PROJECT_ID, RAILWAY_SERVICE_ID, RAILWAY_ENVIRONMENT_ID, RAILWAY_HEALTH_URL. CI-09 / 3.11 → READY_FOR_REVIEW then **CLOSED CONDITIONAL** via DEC-149a. Staging deferred. No Production GO / CI GREEN / ARB invent.

### DEC-149a — Orchestrator CLOSED CONDITIONAL criterion 3.11 / CI-09 (2026-08-02)

**Context:** Architecture prior ARCHITECTURE_PASS on DEC-149 + single-env amend. Validation Lead **VALIDATION_PASS** ([Validate CI-09](8d5bad88-a487-471b-80b9-e8eb2c56a8cf)) — Deploy Production [30723120473](https://github.com/ragheeda-boop/SalesOS/actions/runs/30723120473) @ tip `c3507ed` SUCCESS (Railway up; Health Gate HTTP 200; FE Vercel Git-primary). Production §6 secret names present (repo-scoped). Crumbs at `ac7704a` were READY_FOR_REVIEW.
**Decision:** Accept CI-09 / **3.11 VERIFIED → CLOSED CONDITIONAL** (same honesty pattern as 3.8 DEC-148a / 3.5 DEC-147a). Conditions explicit: (1) FE verified via **Git-primary** workflow path, not Vercel CLI prod deploy; (2) staging deferred (single-env DEC-149); (3) no VPS. Does **not** close **3.6/3.9/3.10** (CI-08 GHCR) or invent EOS **4.1/4.8** ARB PASS. Phase 0 **45/54 → 46/54**. CI/CD Complete **6 → 7** / Blocked **2 → 1** (CI-08). Residual CI **3.6/3.7/3.9/3.10** + ops CI-08 and EOS **4.1/4.8** remain OPEN (non-blocking for 3.11). Do **not** claim Production GO / full CI GREEN / unconditional CLOSED / Phase 0 GO.
**Consequence:** CI/CD cluster Complete **7/11**. Next PARALLEL READY: EOS Audit **4.1/4.8** (ARB — do not invent); optional contract tests / Jest 30. BLOCKED: ops CI-08 (**3.6/3.9/3.10**); **3.7** Stage-6-dep. Phase 0 remains **NO-GO**. **Production GO not claimed. CI GREEN not met.**
**Status:** Accepted. Criterion **3.11 / CI-09 CLOSED CONDITIONAL**.
...
**Decision:** Accept criterion **3.8** as **Cursor COMPLETE** / **READY FOR REVIEW**. Companion: [`decisions/DEC-148-CRITERION-3-8-CI-GREEN-CODE-PATH.md`](decisions/DEC-148-CRITERION-3-8-CI-GREEN-CODE-PATH.md). Residual: *tip Stages 1–5 same-run field-verify PENDING until tip containing this land is pushed*. Historical corroboration Stages 1–5 SUCCESS @ `7ba137b` / `30689682988` (not tip). DEC-085 intact. Closed CONDITIONAL via DEC-148a after Arch+Val PASS_CONDITIONAL.
**Consequence:** Phase 0 criterion **3.8** = READY FOR REVIEW then **CLOSED CONDITIONAL** via DEC-148a. Adjacent **3.7** BLOCKED. Ops CI-08/09 BLOCKED. EOS **4.1/4.8** ARB. **Production GO not claimed. CI GREEN not met.**
**Status:** Accepted. Criterion **CLOSED CONDITIONAL via DEC-148a**.

### DEC-148a — Orchestrator VERIFIED/CLOSED CONDITIONAL criterion 3.8 (2026-08-02)

**Context:** Architecture PASS_CONDITIONAL ([architecture review 3.8](f1aabd28-8f69-4e4b-8b15-65c4255afbab)) + Validation PASS_CONDITIONAL ([Validate 3.8](16d41a8d-77b9-48ce-9ee3-323d90a8c2cf)) on land `14fce5f` / DEC-148; local ruff exit 0; tip Stages 1–5 same-run PENDING push.
**Decision:** Accept **3.8 VERIFIED → CLOSED CONDITIONAL** (same honesty pattern as 3.5 DEC-147a / 1.5 DEC-128a). Residual explicit: *tip Stages 1–5 same-run field-verify PENDING until tip containing `14fce5f` is pushed* (Stage 3/4 may still fail when unblocked). Does **not** close **3.6–3.11** ops / **3.9** full CI GREEN. Phase 0 **44/54 → 45/54**. CI/CD Complete **5 → 6** / Open **4 → 3**. Residual CI **3.6/3.7/3.9–3.11** + ops CI-08/09 and EOS **4.1/4.8** remain OPEN (non-blocking for 3.8). Do **not** push. Do **not** claim Production GO / CI GREEN / unconditional CLOSED.
**Consequence:** CI/CD cluster Complete **6/11**. Next PARALLEL READY: EOS Audit **4.1/4.8** (ARB — do not invent); CI **3.7** non-GHCR E2E/services prep (still blocked on Stage 6 dep for full path); optional contract tests / Jest 30. BLOCKED: ops CI-08/CI-09 (**3.6/3.9/3.10/3.11**). **Production GO not claimed. CI GREEN not met.**
**Status:** Accepted. Criterion **3.8 CLOSED CONDITIONAL**.
...
**Decision:** Accept criterion **8.1** as **Cursor COMPLETE** / **READY FOR REVIEW**. Companion: [`decisions/DEC-143-CRITERION-8-1-ENGINEERING-OS-CLEAN.md`](decisions/DEC-143-CRITERION-8-1-ENGINEERING-OS-CLEAN.md). Working tree clean @ `b82b9fb`; parent gitlink unchanged; conflict #11 RESOLVED note. DEC-085 intact. Closed via DEC-143a after Arch+Val PASS.
**Consequence:** Phase 0 criterion **8.1** = READY FOR REVIEW then **CLOSED** via DEC-143a. Residuals **8.2/8.3** and EOS **4.1/4.8** OPEN. **Production GO not claimed. CI GREEN not met.**
**Status:** Accepted. Criterion **CLOSED via DEC-143a**.

### DEC-143a — Orchestrator VERIFIED/CLOSED criterion 8.1 (2026-08-01)

**Context:** Architecture PASS ([architecture review 8.1](b66ea200-ee23-47eb-b181-3bd2c6de8512)) + Validation PASS (light) ([Validate 8.1](f986e7ff-50be-484c-890f-ce3511ec577c): pin `b82b9fb`, clean tree) on land `89502ef` / DEC-143.
**Decision:** Execution Orchestrator records criterion **8.1 VERIFIED → CLOSED**. Phase 0 **39/54 → 40/54**. Eng Stability Complete **1 → 2** / Open **3 → 2**. Residuals Eng Stability **8.2/8.3** and EOS **4.1/4.8** remain OPEN (non-blocking for 8.1). Do **not** push. Do **not** claim Production GO / CI GREEN.
**Consequence:** Next PARALLEL READY: Eng Stability **8.2/8.3**, EOS Audit **4.1/4.8** (ARB). **Production GO not claimed. CI GREEN not met.**
**Status:** Accepted. Criterion **8.1 CLOSED**.
...
**Decision:** Accept criterion **5.3** as **Cursor COMPLETE** / **READY FOR REVIEW**. Companion: [`decisions/DEC-134-CRITERION-5-3-SOT-ORIENTED-VALIDATE.md`](decisions/DEC-134-CRITERION-5-3-SOT-ORIENTED-VALIDATE.md). Default validate = SoT-oriented (joined secondaries subset-of decorator SoT + join map integrity); `--legacy-equality` diagnostic; import-light source parse. DEC-085 intact. Closed via DEC-134a after Arch+Val PASS.
**Consequence:** Phase 0 criterion **5.3** = READY FOR REVIEW then **CLOSED** via DEC-134a. Capability Drift cluster COMPLETE **4/4**. **Production GO not claimed. CI GREEN not met.**
**Status:** Accepted. Criterion **CLOSED via DEC-134a**.

### DEC-134a — Orchestrator VERIFIED/CLOSED criterion 5.3 (2026-08-01)

**Context:** Architecture PASS ([architecture review 5.3](d7e7dfbd-d9ba-417b-8d14-61e12f29af1f)) + Validation PASS ([Validate 5.3](0d6cdb81-8e10-4094-9057-253549a3d753): host exit **0**; Docker exit **0**; `--legacy-equality` exit **2** diagnostic; DEC-085 untouched) on land `6a98999` / DEC-134.
**Decision:** Execution Orchestrator records criterion **5.3 VERIFIED → CLOSED**. Phase 0 **28/54 → 29/54**. Capability Drift Complete **3 → 4** / Open **1 → 0** (cluster COMPLETE 4/4). Non-blocking INFO residuals: SDK/YAML extras outside SoT, unmapped CAP-### entries, CAP-037→`capability-framework` semantic-join refine (do not re-open 5.3). Do **not** push. Do **not** claim Production GO / CI GREEN.
**Consequence:** Capability Drift residual = none (cluster closed). Next PARALLEL READY: ADR Drift **6.x**, EOS Audit **4.x**, Engineering Stability **8.1–8.3**, ADR-036 **9.2**. **Production GO not claimed. CI GREEN not met.**
**Status:** Accepted. Criterion **5.3 CLOSED**. Capability Drift **COMPLETE 4/4**.
