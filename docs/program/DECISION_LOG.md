...
**Decision:** Accept canonical deploy **Backend → Railway**, **Frontend → Vercel**. Companion: [`decisions/DEC-149-CANONICAL-DEPLOY-RAILWAY-VERCEL.md`](decisions/DEC-149-CANONICAL-DEPLOY-RAILWAY-VERCEL.md) §1a/§6. Single-env amend: production secrets required for close path; staging secrets **optional/deferred**; `deploy-staging.yml` dispatch-only + soft-skip. CI-09 / **3.11** → **BLOCKED** (production secrets) — **not CLOSED**. DEC-085 intact.
