...
| STORY-03-04 (contract test framework) | DevOps/SRE* | P1 | Medium | **DONE** | Template merged, used by first real endpoint test | Commit `623077c` — `tests/contract/` framework + csrf. Expansion **DEC-094** slice 1: `/ping` + `/health/live` typed `response_model` + OpenAPI HTTP contracts. Next: `/health`+`/ready` (DB), 401/422, auth list |
