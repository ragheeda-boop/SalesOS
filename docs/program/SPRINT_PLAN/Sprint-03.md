...
| STORY-02-02 (middleware.ts) | FE-Lead | P0 | High | **DONE** | Client-side-only auth gating removed; server-side redirect verified | Commit `3f4b3c8` on master — `middleware.ts` + cookie session sync + unit tests. **DEC-095** closes browser verify: live `next dev` probe — `GET /dashboard` (no cookies) → **307** `/login?callbackUrl=%2Fdashboard`; `/` + `/login` → **200** (**browser-validated**). Prior DEC-088 units **14/14**. Optional authenticated `smoke-ui.ps1` **not run**. **CI GREEN not met.** |
| STORY-02-03 (JWT audience split, groundwork) | BE1 | P1 | Medium | **DONE** | Issuer/audience claim structure defined (not yet consumed — EPIC-04 consumes it) | Commit `2379e5f` — `jwt_audience=salesos-api` / `jwt_owner_audience=salesos-owner-platform`, owner mint/verify helpers, unit tests. Consumption **CLOSED** (DEC-093) — Owner admin wires `decode_owner_*`. Groundwork **light validated**; consumption host pytest **14/14 PASS**. |
...
| STORY-03-04 (contract test framework) | DevOps/SRE* | P1 | Medium | **DONE** | Template merged, used by first real endpoint test | Commit `623077c` — `tests/contract/` framework + csrf. Expansion **DEC-094** slice 1+2: `/ping`+`/health/live`+`/health`+`/health/ready` typed `response_model` + OpenAPI HTTP contracts. Next: 401/422 (honest error shapes), auth list |
...
3. **Parallel (not critical path):** **CI GREEN not met** (MyPy/CI-20, pip-audit residual, npm audit/CI-14, Trivy fs, etc.; Jest-debt CLOSED DEC-077); STORY-02-02 **DONE** (DEC-095 — browser redirect validated). Continue READY tracks.
