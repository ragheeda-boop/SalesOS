...
| STORY-02-03 (JWT audience split, groundwork) | BE1 | P1 | Medium | **DONE** | Issuer/audience claim structure defined (not yet consumed — EPIC-04 consumes it) | Commit `2379e5f` — `jwt_audience=salesos-api` / `jwt_owner_audience=salesos-owner-platform`, owner mint/verify helpers, unit tests. Consumption **OPEN** (DEC-091) until Owner endpoints wire `decode_owner_*`. **light validated** — Docker **15 passed** (`deae7de` / [`SWARM_VALIDATION`](../../salesos/docs/audit/ga-engineering-audit/SWARM_VALIDATION_2026-08-01.md)); re-verify host pytest **7/7 PASS** (tip `9cfc890`) |
