...
| STORY-14-04 (penetration test) | Security (+ external firm) | P0 | Critical | **CLOSED (in-repo) / IN_REPO_READY:** brief+threat model+test plan+findings stub+vendor handoff+evidence dir+integrity harness landed (`salesos/docs/pentest/`, `docs/program/evidence/story-14-04/`). Crumb [`PHASE1_STORY_14_04_PENTEST_CRUMB.md`](../PHASE1_STORY_14_04_PENTEST_CRUMB.md). Acceptance “zero unresolved criticals” **not validated** (firm/retest). External firm / staging SSRF tabletop = **residual-external**. CI Security Scan ≠ pentest. No Production GO. Stage 6 SKIPPED. |
