...
|---|---|---|---|---|
| STORY-14-01 (load test, 50 tenants) | DevOps/SRE, BE-Lead | P0 | High | **CLOSED for BE (Stream A):** tip HTTP `/api/v1/load/*` @ `8a369f1`/`dd59a3f`. **Field HTTP tip path light/build validated** on Railway Active `b95db185` (phases 1–5 harness exit 0 ×2; both profiles within_slo; evidence `.tmp-1401-http-harness-now.json` + `.tmp-1401-railway-soak-evidence.json`). Optional: prefer login token for load harness (register JWT alone may 401). Crumb [`PHASE1_STORY_14_01_LOAD_SLO_CRUMB.md`](../PHASE1_STORY_14_01_LOAD_SLO_CRUMB.md). **Stale-image Health Gate CLOSED/covered** (`c0e4f6a`+): fresh `uptime_seconds` + `/api/v1/load/meta` ≠ 404; log-stream false-RED CLOSED @ `654b33e`. Real 2h soak: **OPTIONAL** Board residual (not required for BE/HTTP tip-path close; crumbs say “if Board requires/wants”). No new RLS. Companion ≠ acceptance. Do **not** reopen hang for SIM105 tip lint. Live prod kill / Production GO / GA GO not claimed. Stage 6 SKIPPED. |
