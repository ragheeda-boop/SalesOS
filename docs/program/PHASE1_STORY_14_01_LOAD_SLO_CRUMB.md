...
| Field 2h soak (optional wall-clock) | **PASS (honest r3)** — r2 remains FAIL | **r3:** `2026-08-03T20:03:42Z` → `22:03:42Z`. `true_2h_wall_clock_achieved=true`, `all_iters_ok=true`, 12/12 harness_exit=0, profiles within_slo, CSRF `minted` each iter. Evidence `.tmp-1401-field-soak-r3/soak_final.json` (CSRF fix `3506135`). **r2:** wall yes / `all_iters_ok=false` (CSRF 403) — historical FAIL. Periodic HTTP harness ≠ continuous k6. **Not Companion acceptance. Not Production GO.** FE-SEC-02 flags-on https verify **HANDOFF READY** ([`PHASE1_FE_SEC_02_DEVOPS_FLAGS_ON_HANDOFF.md`](PHASE1_FE_SEC_02_DEVOPS_FLAGS_ON_HANDOFF.md); flags OFF until DevOps enable window; lineage ~`100cce8`/`3fccbe6`). |
...
| Field 2h soak | **PASS (optional r3 evidence)** — wall 2h + all_iters_ok; CSRF minted ×12; r2 historical FAIL. Not Companion / not Production GO |
